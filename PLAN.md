...
- despite those improvements, the first-output transient parity remains blocked: current native probes are still off by about `5.46e-1` (`recycling_1d_one_step`, worst field `Nd+`) and about `6.14e-1` (`recycling_dthe_one_step`, worst field `NVt+`) in the trimmed active domain, so Step 2 is now narrowed to the remaining target-boundary transient mismatch rather than the shared solver/RHS backbone
