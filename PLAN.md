...
| `integrated_2d_recycling_one_step` | `native-scaffolded` | First native transient scaffold now starts from the staged one-RHS dump state, marches one native recycling step on the shared implicit backbone, and uses the same dump-backed `SNd`/`SNd+` and `SPd`/`SPd+` staging during the step that already tightened the staged RHS path; parity is not locked yet, but the remaining meaningful live mismatch is now concentrated in the final `NVd+` target-corner update rather than the staged recycling diagnostics or broad pressure fields. |
