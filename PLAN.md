...
short-window prefix gate now finishes inside the local ten-minute policy on
the first three saved intervals and lands around `center Nh ≈ 9.24e-2`,
...
- the first controller-oriented validation lane is also explicit now:
  ...
  - the full closeout coverage audit is now also explicitly bounded by runtime: the current monolithic audit slice still times out at `600s`, so the remaining coverage push needs split critical-path coverage slices and a staged release threshold rather than one single giant local gate
...
| `neutral_mixed_short_window` | `open native runner target` | Baseline exists; the native implicit runner now executes this lane, and a bounded native prefix gate now covers the first three saved intervals inside the local ten-minute policy, but the broader short-window surface is still too loose for promotion. |
