...
1. Promote direct tokamak geometry from staged support to native production support.
   ...
   - `tokamak_recycling_rhs` is now the active next rung on that lane:
     ...
     - that same heat-transport family is now widened to an exact early-window transient rung too: `tokamak_heat_transport_short_window` now carries the curated harmless overrides `nout=2` and `e:diagnose=false`, has committed summary/array baselines, and the native direct tokamak path is the next exact compare to lock on `Pe`;
     ...
     - `tokamak_linear_transport_one_step` on `examples/tokamak-2D/linear-transport` is now staged as the next low-risk exact target, curated with `e:diagnose=false`; its raw one-step output interval is still too slow to promote as an exact gate in this pass, so it should stay the next tokamak-family target while the now-practical `tokamak_heat_transport_short_window` compare is locked.
...
- that direct-geometry move is now started with a stable family rather than a single rung: `tokamak_diffusion_flow_one_step`, `tokamak_diffusion_transport_one_step`, `tokamak_diffusion_transport_short_window`, and `tokamak_heat_transport_one_step` all run through the reference harness at `process_count = 6`, stage the shared parent-directory `tokamak.nc` mesh deterministically, and match their committed summary and array baselines exactly on the native direct tokamak dump-backed path. The next tokamak move should now widen to a multi-output `heat-transport` rung or the next neighboring direct tokamak transport family rather than returning immediately to harder drift/turbulence cases.
