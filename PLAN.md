...
- the latest production-target probes show that two naive fixes do not close that remaining gap: staging `SPe` as a total electron-pressure source reduces the corner-cell `Pe` error but worsens the neighboring target-band cell, and disabling dump-target preservation improves `Pe` while badly degrading `Nd+`, `Pd+`, and `NVd+`. The next production fix therefore needs to target the target-band transient update itself rather than broader source replacement or a looser target-state policy.
