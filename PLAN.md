...
- the current one-step path no longer depends on committed communicated-guard replay inside the active tokamak recycling solve. On the promoted compare surface, `tokamak_recycling_dthe_one_step` now clears a mixed exact gate: the non-negligible ion/electron fields stay below a `5e-2` scaled diff band while the near-zero neutral channels remain inside a small absolute band. The first richer direct-window checks are now bounded too: a live Hermes-backed `nout=2` probe on the same D/T/He surface stays inside a mixed operational band with `near_zero_atol = 5e-5`, worst scaled diff `NVd+ ≈ 2.74e-2`, and worst near-zero absolute diff `Pd ≈ 2.67e-5` under the local ten-minute policy; the drift-enabled `tokamak_recycling_dthe_drifts_one_step` lane now also clears a bounded live `nout=2` operational gate with worst scaled diff `NVhe+ ≈ 2.87e-2`; and the neon-enabled `tokamak_recycling_dthene_one_step` lane now also clears a live Hermes-backed mixed operational gate with `near_zero_atol = 1e-4`, worst scaled diff `Nt ≈ 5.81e-4`, and worst near-zero absolute diff `Pd ≈ 8.42e-5`. The remaining broader blocker is therefore no longer the existence of any richer or neon-enabled surface, but extending that same native behavior to longer direct tokamak recycling windows and eventually to fully native distributed guard evolution.
...
longer blocked at the runner-dispatch layer, and the compare surface itself is
now shaped correctly on named manual runs because `run_input_case()` resolves
the curated trim metadata automatically. The native implicit runner now
executes both `neutral_mixed_one_step` and `neutral_mixed_short_window`, and
the latest bounded native one-step compare is materially tighter after fixing
the neutral density wall-guard reconstruction: the committed centerline gate
now lands around `center Nh ≈ 8.0e-3`, `center Ph ≈ 5.7e-4`, `center NVh ≈
8.6e-4`, `center T ≈ 2.9e-4`, and `momentum RMS ≈ 1.7e-3` max-abs error
against the committed baseline. The bounded full short-window metric gate now
also finishes inside the local ten-minute policy on the matrix-free path and
lands around `center Nh ≈ 8.03e-3`, `center Ph ≈ 6.47e-4`, `center NVh ≈
8.60e-4`, `center T ≈ 2.91e-4`, `total Nh ≈ 3.24e-1`, `total Ph ≈ 2.89e-2`,
and `momentum RMS ≈ 1.71e-3`. The remaining neutral gap is therefore no longer
the existence of a global short-window metric gate, but whether the broader
full-array short-window field surface is needed for the broader standalone
claim.
...
- the first controller-oriented validation lane is also explicit now:
  ...
  - this closes the first honest controller-feedback lane without overstating full production `temperature_feedback` or `detachment_controller`
  - `detachment_controller_campaign` now promotes a materially broader bounded Hermes-backed detachment-controller lane on the local reference build by staging the `1D-recycling-with-detachment-control` example onto a reduced `cvode` deck (`ny=32`, `nout=24`, `timestep=100`), stripping the incompatible `beuler`-only solver options, forcing `settling_time = 0`, and validating exact saved-diagnostic controller identities plus a nontrivial control response on that bounded lane
  - `temperature_feedback_campaign` is now also a real bounded reduced Hermes-backed lane: it streams the local reference run directly into `run.stdout`, strips the incompatible `beuler`-only solver options when staging a reduced `cvode` deck, auto-prepares a clean patched reference worktree when the local Hermes source still carries the known `temperature_feedback.hxx` permission bug, and validates exact multiplier/proportional/source balance plus bounded integral reconstruction and target-temperature error reduction on `ny=16`, `nout=4`, `timestep=100`
  - that closes the bounded temperature-controller existence proof cleanly and without modifying the user's dirty reference tree
  - the remaining control-oriented blocker is now narrower: promote a broader production temperature/detachment-control workflow beyond the reduced controller lanes rather than proving that *any* bounded controller lane can exist
  - the closeout coverage threshold is now explicit and passing: `scripts/run_closeout_coverage.py` enforces `95%` on the bounded controller/runtime/profile/audit closeout slice, while the broader monolithic repo matrix remains a separate long-horizon hardening target rather than the local ship gate
...
| `neutral_mixed_short_window` | `native operational target` | Baseline exists; the native implicit runner now executes this lane, the matrix-free path clears a bounded full short-window metric gate inside the local ten-minute policy including total-density and total-pressure histories, and the trimmed active-domain `Nh`/`Ph`/`NVh` surface now also clears a bounded full-array short-window field gate. |
...
| `recycling_1d_short_window` | `native operational target` | First curated repeated-output open-field recycling rung (`nout=5`); the latest bounded native compare stays around `Nd+ ≈ 4.55e-2`, `Pd+ ≈ 6.13e-2`, `NVd+ ≈ 1.05e-1`, `Nd ≈ 2.61e-2`, `Pd ≈ 7.34e-3`, `NVd ≈ 2.87e-3`, `Pe ≈ 1.99e-2`, so the local native backbone is materially tighter but not yet exact. |
