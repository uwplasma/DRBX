...
1. Promote direct tokamak geometry from staged support to native production support.
   ...
   - `tokamak_recycling_rhs` is now the active next rung on that lane:
     ...
     - the next concrete deliverable after that is to decide whether the slow `tokamak_heat_transport_short_window` history should be promoted to an exact gate or remain a staged sidecar while the direct tokamak family widens further.
