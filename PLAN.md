...
| `integrated_2d_recycling_one_step` | `scaffolded operational target` | Dump-backed one-step integrated recycling baseline is now committed and the staged transient path stays inside a bounded operational band led by `Sd_target_recycle`, then tiny `Ed_target_recycle`, `Pd+`, and `NVd+` residuals. |
| `integrated_2d_recycling_short_window` | `scaffolded operational target` | The staged integrated 2D recycling short-window path now has bounded operational tests against the committed baseline; residuals remain small and are still led by `Sd_target_recycle`, `Ed_target_recycle`, and then `Pd+` / `NVd+`. |
| `integrated_2d_recycling_medium_window` | `scaffolded operational target` | The staged integrated 2D recycling medium-window path also now clears a bounded operational band against the committed baseline. This remains scaffolded evidence, not a fully native claim-bearing lane, because the transient still relies on dump-backed source/target preservation. |
