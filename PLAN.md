...
- the first controller-oriented validation lane is also explicit now:
  ...
  - `temperature_feedback_campaign` now streams the local reference run directly into `run.stdout` instead of capturing the full solver output in-memory, which removes the pipe-fill failure mode from the bounded controller probe and adds explicit stage/run/load/reconstruction timings to the artifact summary; it now also strips the incompatible `beuler`-only solver options when staging a reduced `cvode` deck, but the bounded local Tt-control example still does not finish inside the current ten-minute policy here
  ...
  - the remaining control-oriented blocker is now narrower: promote a broader production temperature/detachment-control workflow beyond the reduced detachment-controller lane rather than proving that *any* bounded controller lane can exist
...
| `neutral_mixed_short_window` | `open native runner target` | Baseline exists; the native implicit runner now executes this lane, and the matrix-free path now clears a bounded full short-window metric gate inside the local ten-minute policy, including total-density and total-pressure histories. The remaining gap is the broader full-array short-window field surface rather than the existence of any global short-window transient gate. |
