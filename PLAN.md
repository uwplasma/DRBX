...
| `integrated_2d_recycling_one_step` | `native operational target` | Dump-backed one-step integrated recycling baseline is now committed and the native transient path stays inside a bounded operational band. The path now computes `Sd_target_recycle` and `Ed_target_recycle` natively at every saved time, including `t=0`, and it no longer depends on dump-backed density, pressure, or momentum source replay or on dump target preservation during the transient. It still starts from a dump-seeded initial state. |
| `integrated_2d_recycling_short_window` | `native operational target` | The integrated 2D recycling short-window path now has bounded operational tests against the committed baseline; residuals remain small and are still led by `Sd_target_recycle`, `Ed_target_recycle`, and then `Pd+` / `NVd+`. The saved diagnostics are native across the whole window, and the transient no longer replays dump-backed source fields or preserves dump target state, but it still starts from a dump-seeded initial state. |
| `integrated_2d_recycling_medium_window` | `native operational target` | The integrated 2D recycling medium-window path also now clears a bounded operational band against the committed baseline. It is still operational rather than exact because the transient starts from a dump-seeded initial state even though the saved recycling diagnostics are recomputed natively and the remaining dump target preservation has been removed. |
