...
1. Finish the Step 3 integrated 2D production lane.
   ...
   - Current blocker is now localized: on the reference-evolved one-step state, `Sd_target_recycle` / `Ed_target_recycle` are exact, `ddt(Nd+)` collapses once the diagnosis uses the evolved one-step source fields, and the remaining meaningful target-band RHS leaders are `ddt(Pe)` first, then `ddt(Pd+)`, then `ddt(NVd+)`, with `ddt(Nd)` smaller but still visible on the first accepted step.
