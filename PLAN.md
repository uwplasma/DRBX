...
- that direct-geometry move is now started with a stable rung: `tokamak_diffusion_flow_one_step` on `examples/tokamak-2D/diffusion-flow-evolveT` now runs through the reference harness at `process_count = 6`, stages the shared parent-directory `tokamak.nc` mesh deterministically, and matches its committed summary and array baselines exactly on a native dump-backed one-step scaffold for `Nh`, `Ph`, and `NVh`. The next tokamak move should extend outward from this case family rather than returning immediately to the still-broken tokamak recycling examples.
