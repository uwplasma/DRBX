...
- the current one-step path no longer depends on committed communicated-guard replay inside the active tokamak recycling solve. The surviving `tokamak_recycling_dthe_one_step` residuals still sit at about `NVd+ ≈ 6.75e-3`, `NVt+ ≈ 5.87e-3`, `Pd+ ≈ 5.22e-3`, `Pt+ ≈ 4.04e-3`, and `Pe ≈ 1.67e-4` on the committed compare surface, which means the remaining blocker is the active native transient itself rather than cached guard restoration.
...
| `recycling_1d_one_step` | `native operational target` | Native first-step transient now sits inside a bounded operational relative band (`rtol <= 2e-1`) and is good enough to unblock the 2D closure path, but it is not yet exact. |
| `recycling_dthe_one_step` | `native operational target` | Native first-step transient now sits inside the bounded operational relative band (`rtol <= 2e-1`) and is good enough to unblock the 2D closure path, but it is not yet exact. |
...
| `integrated_2d_recycling_one_step` | `scaffolded_reference_backed` | Dump-backed one-step integrated recycling baseline is now committed and the staged transient path stays inside a bounded operational band. The staged path now computes `Sd_target_recycle` and `Ed_target_recycle` natively at every saved time, including `t=0`, and it no longer depends on dump-backed density, pressure, or momentum source replay. It still relies on dump-seeded initial state and target-state preservation. |
| `integrated_2d_recycling_short_window` | `scaffolded_reference_backed` | The staged integrated 2D recycling short-window path now has bounded operational tests against the committed baseline; residuals remain small and are still led by `Sd_target_recycle`, `Ed_target_recycle`, and then `Pd+` / `NVd+`. The saved diagnostics are native across the whole window, and the transient no longer replays dump-backed source fields, but it is still staged through dump-seeded initial state and target preservation. |
| `integrated_2d_recycling_medium_window` | `scaffolded_reference_backed` | The staged integrated 2D recycling medium-window path also now clears a bounded operational band against the committed baseline. This remains scaffolded evidence, not a fully native claim-bearing lane, because the transient still relies on dump-seeded initial state and target preservation even though the saved recycling diagnostics are recomputed natively and the dump-backed source replay is gone. |
