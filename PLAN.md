...
1. Promote direct tokamak geometry from staged support to native production support.
   ...
   - `tokamak_recycling_rhs` is now the active next rung on that lane:
     ...
     - the next concrete deliverable after that is `tokamak_linear_transport_one_step` on `examples/tokamak-2D/linear-transport`, keeping the compare surface at `Pe` and treating the slow `tokamak_heat_transport_short_window` history as a staged sidecar rather than the main critical path.
