...
| `integrated_2d_recycling_medium_window` | `native-scaffolded` | The integrated 2D recycling transient path now also honors the staged `nout=20` medium-window manifest override and can march the longer window on the same dump-backed source-preserving workflow. The remaining live differences are still concentrated in `Ed_target_recycle`, `Sd_target_recycle`, `Pe`, and small neutral-side residuals, so this extends the Step 3 transient ladder without claiming locked parity yet. |
