...
- a new one-step production step diagnostic now compares both the native evolved state and the native RHS evaluated on the reference-evolved state. That result is stronger than the older end-state-only traces: on `integrated_2d_production_one_step`, `Sd_target_recycle` and `Ed_target_recycle` are exact on the reference-evolved state, `ddt(NVd+)` is already small, but `ddt(Pe)` is still badly wrong at the target-band cell and `ddt(Nd+)` / `ddt(Pd+)` / `ddt(Nd)` are also still off there. The next Step 3 work should therefore focus on the local target-band pressure/density RHS assembly rather than more recycling-diagnostic or momentum-side tuning.
