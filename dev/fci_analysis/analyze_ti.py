...
TI_TERM_SPECS = build_term_specs(
    (
        ("exb", "ExB"),
        ("parallel_advection", "-Vi d_parallel Ti"),
...
def _ti_rhs_term_fields(
    ...
    *,
    ...
    Ti_stencil = local_stencil_builder(
        Ti,
        geometry,
        PERIODIC_AXES,
        boundary_conditions.Ti_face_bc,
        cut_wall_geometry,
        boundary_conditions.Ti_cut_wall_bc,
    )
    ...
    Pe = density * jnp.asarray(state.Te, dtype=jnp.float64)
    ...
    exb = -(poisson_bracket_op(potential_stencil, Ti_stencil, geometry) / (rho_star * bmag))
    parallel_advection = -(Vi * grad_parallel_op_direct(Ti_stencil, geometry))
    curvature_Pe = curvature_op(Pe_stencil, geometry, curvature_coefficients=curvature_coefficients)
    curvature_Ti = curvature_op(Ti_stencil, geometry, curvature_coefficients=curvature_coefficients)
    ...
    curvature_pressure = (4.0 * Ti / (3.0 * bmag)) * (curvature_Pe / density_safe)
    curvature_temperature = (4.0 * Ti / (3.0 * bmag)) * (-2.5 * tau * curvature_Ti)
    curvature_potential = (4.0 * Ti / (3.0 * bmag)) * (-curvature_potential_op)
    parallel_current_term = (2.0 * Ti / (3.0 * density_safe)) * parallel_current_density
    parallel_vi_term = (2.0 * Ti / (3.0 * density_safe)) * (-density_safe * parallel_Vi)

    parallel_diffusion = jnp.zeros_like(Ti)
    ...
    perp_diffusion = jnp.zeros_like(Ti)
    if float(D_perp) != 0.0:
        if Ti_conservative_stencil is None:
            raise ValueError("Ti conservative stencil is required when ion_temperature_D_perp is nonzero")
    ...
    return {
        ...
        "parallel_vi": np.asarray(parallel_vi_term, dtype=np.float64),
...
FIELD_SPEC = FieldAnalyzerSpec(
    ...
    evaluate_terms=_ti_rhs_term_fields,
    legend_ncol=4,
)


def _ti_profile_history(ti_history: np.ndarray, geometry) -> np.ndarray:
    ti = jnp.asarray(ti_history, dtype=jnp.float64)
    if ti.ndim != 4:
        raise ValueError(f"ti_history must have shape (time, nx, ny, nz), got {ti.shape}")

    jacobian = jnp.asarray(geometry.cell_metric.J, dtype=jnp.float64)
    if jacobian.shape != ti.shape[1:]:
        raise ValueError(f"geometry jacobian shape {jacobian.shape} does not match Ti cell shape {ti.shape[1:]}")

    radial_weight = jnp.sum(jacobian, axis=(1, 2))
    if np.any(np.asarray(radial_weight == 0.0, dtype=bool)):
        raise ValueError("cannot build Ti profile with a zero radial jacobian weight")

    profile = jnp.sum(ti * jacobian[None, :, :, :], axis=(2, 3)) / radial_weight[None, :]
    return np.asarray(profile, dtype=np.float64)


def _save_ti_profile_movie(
    times: np.ndarray,
    ...
    *,
    ...
    profiles = _ti_profile_history(ti_history, geometry)
    ...
    if finite_profiles.size == 0:
        raise ValueError("cannot build Ti profile movie from non-finite profile values")
    ...
    ax.set_ylabel("J-weighted Ti profile")
...
def _ti_profile_movie_path(rhs_plot_path: Path) -> Path:
    ...
    return rhs_plot_path.parent / f"{artifact_stem}_ti_profile.gif"
...
def main(argv: Sequence[str] | None = None) -> Path:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Frame stride to use when rendering the Ti profile GIF.",
    )
    ti_args, remaining_argv = parser.parse_known_args(argv)
    ...
    ti_shape = tuple(int(size) for size in np.asarray(ti_history).shape[1:])
    geometry = _build_eb_blob_geometry(ti_shape, construct_fci_maps=False)
    movie_path = _ti_profile_movie_path(rhs_plot_path)
    _save_ti_profile_movie(
        np.asarray(times, dtype=np.float64),
        np.asarray(ti_history, dtype=np.float64),
        geometry,
        output_path=movie_path,
        title="EB blob J-weighted Ti profile",
        frame_stride=int(ti_args.frame_stride),
    )
    print(f"saved Ti profile movie to {movie_path}", flush=True)
