...
VORTICITY_TERM_SPECS = build_term_specs(
    (
        ("exb", "ExB"),
        ("parallel_advection", "-Vi d_parallel vorticity"),
        ("parallel_current_density", "parallel current"),
...
def _vorticity_rhs_term_fields(
    state,
    *,
    geometry,
    parameters,
    ...
    vorticity_stencil = local_stencil_builder(
        vorticity,
        geometry,
        PERIODIC_AXES,
        boundary_conditions.vorticity_face_bc,
        cut_wall_geometry,
        boundary_conditions.vorticity_cut_wall_bc,
    )
    ...
    pressure = density * Te + jnp.asarray(parameters.tau, dtype=jnp.float64) * (density * Ti)
    ...
    current_density = density * (Vi - jnp.asarray(state.Ve, dtype=jnp.float64))
    ...
    exb = -(poisson_bracket_op(potential_stencil, vorticity_stencil, geometry) / (rho_star * bmag))
    parallel_advection = -(Vi * grad_parallel_op_direct(vorticity_stencil, geometry))
    parallel_current_density = (bmag * bmag / density_safe) * grad_parallel_op_direct(
        current_density_stencil,
        geometry,
    )
    curvature_pressure = (2.0 * bmag / density_safe) * curvature_op(
        pressure_stencil,
        geometry,
        curvature_coefficients=curvature_coefficients,
    )

    parallel_diffusion = jnp.zeros_like(vorticity)
    if float(vorticity_D_parallel) != 0.0:
        parallel_diffusion = vorticity_D_parallel * parallel_laplacian_direct_op(
            vorticity,
            geometry,
            face_bc=boundary_conditions.vorticity_face_bc,
            periodic_axes=PERIODIC_AXES,
        )

    perp_diffusion = jnp.zeros_like(vorticity)
    if float(vorticity_D_perp) != 0.0:
        if vorticity_conservative_stencil is None:
            raise ValueError("vorticity conservative stencil is required when vorticity_D_perp is nonzero")
        perp_diffusion = vorticity_D_perp * perp_laplacian_conservative_op(
            vorticity_conservative_stencil,
            geometry,
            face_bc=boundary_conditions.vorticity_face_bc,
            cut_wall_geometry=cut_wall_geometry,
            cut_wall_bc=boundary_conditions.vorticity_cut_wall_bc,
            periodic_axes=PERIODIC_AXES,
        )

    total = exb + parallel_advection + parallel_current_density + curvature_pressure + parallel_diffusion + perp_diffusion

    return {
        "exb": np.asarray(exb, dtype=np.float64),
        "parallel_advection": np.asarray(parallel_advection, dtype=np.float64),
        "parallel_current_density": np.asarray(parallel_current_density, dtype=np.float64),
...
FIELD_SPEC = FieldAnalyzerSpec(
    ...
    output_suffix="vorticity",
    term_specs=VORTICITY_TERM_SPECS,
    sum_keys=("exb", "parallel_advection", "parallel_current_density", "curvature_pressure", "parallel_diffusion", "perp_diffusion"),
    evaluate_terms=_vorticity_rhs_term_fields,
    legend_ncol=3,
)


def _vorticity_profile_history(vorticity_history: np.ndarray, geometry) -> np.ndarray:
    vorticity = jnp.asarray(vorticity_history, dtype=jnp.float64)
    if vorticity.ndim != 4:
        raise ValueError(f"vorticity_history must have shape (time, nx, ny, nz), got {vorticity.shape}")

    jacobian = jnp.asarray(geometry.cell_metric.J, dtype=jnp.float64)
    if jacobian.shape != vorticity.shape[1:]:
        raise ValueError(
            f"geometry jacobian shape {jacobian.shape} does not match vorticity cell shape {vorticity.shape[1:]}"
        )

    radial_weight = jnp.sum(jacobian, axis=(1, 2))
    if np.any(np.asarray(radial_weight == 0.0, dtype=bool)):
        raise ValueError("cannot build vorticity profile with a zero radial jacobian weight")

    profile = jnp.sum(vorticity * jacobian[None, :, :, :], axis=(2, 3)) / radial_weight[None, :]
    return np.asarray(profile, dtype=np.float64)


def _save_vorticity_profile_movie(
    times: np.ndarray,
    ...
    *,
    ...
    profiles = _vorticity_profile_history(vorticity_history, geometry)
    ...
    if finite_profiles.size == 0:
        raise ValueError("cannot build vorticity profile movie from non-finite profile values")
    ...
    ax.set_ylabel("J-weighted vorticity profile")
...
def _vorticity_profile_movie_path(rhs_plot_path: Path) -> Path:
    ...
    return rhs_plot_path.parent / f"{artifact_stem}_vorticity_profile.gif"
...
def main(argv: Sequence[str] | None = None) -> Path:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Frame stride to use when rendering the vorticity profile GIF.",
    )
    vorticity_args, remaining_argv = parser.parse_known_args(argv)
    ...
    vorticity_shape = tuple(int(size) for size in np.asarray(vorticity_history).shape[1:])
    geometry = _build_eb_blob_geometry(vorticity_shape, construct_fci_maps=False)
    movie_path = _vorticity_profile_movie_path(rhs_plot_path)
    _save_vorticity_profile_movie(
        np.asarray(times, dtype=np.float64),
        np.asarray(vorticity_history, dtype=np.float64),
        geometry,
        output_path=movie_path,
        title="EB blob J-weighted vorticity profile",
        frame_stride=int(vorticity_args.frame_stride),
    )
    print(f"saved vorticity profile movie to {movie_path}", flush=True)
