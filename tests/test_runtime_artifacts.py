...
from drbx.runtime import artifacts
...
def test_ensure_docs_media_restores_release_bundle(
    ...
    assert calls == [
        (
            ...
            root / ".drbx_artifact_cache" / artifacts.DOCS_MEDIA_ASSET,
...
def test_ensure_docs_media_uses_configured_cache_dir(
    ...
    monkeypatch.setenv("DRBX_ARTIFACT_CACHE_DIR", str(cache_dir))
    monkeypatch.delenv("DRBX_ARTIFACT_CACHE", raising=False)
    monkeypatch.delenv("DRBX_OFFLINE_ARTIFACTS", raising=False)
    monkeypatch.setattr(artifacts, "_download_release_asset", fake_download)
...
def test_urllib_download_retries_with_configured_timeout(
    ...
    monkeypatch.setenv("DRBX_ARTIFACT_DOWNLOAD_TIMEOUT", "0.5")
    monkeypatch.setenv("DRBX_ARTIFACT_DOWNLOAD_ATTEMPTS", "2")
