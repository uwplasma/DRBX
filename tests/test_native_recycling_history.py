...
def test_fixed_bdf2_active_array_history_aggregates_solver_diagnostics(
    ...
    def make_info() -> recycling.Recycling1DImplicitStepInfo:
        return recycling.Recycling1DImplicitStepInfo(
            ...
            diagnostics={
                ...
                "linear_preconditioner_apply_seconds": 0.0625,
                "initial_residual_mode": "linearize",
            },
        )
    ...
    assert diagnostics["fixed_bdf2_total_linear_preconditioner_apply_count"] == 10
    assert diagnostics[
        "fixed_bdf2_total_linear_preconditioner_apply_seconds"
    ] == pytest.approx(0.125)
    assert diagnostics["fixed_bdf2_total_linear_operator_call_count"] == 14
    assert diagnostics[
...
def test_record_adaptive_bdf_step_solver_info_counts_convergence_states() -> None:
    ...
    recycling._record_adaptive_bdf_step_solver_info(
        ...
        SimpleNamespace(
            ...
            diagnostics={
                ...
                "linear_operator_call_count": 19,
                "linear_operator_dispatch_seconds": 0.0625,
                ...
                "linear_preconditioner_apply_count": 23,
                "linear_preconditioner_apply_seconds": 0.045,
    ...
    assert stats["adaptive_bdf_linear_operator_call_count"] == 19
    ...
    assert stats["adaptive_bdf_linear_preconditioner_apply_count"] == 23
    assert stats["adaptive_bdf_linear_preconditioner_apply_seconds"] == pytest.approx(
        0.045
    )
    ...
    assert stats["adaptive_bdf_linear_operator_dispatch_seconds"] == pytest.approx(
        0.0625
    )
...
def test_adaptive_bdf_interval_stats_accumulates_timing_fields() -> None:
    ...
    step["adaptive_bdf_linear_operator_dispatch_seconds"] = 6.5
    ...
    step["adaptive_bdf_linear_preconditioner_apply_count"] = 19
    step["adaptive_bdf_linear_preconditioner_apply_seconds"] = 7.75
    ...
    step["adaptive_bdf_linear_operator_call_count"] = 18
    ...
    assert total["adaptive_bdf_linear_operator_dispatch_seconds"] == pytest.approx(
        6.5
    )
    ...
    assert total["adaptive_bdf_linear_preconditioner_apply_count"] == 19
    assert total["adaptive_bdf_linear_preconditioner_apply_seconds"] == pytest.approx(
        7.75
    )
    ...
    assert total["adaptive_bdf_linear_operator_call_count"] == 18
