...
def test_momentum_line_preconditioner_reduces_selected_stiff_operator_budget() -> None:
    jnp = pytest.importorskip("jax.numpy")

    active_shape = (1, 12)
    line_length = active_shape[1]
    density_weight = 0.5
    momentum_matrix = np.diag(4.0 * np.ones(line_length))
    momentum_matrix += np.diag(-1.95 * np.ones(line_length - 1), k=1)
    momentum_matrix += np.diag(-1.95 * np.ones(line_length - 1), k=-1)
    momentum_matrix = jnp.asarray(momentum_matrix, dtype=jnp.float64)
    density_target = jnp.cos(jnp.linspace(0.0, 2.0, line_length, dtype=jnp.float64))
    momentum_target = jnp.sin(jnp.linspace(0.0, 3.0, line_length, dtype=jnp.float64))
    target = jnp.concatenate((density_target, momentum_target))

    def residual(state):
        vector = jnp.asarray(state, dtype=jnp.float64)
        density = vector[:line_length]
        momentum = vector[line_length:]
        return jnp.concatenate(
            (
                density_weight * (density - density_target),
                momentum_matrix @ (momentum - momentum_target),
            )
        )
    ...
    preconditioned_solution, preconditioned_info = solve_jax_linearized_newton_system(
        ...
        np.zeros(2 * line_length, dtype=np.float64),
        **common_solver_options,
        linear_preconditioner_name="momentum_line",
        linear_preconditioner_context={
            "active_shape": active_shape,
            "field_count": 2,
            "feedback_count": 0,
    ...
    assert preconditioned_info.linear_preconditioner == "momentum_line"
    assert preconditioned_info.linear_preconditioner_build_count == 1
    assert preconditioned_info.linear_preconditioner_apply_count > 0
    np.testing.assert_allclose(
        preconditioned_solution,
...
def test_sheath_line_preconditioner_reduces_selected_stiff_operator_budget() -> None:
    jnp = pytest.importorskip("jax.numpy")

    active_shape = (1, 12)
    line_length = active_shape[1]
    weak_weight = 0.5
    sheath_matrix = np.diag(4.0 * np.ones(line_length))
    sheath_matrix += np.diag(-1.95 * np.ones(line_length - 1), k=1)
    sheath_matrix += np.diag(-1.95 * np.ones(line_length - 1), k=-1)
    sheath_matrix = jnp.asarray(sheath_matrix, dtype=jnp.float64)
    electron_target = jnp.cos(jnp.linspace(0.0, 2.0, line_length, dtype=jnp.float64))
    neutral_target = jnp.sin(jnp.linspace(0.0, 1.0, line_length, dtype=jnp.float64))
    ion_target = jnp.sin(jnp.linspace(0.0, 3.0, line_length, dtype=jnp.float64))
    target = jnp.concatenate((electron_target, neutral_target, ion_target))

    def residual(state):
        vector = jnp.asarray(state, dtype=jnp.float64)
        electron = vector[:line_length]
        neutral = vector[line_length : 2 * line_length]
        ion = vector[2 * line_length :]
        return jnp.concatenate(
            (
                sheath_matrix @ (electron - electron_target),
                weak_weight * (neutral - neutral_target),
                sheath_matrix @ (ion - ion_target),
            )
        )

    common_solver_options = dict(
        active_shape=(3 * line_length,),
        residual_tolerance=1.0e-10,
        step_tolerance=1.0e-12,
        max_nonlinear_iterations=2,
        linear_restart=3,
    ...
    unpreconditioned_solution, unpreconditioned_info = (
        solve_jax_linearized_newton_system(
            residual,
            np.zeros(3 * line_length, dtype=np.float64),
            **common_solver_options,
        )
    )
    preconditioned_solution, preconditioned_info = solve_jax_linearized_newton_system(
    ...
    assert preconditioned_info.linear_preconditioner == "sheath_line"
    assert preconditioned_info.linear_preconditioner_build_count == 1
    assert preconditioned_info.linear_preconditioner_apply_count > 0
    np.testing.assert_allclose(
        preconditioned_solution,
...
def test_field_block_sample_preconditioner_uses_representative_field_coupling() -> None:
    jnp = pytest.importorskip("jax.numpy")
    ...
    def linear_map(vector):
        ...
        return jnp.concatenate((coupled_fields, 10.0 * flat[field_unknown_count:]))
...
def test_field_block_sample_preconditioner_reduces_coupled_field_budget() -> None:
    ...
    target_fields = jnp.stack(
        (
            ...
            jnp.linspace(-0.25, 0.75, active_cell_count, dtype=jnp.float64),
        ),
        axis=0,
    )
    target = target_fields.reshape((field_unknown_count,))

    def residual(state):
        fields_by_cell = jnp.asarray(state, dtype=jnp.float64).reshape(
            (field_count, active_cell_count)
        ).transpose((1, 0))
        target_by_cell = target_fields.transpose((1, 0))
        residual_by_cell = (fields_by_cell - target_by_cell) @ field_block.T
        return residual_by_cell.transpose((1, 0)).reshape((field_unknown_count,))
    ...
    assert preconditioned_info.linear_preconditioner == "field_block_sample"
    assert preconditioned_info.linear_preconditioner_build_count == 1
    assert preconditioned_info.linear_preconditioner_apply_count > 0
    np.testing.assert_allclose(
        preconditioned_solution,
...
def test_jax_linearized_newton_solver_reports_actual_attempts_after_stall() -> None:
    ...
    solution, info = solve_jax_linearized_newton_system(
        lambda state: jnp.asarray(state) ** 2 + 1.0,
        np.array([1.0], dtype=np.float64),
        active_shape=(1,),
        residual_tolerance=1.0e-12,
        step_tolerance=1.0e-12,
    ...
    assert info.residual_inf_norm == pytest.approx(1.0)
    assert info.nonlinear_iterations == 2
    assert info.linear_solve_count == 2
    assert info.nonlinear_iterations < 8
