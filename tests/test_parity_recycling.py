...
def _assert_live_tokamak_recycling_dthene_five_output_window_operational_mixed_band() -> None:
    if os.environ.get("JAX_DRB_RUN_RECYCLING_2D_PARITY") != "1":
        pytest.skip("set JAX_DRB_RUN_RECYCLING_2D_PARITY=1 to run the bounded parity gate for the richer neon-enabled tokamak recycling short window")
    ...
    with tempfile.TemporaryDirectory(prefix="jaxdrb-dthene-five-output-window-") as workdir:
        execution = run_reference_case(
            case.name,
            reference_root=_REFERENCE_ROOT,
        ...
        expected = build_dataset_array_payload(
            Path(summary.artifacts["BOUT.dmp.0.nc"]),
            case_name="tokamak_recycling_dthene_short_window",
            parity_mode="short_window",
            capability_tier=probe_case.capability_tier,
            compare_variables=probe_case.compare_variables,
            component_labels=tuple(summary.component_labels),
    ...
    assert worst_relative < 5.0e-3, entries
    assert worst_near_zero_abs_diff < 5.0e-4, near_zero_entries
...
def test_tokamak_recycling_dthene_five_output_window_native_parity_stays_within_operational_mixed_band() -> None:
    _assert_live_tokamak_recycling_dthene_five_output_window_operational_mixed_band()
