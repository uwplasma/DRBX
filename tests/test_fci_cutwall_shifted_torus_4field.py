...
from jax_drb.native.fci_model import inject_owned_field_to_halo, inject_owned_state_to_halo
    ...
    for resolution in resolutions:
        ...
        with make_mesh_for_shard_counts(shard_counts) as mesh:
            ...
            for operator_name in operator_names:
                ...
                if False and operator_name == "perp_laplacian_phi":
                    target_shard = tuple(
                        int(top_index[axis]) // int(owned_shape[axis])
                        for axis in range(3)
                    )
                    ...
                    def perpendicular_flux_diagnostic_kernel(
                        phi_owned,
                        local_invariants,
                        control_volume_geometry,
                        stage_time,
                        ...
                        direct_regular_flux = jnp.zeros(
                            (3, 2),
                            dtype=jnp.float64,
                        )
                        exact_direct_regular_flux = jnp.zeros(
                            (3, 2),
                            dtype=jnp.float64,
                        )
                        direct_regular_gradient = jnp.zeros(
                            (3, 2, 3),
                            dtype=jnp.float64,
                        )
                        exact_direct_regular_gradient = jnp.zeros(
                            (3, 2, 3),
                            dtype=jnp.float64,
                        )
                        direct_open_measure = jnp.zeros(
                            (3, 2),
                            dtype=jnp.float64,
                        )
                        direct_open_mask = jnp.zeros(
                            (3, 2),
                            dtype=bool,
                        )
                        storage_regular_sum = jnp.zeros(
                            local_geometry_value.owned_shape,
                            dtype=jnp.float64,
                        )
                        for axis in range(3):
                            ...
                            for side in range(2):
                                ...
                                face_index[axis] += side
                                face_index_tuple = tuple(face_index)
                                orientation = -1.0 if side == 0 else 1.0
                                direct_regular_flux = (
                                    direct_regular_flux.at[axis, side].set(
                                        orientation
                                        * integrated_face[face_index_tuple]
                                    )
                                )
                                direct_open_measure = (
                                    direct_open_measure.at[axis, side].set(
                                        open_measure[face_index_tuple]
                                    )
                                )
                                face_is_open = (
                                    jnp.asarray(
                                ...
                                direct_open_mask = (
                                    direct_open_mask.at[axis, side].set(
                                        face_is_open
                                    )
                                )
                                direct_regular_gradient = (
                                    direct_regular_gradient.at[
                                        axis,
                                        side,
                                    ].set(
                                        jnp.asarray(
                                            regular_gradient[axis],
                                            dtype=jnp.float64,
                                        )[face_index_tuple]
                                    )
                                )
                                ...
                                for tangential_slot, tangential_axis in (
                                    enumerate(tangential_axes)
                                ):
                                    face_points = face_points.at[
                                        :,
                                        tangential_axis,
                                ...
                                (
                                    ...
                                    _exact_face_g_contra,
                                    _exact_face_g_cov,
                                    _exact_face_B_contra,
                                    _exact_face_Bmag,
                                    exact_face_projector,
                                ) = _shifted_torus_metric_payload_jax(
                                    face_points
                                )
                                exact_flux_density = exact_face_J * (
                                    jnp.einsum(
                                        "qi,qi->q",
                                        exact_face_projector[
                                            :,
                                ...
                                exact_direct_regular_flux = (
                                    exact_direct_regular_flux.at[
                                        axis,
                                        side,
                                    ].set(
                                        jnp.where(
                                ...
                                exact_direct_regular_gradient = (
                                    exact_direct_regular_gradient.at[
                                        axis,
                                        side,
                                    ].set(
                                        jnp.mean(
                        ...
                        row_signed_flux = (
                            jnp.where(minus_target, irregular_flux, 0.0)
                            - jnp.where(plus_target, irregular_flux, 0.0)
                        )
                        physical_row = (
                            minus_target
                            & (
                        ...
                        other_irregular_sum = jnp.sum(row_signed_flux) - (
                            physical_sum
                        )
                        regular_sum = owner_regular_sum[
                            local_i,
                            local_j,
                            local_k,
                        ]
                        aggregate_volume = cells.aggregate_volume[
                            local_i,
                            local_j,
                            local_k,
                        ]

                        cubic_gradient, cubic_valid = (
                            reconstruct_local_physical_boundary_gradient(
                                jnp.asarray(
                                    local.x.center,
                                    dtype=jnp.float64,
                                ),
                        ...
                        physical_gradient_rows = (
                            control_volume_geometry
                            .physical_boundary_gradient
                        )
                        values_owned = jnp.asarray(
                            local.x.center,
                            dtype=jnp.float64,
                        )
                        target_value_by_row = values_owned[
                            faces.minus_owner_i,
                            faces.minus_owner_j,
                            faces.minus_owner_k,
                        ]
                        equation_sample_value = values_owned[
                            physical_gradient_rows.sample_i,
                            physical_gradient_rows.sample_j,
                            physical_gradient_rows.sample_k,
                        ]
                        equation_boundary_value = (
                            boundary_bc.quadrature_value[
                                physical_gradient_rows.boundary_face_row,
                                physical_gradient_rows.boundary_patch,
                                physical_gradient_rows
                                .boundary_quadrature,
                            ]
                        )
                        equation_rhs = jnp.where(
                            physical_gradient_rows.equation_kind
                            == CV_RECONSTRUCTION_EQUATION_CELL,
                            equation_sample_value
                            - target_value_by_row[:, None],
                            equation_boundary_value
                            - target_value_by_row[:, None],
                        )
                        equation_rhs = jnp.where(
                            physical_gradient_rows.equation_active,
                            equation_rhs,
                            0.0,
                        )
                        equation_gradient_contribution = (
                            physical_gradient_rows.gradient_transform
                            * equation_rhs[:, None, None, :]
                        )
                        cell_equation_mask = (
                            physical_gradient_rows.equation_active
                            & (
                        ...
                        cubic_cell_contribution = jnp.sum(
                            jnp.where(
                                cell_equation_mask[:, None, None, :],
                                equation_gradient_contribution,
                                0.0,
                            ),
                            axis=-1,
                        )
                        cubic_boundary_contribution = jnp.sum(
                            jnp.where(
                                boundary_equation_mask[:, None, None, :],
                                equation_gradient_contribution,
                                0.0,
                            ),
                            axis=-1,
                        ...
                        boundary_points = faces.quadrature_points[:, 0]
                        boundary_axis_vector = jax.nn.one_hot(
                            faces.boundary_normal_axis,
                            3,
                            dtype=jnp.float64,
                        )
                        (
                            _base_boundary_value,
                            base_boundary_gradient,
                            base_boundary_valid,
                        ) = evaluate_local_control_volume_polynomial(
                            values_owned,
                            phi_poly,
                            cells,
                            faces.minus_owner_i[:, None],
                            faces.minus_owner_j[:, None],
                            faces.minus_owner_k[:, None],
                            boundary_points,
                        )
                        boundary_normal_coordinate = jnp.einsum(
                            "rqi,ri->rq",
                            boundary_points,
                            boundary_axis_vector,
                        )
                        inward_values = []
                        exact_inward_values = []
                        inward_valid = []
                        for sample_index in range(3):
                            ...
                            sample_points = (
                                boundary_points
                                + boundary_axis_vector[:, None, :]
                                * (
                                    sample_coordinate[:, None, None]
                                    - boundary_normal_coordinate[..., None]
                                )
                            )
                            (
                                sample_value,
                                ...
                                phi_poly,
                                cells,
                                faces.boundary_sample_owner_i[
                                    :,
                                    sample_index,
                                    None,
                                ],
                                faces.boundary_sample_owner_j[
                                    :,
                                    sample_index,
                                    None,
                                ],
                                faces.boundary_sample_owner_k[
                                    :,
                                    sample_index,
                                    None,
                                ],
                            ...
                            exact_sample_value, _exact_sample_gradient = (
                                _shifted_torus_exact_field_and_gradient_at_logical_points(
                                    sample_points,
                                    stage_time,
                                    "phi",
                        ...
                        inward_values = jnp.stack(inward_values, axis=-1)
                        exact_inward_values = jnp.stack(
                            exact_inward_values,
                            axis=-1,
                        )
                        inward_valid = jnp.all(
                            jnp.stack(inward_valid, axis=-1),
                            axis=-1,
                        )
                        boundary_weights = (
                            faces.boundary_dcoordinate_weights
                        )
                        wall_value = boundary_bc.quadrature_value[:, 0]
                        exact_wall_value, _exact_wall_gradient = (
                            _shifted_torus_exact_field_and_gradient_at_logical_points(
                                boundary_points,
                                stage_time,
                                "phi",
                        ...
                        normal_stencil_derivative = (
                            boundary_weights[:, 0, None] * wall_value
                            + jnp.sum(
                                boundary_weights[:, None, 1:]
                                * inward_values,
                                axis=-1,
                            )
                        )
                        exact_input_normal_derivative = (
                            boundary_weights[:, 0, None]
                            * exact_wall_value
                            + jnp.sum(
                                boundary_weights[:, None, 1:]
                                * exact_inward_values,
                        ...
                        base_normal_derivative = jnp.einsum(
                            "rqi,ri->rq",
                            base_boundary_gradient,
                            boundary_axis_vector,
                        )
                        normal_stencil_gradient = (
                            base_boundary_gradient
                            + boundary_axis_vector[:, None, :]
                            * (
                                normal_stencil_derivative
                                - base_normal_derivative
                            )[..., None]
                        )
                        _exact_phi, exact_gradient = (
                            _shifted_torus_exact_field_and_gradient_at_logical_points(
                                faces.quadrature_points,
                                stage_time,
                                "phi",
                        ...
                        numerical_quadrature_flux = faces.J[:, 0, :] * (
                            jnp.einsum(
                                "rqi,rqij,rqj->rq",
                        ...
                        exact_quadrature_flux = faces.J[:, 0, :] * (
                            jnp.einsum(
                                "rqi,rqij,rqj->rq",
                                faces.area_covector_weight[:, 0, :, :],
                                faces.projector[:, 0, :, :, :],
                        ...
                        selected_exact_gradient = jnp.sum(
                            jnp.where(
                                selected_q[..., None],
                                exact_gradient[:, 0, :, :],
                                0.0,
                        ...
                        selected_numerical_q_flux = jnp.sum(
                            jnp.where(
                                selected_q,
                                numerical_quadrature_flux,
                                0.0,
                        ...
                        selected_q_active = jnp.sum(
                            selected_q.astype(jnp.int32),
                            axis=0,
                        )
                        selected_condition = jnp.sum(
                            jnp.where(
                                selected,
                                control_volume_geometry
                                .physical_boundary_gradient
                        ...
                        selected_exact_normal_derivative = jnp.einsum(
                            "qi,i->q",
                            selected_exact_gradient,
                            selected_axis_vector,
                        )
                        selected_equation_counts = jnp.stack(
                            (
                                jnp.sum(
                                    jnp.where(
                                        selected[:, None],
                        ...
                        average_reference = phi_owned[
                            local_i,
                            local_j,
                            local_k,
                        ] * 0.0 + exact_state.omega[
                            top_index
                        ]
                        ...
                        scalar_values = jnp.stack(
                            (
                                ...
                                point_reference,
                                average_reference,
                                jnp.sum(selected_numerical_q_flux),
                                jnp.sum(selected_exact_q_flux),
                                selected_condition,
                                jnp.sum(selected.astype(jnp.int32)),
                            ),
                        ).astype(jnp.float64)
                        scalar_values = jnp.where(
                            owns_target,
                            scalar_values,
                            0.0,
                        )
                        ...
                        cubic_detail = {
                            ...
                            "base_gradient": global_sum(
                                jnp.sum(
                                    jnp.where(
                                        selected_normal_q[..., None],
                                        base_boundary_gradient,
                                        0.0,
                                    ),
                                    axis=0,
                            ...
                            "normal_stencil_gradient": global_sum(
                                jnp.sum(
                                    jnp.where(
                                        selected_normal_q[..., None],
                                        normal_stencil_gradient,
                            ...
                            "exact_inward_values": global_sum(
                                jnp.sum(
                                    jnp.where(
                                        selected_normal_q[..., None],
                                        exact_inward_values,
                            ...
                            "normal_stencil_derivative": global_sum(
                                jnp.sum(
                                    jnp.where(
                                        selected_normal_q,
                                        normal_stencil_derivative,
                            ...
                            "exact_input_normal_derivative": global_sum(
                                jnp.sum(
                                    jnp.where(
                                        selected_normal_q,
                                        exact_input_normal_derivative,
                            ...
                            "exact_normal_derivative": global_sum(
                                selected_exact_normal_derivative
                            ),
                            "weights": global_sum(
                                jnp.sum(
                                    jnp.where(
                            ...
                            "cell_contribution": global_sum(
                                jnp.sum(
                                    jnp.where(
                                        selected_q[..., None],
                                        cubic_cell_contribution,
                                        0.0,
                                    ),
                                    axis=0,
                                )
                            ),
                            "boundary_contribution": global_sum(
                                jnp.sum(
                                    jnp.where(
                                        selected_q[..., None],
                                        cubic_boundary_contribution,
                                        0.0,
                                    ),
                                    axis=0,
                                )
                            ),
                            "equation_counts": global_sum(
                                selected_equation_counts
                            ),
                        }

                        return (
                            global_sum(scalar_values),
                            global_sum(selected_gradient),
                            global_sum(selected_exact_gradient),
                            global_sum(selected_numerical_q_flux),
                            global_sum(selected_exact_q_flux),
                            global_sum(selected_q_active),
                            global_sum(
                                jnp.where(
                                    owns_target,
                                    direct_regular_flux,
                            ...
                            global_sum(
                                jnp.where(
                                    owns_target,
                                    direct_open_mask.astype(jnp.int32),
                                    0,
                                )
                            ),
                            cubic_detail,
                    ...
                    compiled_flux_diagnostic = jax.jit(
                        shard_map(
                            perpendicular_flux_diagnostic_kernel,
                            mesh=mesh,
                            in_specs=(
                                field_spec,
                                invariant_spec,
                            ...
                            out_specs=(
                                ...
                                P(),
                                {
                                    ...
                                    "exact_wall_value": P(),
                                    "base_gradient": P(),
                                    "normal_stencil_gradient": P(),
                                    "inward_values": P(),
                                    "exact_inward_values": P(),
                                    "normal_stencil_derivative": P(),
                                    "exact_input_normal_derivative": P(),
                                    "exact_normal_derivative": P(),
                                    "weights": P(),
                                    "sample_coordinates": P(),
                                    "sample_owners": P(),
                                    "cell_contribution": P(),
                                    "boundary_contribution": P(),
                                    "equation_counts": P(),
                                },
                            ),
                            check_rep=False,
                    ...
                    numerical_q_flux = np.asarray(
                        numerical_q_flux,
                        dtype=np.float64,
                    )
                    exact_q_flux = np.asarray(
                        exact_q_flux,
                        dtype=np.float64,
                    )
                    q_active = np.asarray(q_active, dtype=np.int32)
                    direct_regular_flux = np.asarray(
                        direct_regular_flux,
                        dtype=np.float64,
                    )
                    exact_direct_regular_flux = np.asarray(
                        exact_direct_regular_flux,
                        dtype=np.float64,
                    )
                    direct_regular_gradient = np.asarray(
                        direct_regular_gradient,
                        dtype=np.float64,
                    )
                    exact_direct_regular_gradient = np.asarray(
                        exact_direct_regular_gradient,
                        dtype=np.float64,
                    )
                    direct_open_measure = np.asarray(
                        direct_open_measure,
                        dtype=np.float64,
                    )
                    direct_open_mask = np.asarray(
                        direct_open_mask,
                        dtype=np.int32,
                    )
                    cubic_detail = {
                        name: np.asarray(value)
                        for name, value in cubic_detail.items()
                    }
                    (
                        regular_sum,
                        physical_sum,
                    ...
                    print(
                        "  perpendicular_flux_decomposition "
                        f"target={top_index} "
                        f"physical_rows={int(round(physical_row_count))} "
                        f"cubic_condition={cubic_condition:.6e}"
                    )
                    print(
                        "    cubic_boundary_inputs axis={} target_value={:.8e} "
                        "equations(cell/wall)={}/{}".format(
                            int(cubic_detail["axis"]),
                            float(cubic_detail["target_value"]),
                            int(cubic_detail["equation_counts"][0]),
                            int(cubic_detail["equation_counts"][1]),
                        )
                    )
                    print(
                        "      normal_stencil owners={} coordinates={} "
                        "weights={}".format(
                            np.array2string(
                                cubic_detail["sample_owners"],
                                separator=",",
                            ),
                            np.array2string(
                                cubic_detail["sample_coordinates"],
                                precision=8,
                                separator=",",
                            ),
                            np.array2string(
                                cubic_detail["weights"],
                                precision=8,
                                separator=",",
                            ),
                        )
                    ...
                    for quadrature in range(4):
                        print(
                            "    q{} active={} grad={} exact_grad={} "
                            "flux={:.6e} exact_flux={:.6e} "
                            "flux_error={:.6e}".format(
                                quadrature,
                                int(q_active[quadrature]),
                                np.array2string(
                                    numerical_gradient[quadrature],
                                    precision=8,
                                    separator=",",
                                ),
                                np.array2string(
                                    exact_gradient[quadrature],
                                    precision=8,
                                    separator=",",
                                ),
                                numerical_q_flux[quadrature],
                                exact_q_flux[quadrature],
                                numerical_q_flux[quadrature]
                                - exact_q_flux[quadrature],
                            )
                        )
                        print(
                            "      point={} wall={:.8e} exact_wall={:.8e} "
                            "inward={} exact_inward={}".format(
                                np.array2string(
                                    cubic_detail["points"][quadrature],
                                    precision=8,
                                    separator=",",
                                ),
                                cubic_detail["wall_value"][quadrature],
                                cubic_detail["exact_wall_value"][quadrature],
                                np.array2string(
                                    cubic_detail["inward_values"][
                                        quadrature
                                    ],
                                    precision=8,
                                    separator=",",
                                ...
                                np.array2string(
                                    cubic_detail["exact_inward_values"][
                                        quadrature
                                    ],
                                    precision=8,
                                    separator=",",
                        ...
                        print(
                            "      base_grad={} normal_stencil_grad={} "
                            "normal_derivative={:.8e} "
                            "exact_input_derivative={:.8e} "
                            "analytic_derivative={:.8e}".format(
                                np.array2string(
                                    cubic_detail["base_gradient"][
                                        quadrature
                                    ],
                                    precision=8,
                                    separator=",",
                                ...
                                cubic_detail[
                                    "exact_normal_derivative"
                                ][quadrature],
                            )
                        )
                        print(
                            "      full_cubic_contributions cell={} wall={} "
                            "sum={}".format(
                                np.array2string(
                                    cubic_detail["cell_contribution"][
                                        quadrature
                                    ],
                                    precision=8,
                                    separator=",",
                    ...
                    print(
                        "    integrated_flux regular={:.6e} "
                        "physical={:.6e} other_irregular={:.6e} "
                        "total={:.6e} volume={:.6e}".format(
                            regular_sum,
                            physical_sum,
                            other_irregular_sum,
                            integrated_sum,
                    ...
                    direct_regular_sum = float(
                        np.sum(direct_regular_flux)
                    )
                    mapped_regular_remainder = (
                        regular_sum - direct_regular_sum
                    )
                    axis_names = ("x", "theta", "zeta")
                    side_names = ("low", "high")
                        ...
                        for side, side_name in enumerate(side_names):
                            print(
                                ...
                                "flux_error={:.6e}".format(
                                    ...
                                    direct_regular_flux[axis, side],
                                    exact_direct_regular_flux[axis, side],
                                    direct_regular_flux[axis, side]
                                    - exact_direct_regular_flux[axis, side],
                                )
                            )
                    print(
                        "    regular_face_sum direct={:.6e} "
                        "mapped_source_remainder={:.6e} "
                        "exact_direct={:.6e} direct_error={:.6e}".format(
                            direct_regular_sum,
                            mapped_regular_remainder,
                            float(np.sum(exact_direct_regular_flux)),
                            direct_regular_sum
                            - float(
                                np.sum(exact_direct_regular_flux)
                            ),
                        )
                    )
                    print(
                        "    physical_q_sum={:.6e} "
                        "exact_physical_q_sum={:.6e} error={:.6e}".format(
                            physical_q_sum,
                            exact_physical_q_sum,
                            physical_q_sum - exact_physical_q_sum,
                        )
                    )
                    print(
                        "    divergence_from_flux={:.6e} "
                        "reported_actual={:.6e} "
                        "point_reference={:.6e} "
                        "volume_average_reference={:.6e}".format(
                            integrated_sum
                            / max(aggregate_volume, 1.0e-30),
                            float(np.asarray(actual)[top_index]),
                            point_reference,
                            average_reference,
                        )
...
def run_shifted_torus_4field_cutwall_convergence(
    *,
    ...
    per_field_orders: dict[str, tuple[float, float]] = {}
    if len(successful_resolutions) >= 2:
        ...
        for field_name in (
            ...
            field_l2_slope = np.polyfit(
                np.log(plotted_resolutions),
                np.log(field_l2),
                1,
            )[0]
            field_linf_slope = np.polyfit(
                np.log(plotted_resolutions),
                np.log(field_linf),
                1,
            )[0]
            per_field_orders[field_name] = (
                float(-field_l2_slope),
                float(-field_linf_slope),
            )
            print(
                "shifted_torus_4field_cutwall "
                f"{field_name} orders: volume_L2="
                f"{-field_l2_slope:.6f}, active_owner_Linf="
                f"{-field_linf_slope:.6f}"
            )
        if minimum_order is not None:
            failed_orders = [
                (
                    f"{field_name}: volume_L2={orders[0]:.6f}, "
                    f"active_owner_Linf={orders[1]:.6f}"
                )
                for field_name, orders in per_field_orders.items()
                if (
                    not np.isfinite(orders[0])
