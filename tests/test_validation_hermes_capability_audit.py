...
def test_build_hermes_capability_audit_reports_major_open_families() -> None:
    ...
    assert families["neutral_mixed"]["jax_capability"] == "native_rhs_exact_bounded_full_short_window_field_window"
    assert families["direct_tokamak_recycling"]["jax_status"] == "partially_closed"
    assert families["direct_tokamak_recycling"]["jax_capability"] == "native_exact_first_output_plus_bounded_two_output_and_neon_short_windows"
    assert families["impurity_radiation_and_detachment_control"]["jax_status"] == "partially_closed"
    assert families["impurity_radiation_and_detachment_control"]["jax_capability"] == "dedicated_campaigns_plus_reduced_temperature_and_detachment_controller_lanes"
    assert families["non_tokamak_3d_geometry_adapters"]["jax_status"] == "closed_selected"
    assert "neutral_mixed" not in audit["remaining_priority_families"]
