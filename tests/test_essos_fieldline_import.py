...
from drbx.validation.essos_vmec_closed_field_campaign import (
    build_essos_vmec_closed_field_report,
)
from drbx.validation.essos_vmec_closed_field_transient_campaign import (
    build_essos_vmec_closed_field_transient_campaign,
)
import drbx.validation.essos_imported_fci_campaign as imported_fci_campaign
import drbx.validation.essos_imported_drb_movie_campaign as imported_movie_campaign
...
def test_essos_imported_fci_maps_feed_native_sheath_and_neutral_gates(tmp_path: Path) -> None:
    ...
    report = json.loads(artifacts.report_json_path.read_text(encoding="utf-8"))
    assert report["passed"] is True
    assert report["source"] == "ESSOS-imported field-line maps with drbx FCI closures"
...
def test_essos_imported_maps_feed_pytree_jvp_rhs_gate(tmp_path: Path) -> None:
    ...
    report = json.loads(artifacts.report_json_path.read_text(encoding="utf-8"))
    assert report["passed"] is True
    assert report["source"] == "ESSOS-imported field-line maps with DRBX fixed-layout PyTree RHS"
...
def test_essos_imported_maps_generate_drb_movie_gate(tmp_path: Path) -> None:
    ...
    report = json.loads(artifacts.report_json_path.read_text(encoding="utf-8"))
    assert report["passed"] is True
    assert report["source"] == "ESSOS-imported Landreman-Paul QA coil FCI maps with DRBX fixed-layout DRB transient"
    assert report["publication_ready"] is False
