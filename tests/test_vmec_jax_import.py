...
def _find_local_wout() -> Path | None:
    ...
    essos_root = Path(os.environ.get("JAX_DRB_ESSOS_ROOT", Path.home() / "local" / "ESSOS_test")).expanduser()
    vmec_jax_root = Path(os.environ.get("JAX_DRB_VMEC_JAX_ROOT", Path.home() / "local" / "vmec_jax")).expanduser()
    candidates = (
