...
def _no_guard_mesh() -> StructuredMesh:
    return StructuredMesh(
        nx=1,
        ny=2,
        nz=1,
...
def test_apply_neutral_target_density_guards_respects_disabled_targets() -> None:
    mesh = _guard_mesh()
    field = np.arange(6, dtype=np.float64).reshape(1, 6, 1)
    ...
    np.testing.assert_allclose(guarded, field)


def test_apply_neutral_target_density_guards_noops_without_y_guards() -> None:
    mesh = _no_guard_mesh()
    field = np.arange(2, dtype=np.float64).reshape(1, 2, 1)
...
def test_apply_open_field_dirichlet_scalar_guards_respects_disabled_targets() -> None:
    mesh = _guard_mesh()
    field = np.arange(6, dtype=np.float64).reshape(1, 6, 1)
...
def test_apply_open_field_dirichlet_scalar_guards_noops_without_y_guards() -> None:
    mesh = _no_guard_mesh()
    field = np.arange(2, dtype=np.float64).reshape(1, 2, 1)
