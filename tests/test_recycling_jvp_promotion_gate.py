...
def test_recycling_jvp_promotion_gate_builds_preconditioned_fixed_bdf2_command() -> None:
    ...
    assert "runtime:recycling_jax_linear_operator_counting=direct" in overrides
    assert (
    ...
    assert command[
        command.index("--require-fixed-bdf2-max-linear-update-relative-residual") + 1
    ] == "0.0001"
    assert command[
...
def test_recycling_jvp_promotion_gate_rejects_invalid_performance_budgets() -> None:
    ...
    for kwargs, expected in (
        ...
        (
            ...
            "fixed_bdf2_max_linear_update_residual must be finite and nonnegative",
        ),
        (
            {"fixed_bdf2_max_linear_update_relative_residual": float("nan")},
            "fixed_bdf2_max_linear_update_relative_residual must be finite and nonnegative",
        ),
        (
        ...
        (
            ...
            "do not combine it with fixed_bdf2_max_linear_operator_calls",
        ),
    ):
        try:
            module._build_case_command(
