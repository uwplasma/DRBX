...
def test_compare_native_stellarator_vmec_selected_fields_zero_for_identical_inputs(tmp_path: Path) -> None:
    ...
    result, _, _, source = compare_native_stellarator_vmec_selected_fields(
...
def test_create_native_stellarator_vmec_selected_field_package_writes_artifacts(tmp_path: Path) -> None:
...
def test_create_native_stellarator_vmec_selected_field_package_supports_explicit_external_pair(tmp_path: Path) -> None:
    reference = tmp_path / "reference.nc"
    candidate = tmp_path / "candidate.nc"
    _write_vmec_case(reference, scale=1.0)
    _write_vmec_case(candidate, scale=1.15)
    artifacts = create_native_stellarator_vmec_selected_field_package(
        reference_equilibrium_path=reference,
        candidate_equilibrium_path=candidate,
        output_root=tmp_path / "output",
    ...
    assert observable["metadata"]["source_mode"] == "explicit_pair"
