...
def test_recycling_backend_environment_resolvers_are_bounded(
    ...
    monkeypatch.delenv(
        "JAX_DRB_RECYCLING_JAX_LINEAR_JIT_LINEAR_OPERATOR", raising=False
    )
    monkeypatch.delenv(
    ...
    assert (
        recycling_1d_mod._resolve_recycling_jax_linear_jit_linear_operator()
        is False
    )
    assert (
    ...
    assert (
        recycling_1d_mod._resolve_recycling_jax_linear_initial_residual_mode()
        == "residual"
    )
    assert (
    ...
    monkeypatch.setenv("JAX_DRB_RECYCLING_JAX_LINEAR_LINE_SEARCH_MODE", "bad")
    assert (
        recycling_1d_mod._resolve_recycling_jax_linear_line_search_mode()
        == "backtracking"
    )
    ...
    assert recycling_1d_mod._resolve_recycling_jax_linear_jit_residual() is False
    monkeypatch.setenv("JAX_DRB_RECYCLING_JAX_LINEAR_JIT_LINEAR_OPERATOR", "yes")
    assert (
        recycling_1d_mod._resolve_recycling_jax_linear_jit_linear_operator()
        is True
    )
    monkeypatch.setenv("JAX_DRB_RECYCLING_JAX_LINEAR_JIT_LINEAR_OPERATOR", "off")
    assert (
        recycling_1d_mod._resolve_recycling_jax_linear_jit_linear_operator()
        is False
    )
    ...
    monkeypatch.setenv(
        "JAX_DRB_RECYCLING_JAX_LINEAR_CHECK_INITIAL_RESIDUAL", "yes"
    )
    assert (
        recycling_1d_mod._resolve_recycling_jax_linear_check_initial_residual()
        is True
    ...
    monkeypatch.setenv(
        "JAX_DRB_RECYCLING_JAX_LINEAR_INITIAL_RESIDUAL_MODE", "linearized"
    )
    assert (
        recycling_1d_mod._resolve_recycling_jax_linear_initial_residual_mode()
        == "linearize"
    ...
    monkeypatch.setenv(
        "JAX_DRB_RECYCLING_JAX_LINEAR_INITIAL_RESIDUAL_MODE", "bad"
    )
    ...
    monkeypatch.setenv("JAX_DRB_RECYCLING_JAX_LINEAR_PRECONDITIONER", "neutral-diffusion")
    assert (
        recycling_1d_mod._resolve_recycling_jax_linear_preconditioner_name()
        == "neutral_line"
    )
    monkeypatch.setenv("JAX_DRB_RECYCLING_JAX_LINEAR_PRECONDITIONER", "parallel-momentum")
    assert (
        recycling_1d_mod._resolve_recycling_jax_linear_preconditioner_name()
        == "momentum_line"
    )
...
def test_recycling_jax_linear_solver_controls_prefer_runtime_config(
    ...
    assert (
        recycling_1d_mod._resolve_recycling_jax_linear_jit_linear_operator(config)
        is True
    )
    assert (
    ...
    assert (
        recycling_1d_mod._resolve_recycling_jax_linear_initial_residual_mode(config)
        == "linearize"
    )
    assert (
...
def test_recycling_state_scale_preconditioner_scales_packed_rows() -> None:
    preconditioner = recycling_1d_mod._build_recycling_jax_linear_preconditioner(
        np.asarray([2.0, -0.5, 4.0], dtype=np.float64),
        name="state_scale",
    )

    assert preconditioner is not None
    np.testing.assert_allclose(
        np.asarray(preconditioner(np.asarray([2.0, 2.0, 8.0], dtype=np.float64))),
...
def test_recycling_local_block_preconditioner_context_uses_packed_layout() -> None:
    layout = SimpleNamespace(
        active_shape=(3,),
        field_names=("Nd+", "Pe", "NVd+", "Nd", "Pd", "NVd", "Ne"),
    ...
    neutral_context = recycling_1d_mod._recycling_jax_linear_preconditioner_context(
        "neutral_line",
        layout=layout,
    )
    ...
    momentum_context = recycling_1d_mod._recycling_jax_linear_preconditioner_context(
        "momentum_line",
        layout=layout,
    )
    ...
    assert configured_linearized_context["refresh_frequency"] == 4
    assert configured_linearized_context["floor"] == 1.0e-8
    assert configured_linearized_context["max_unknowns"] == 192
    ...
    assert configured_momentum_context["refresh_frequency"] == 4
    assert configured_momentum_context["floor"] == 1.0e-8
    assert configured_momentum_context["max_line_unknowns"] == 64
...
def test_recycling_runtime_option_resolvers_reject_invalid_preconditioner_and_config(
    ...
    assert recycling_1d_mod._resolve_recycling_jax_linear_jit_residual(config) is False
    with pytest.raises(ValueError, match="Unsupported recycling JAX preconditioner"):
        recycling_1d_mod._build_recycling_jax_linear_preconditioner(
            np.asarray([1.0], dtype=np.float64),
            name="unsupported",
        )
...
def test_recycling_backward_euler_routes_jax_native_solver_backends(
    ...
    def fake_jax_linearized(
        ...
        if linear_solver_backend == "jax_gmres":
            ...
            assert kwargs["linear_preconditioner_name"] is None
            assert kwargs["linear_preconditioner"] is None
        return np.asarray(initial_state, dtype=np.float64), ImplicitStepInfo(
            residual_inf_norm=0.0,
            active_shape=tuple(active_shape),
            nonlinear_iterations=1,
