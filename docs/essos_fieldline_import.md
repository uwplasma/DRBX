...
section extraction are performed by ESSOS. `dkx` stores the resulting
...
the default file from `DKX_ESSOS_ROOT`. The committed report stores only
...
prediction. It proves that `dkx` can consume field-line data generated by
...
Poincare root extraction. `dkx` owns import, QA plotting, and later
...
imports the resulting field-line bundle into `dkx`. This replaces the
previous internal annular coil-field proxy: no `dkx` source file now
...
and routes DKX sheath/recycling and neutral closures through the imported
