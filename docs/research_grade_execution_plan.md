...
Audit date: 2026-06-22. This is the current single execution objective for the
...
1. Keep the `main` branch centered on ESSOS direct-coil imports first:
   `examples/geometry-3D/essos-field-lines/direct_coil_open_sol_demo.py`
   should remain the canonical open-field workflow and
   `direct_coil_closed_field_demo.py` should remain the matching
   closed/near-closed control. Finish endpoint-label refinement, source
   accounting, direct-coil profile/source plots, and a diagnostic turbulence
   movie before promoting any direct-coil media in the README.
2. Finish the VMEC closed-field control path next. Complete and integrate the
   reduced transient/profile/spectrum/movie campaign around
   `essos_vmec_closed_field_campaign.py` and
   `essos_vmec_closed_field_transient_campaign.py`. Closed VMEC examples must
   keep endpoint masks at zero and must explicitly disable target, sheath,
   recycling, and neutral-loss semantics.
3. Promote the hybrid VMEC/coil open-SOL lane only after the direct-coil and
   closed-VMEC semantics are unambiguous. Hybrid examples should use VMEC
   smooth coordinates and coil-derived endpoint masks, then run longer/refined
   reduced transients with source/target/profile plots, grid and timestep
   checks, and frame-by-frame movie QA.
4. Treat VMEC-extender finite-beta SOL as an artifact-import lane, not a live
   runtime dependency. The JAXDRB VMEC-extender PR is already merged; upstream
   exporter PRs are useful, but `main` should consume frozen
   provenance-rich `(R, physical phi, Z, BR, Bphi, BZ, |B|)` artifacts until
   those exporters stabilize.

## Final Execution Roadmap To Follow Next

This is the compact roadmap to execute before reopening broad implementation
questions. It is deliberately narrower than the long appendices below: every
item either promotes a user-facing result, removes a false claim boundary, or
keeps the release lightweight and reproducible.

Current upstream decision, verified with `gh` on 2026-06-22:

- `uwplasma/jax_drb#2` is merged, so JAXDRB already has the VMEC-extender
  artifact importer, physical-phi interpolation checks, field-line RHS helper,
  synthetic compact SOL smoke gate, and docs on `main`.
- `uwplasma/virtual_casing_jax#2` is open with clean merge state, while
  `uwplasma/ESSOS#31` and `uwplasma/ESSOS#33` are open with blocked merge
  state. They are finite-beta exterior-field artifact/export enablers, not
  ordinary user dependencies.
- `uwplasma/vmec_jax#18` and `uwplasma/vmec_jax#19` are merged and provide
  useful finite-pressure/free-boundary context. Current open VMEC-JAX PRs are
  ecosystem context, not direct JAXDRB SOL-runtime dependencies.
- Therefore, `main` should ship direct ESSOS-coil imported-field examples
  first, hybrid VMEC/coil examples second, and finite-beta VMEC-extender SOL
  only through frozen provenance-rich artifacts until the real exterior-field
  artifact gates pass.

Roadmap:

| Priority | Lane | Specific next changes | Definition of success |
| --- | --- | --- | --- |
| P0 | Plan and artifact control | Keep this file as the only authoritative plan; keep `plan_jax_drb.md` as a redirect; keep heavy NetCDF, trace, movie, profiler, and reference files out of git and in release assets or ignored local paths. | A new contributor can start from this file, no subordinate page contradicts it, and a clone stays small. |
| P1 | Direct ESSOS-coil open-field SOL on `main` | Finish the pure-coil open workflow around `direct_coil_open_sol_demo.py`: target-exit nested diagnostics, adjacent-step nested diagnostics, source/profile and neutral-source diagnostics, target heat/particle flux plots, and one diagnostic reduced turbulence movie generated from the exact endpoint masks consumed by sheath/recycling/neutrals. Keep the signed-shell endpoint result as diagnostic evidence rather than hiding it with broad projection. | The report either sets `promotion_ready=true` or names the blocker. Source accounting closes on the consumed masks, target-exit lengths are finite only on endpoint cells, endpoint mismatch is classified as shell-localized or bulk, and the movie is not promoted unless geometry/operator/source/refinement/movie-QA gates all pass. |
| P2 | Direct ESSOS-coil closed and near-closed controls | Package the pure-coil closed-control workflow with release-hosted media, README/docs commands, return-map/provenance synchronization, and an open-versus-closed comparison figure. | Users can regenerate closed/near-closed Poincare, return-map, profile, spectrum, and movie diagnostics with explicit absence of target, sheath, recycling, neutral-loss, and target-to-target connection-length semantics. |
| P3 | VMEC closed-field stellarator control | Keep VMEC-coordinate maps as the smooth closed-field control. Add stronger periodic bracket/vorticity forcing and profile/spectrum polish only when equations, code links, and tests are updated together. | Closed VMEC examples pass zero-endpoint, closed-map operator, mass/profile/spectrum, fixed-camera, and fixed-color QA gates and never imply open-SOL target physics. |
| P4 | Hybrid VMEC/coil open-SOL promotion | Run the heavier hybrid promotion preset: live FCI/source-profile regeneration, stationarity, grid/time refinement, exact endpoint-mask reconstruction, target/source/profile plots, combined `release_evidence` audit, release-hosted media, and frame-by-frame movie QA. | Hybrid becomes the first promoted stellarator open-SOL movie only if all gates are green on the same consumed map. README labels it as the promoted open-SOL bridge if pure direct-coil endpoint maps remain rough. |
| P5 | VMEC-extender finite-beta artifact intake | Add a real/frozen artifact manifest and validation checklist for `(R, physical phi, Z, BR, Bphi, BZ, |B|)`, branch/sign metadata, field-line/Poincare samples, wall/endpoint labels, connection lengths, FCI/operator gates, compact SOL closures, and visual QA. | A finite-beta SOL result is promoted only after a frozen exterior-field artifact passes field, convention, wall/endpoint, connection-length, FCI/operator, sheath/recycling/neutral, refinement, and movie-QA gates. The synthetic importer remains a contract test, not a physics claim. |
| P6 | Device matrix | Add devices one at a time: Landreman-Paul QA first, HSX QHS vacuum second, NCSX third, Dommaschk potentials fourth. Each device must reuse the same source-metadata, geometry, open/closed classification, operator, physics, refinement, media, and docs gates. | Each promoted device has provenance, boundary/Poincare plots, connection-length or closed-step maps, endpoint labels where open, one validated physics result, release-hosted media if large, and a documented command. |
| P7 | Full DRB physics on promoted geometries | Carry the potential-fed \(E\times B\) bracket, vorticity/potential solve, Boussinesq and non-Boussinesq comparisons, sheath, recycling, neutrals, detachment, and selected electromagnetic terms onto the promoted geometry examples. Remove surrogate nonlinear-transfer terms from promoted examples or label them pedagogical. | Every promoted term has equations, implementation links, tests, plots, and parity or literature anchors. Coverage remains above `95%` on meaningful promoted surfaces. |
| P8 | Reference parity, recycling solver, preconditioning, and scaling | Keep reference parity and stable compatibility BDF as the default unless full-output JAX/JVP recycling parity and same-fidelity runtime evidence are green. Try only a bounded number of materially different physics/block preconditioners, record negative evidence, and prioritize parity/geometry once they fail. Tie CPU/GPU claims to promoted heavy kernels only. | Solver, preconditioner, differentiability, CPU, GPU, and multi-device claims include parity, solver-health, compile/execute timing, memory, profiler evidence, and exact experimental/default labels. |
| P9 | Tokamak tutorials, docs, examples, release gates | Keep diverted tokamak tutorials self-contained with small fixtures or release assets. Every README/docs plot or movie must have a command. Before tagging, run docs strict build, promoted coverage above `95%`, release-surface tests, media restore, package audit, and footprint audit. | A clean clone can install JAXDRB, run advertised tokamak and stellarator examples, regenerate or restore advertised media, and understand which outputs are native simulations versus validation fixtures. |

Near-term finalization decision for this pass:

- Finalize now: vacuum direct ESSOS-coil open diagnostics, direct ESSOS-coil
  closed/near-closed controls, VMEC closed-field controls, and the compact
  release-backed hybrid VMEC/coil open-SOL bridge.
- Defer: finite-beta VMEC-extender exterior-field artifacts, full DRB physics
  on every imported geometry, Boussinesq/non-Boussinesq expansion on every
  device, full-output JAX-native recycling as the default, new broad
  preconditioner sweeps, multi-GPU promotion, HSX/NCSX/Dommaschk device
  expansion, and long-window predictive turbulence.
- Closeout rule: the near-term lanes may be called finalized only within their
  current vacuum/compact/diagnostic claim boundaries. A diagnostic pure-coil
  open-field movie is not a promoted open-SOL turbulence result. A closed VMEC
  or closed direct-coil movie is not a target/sheath/recycling result. The
  hybrid movie is the compact release-backed bridge, not a finite-beta or
  device-scale prediction.

Open/closed stellarator semantics:

- Open-field examples use endpoint masks, one-sided or target-to-target
  connection lengths, sheath/recycling/neutral closures, source balances, and
  target heat/particle fluxes.
- Closed-field examples use periodic or return-map parallel-step diagnostics,
  conservative closed-surface operators, profiles, spectra, and turbulence
  movies with no target/sheath/recycling language unless an artificial loss
  model is explicitly selected.
- Hybrid examples are open-field examples because they consume coil-derived
  endpoint masks, even when VMEC supplies the smooth coordinate map.
- VMEC-extender finite-beta examples are artifact-import examples until a real
  exterior-field artifact passes the full open-field gate stack.

Immediate next implementation pass:

1. Lock the workflow summaries so the near-term closeout state is
   machine-readable: `finalized_diagnostic_contract` for default direct-coil
   open diagnostics and `release_backed_compact_vacuum_bridge_ready` for the
   default hybrid release-evidence ledger.
2. Keep direct-coil closed/near-closed and VMEC closed-field examples as the
   finalized closed-control paths. Their reports must continue to forbid
   target, sheath, recycling, and neutral-loss semantics.
3. Keep pure direct-coil open media diagnostic. The next live work is only
   target-exit/adjacent-step/source/profile/target-flux/movie QA on the same
   consumed endpoint masks, not broad endpoint projection or full-physics
   expansion.
4. Treat hybrid as the compact release-backed open-SOL bridge. Live
   regeneration is optional for developers; default clean-clone evidence comes
   from the audited release-backed JSON/media manifest.
5. Run focused release-surface tests and strict docs build, then commit this
   near-term closeout boundary.
...
Audit date: 2026-06-22. Percentages are approximate and evidence-based. A lane
...
| 3D stellarator open/closed SOL program | 84% overall for near-term vacuum scope | Near-term vacuum closeout is now explicit. Direct ESSOS-coil open-field is finalized as a diagnostic contract/live-validation ledger, not as promoted pure-coil turbulence media; the signed-shell endpoint gate shows the current pure-coil endpoint mismatch is shell-localized rather than bulk-map dominated. Direct ESSOS-coil closed/near-closed controls are finalized as self-contained closed-control examples with return-map/refinement/transient/movie evidence and no target, sheath, recycling, or neutral-loss semantics. VMEC closed-field controls are finalized as self-contained contracts with optional live VMEC regeneration and zero endpoint masks. The compact hybrid VMEC/coil open-SOL bridge is finalized within the release-backed vacuum evidence scope through the committed FCI/source, stationarity, grid/time, media-manifest, and visual-QA audit. Finite-beta VMEC-extender exterior fields, full DRB physics on every geometry, HSX/NCSX/Dommaschk expansion, and long-window predictive turbulence are deferred. |
...
Audit date: 2026-06-22. This board is the only active execution plan. The
older milestone details below are retained as implementation notes, not as
separate roadmaps. Future work should update this board first, then update a
subordinate page only when more derivation, command output, or figure detail is
needed.

Single-source rule for the remaining work:

- The lane board below is the only source of active priorities, completion
  percentages, definitions of success, and stop conditions.
- Older "next step" text in subordinate pages is evidence or background only.
  If a subordinate page disagrees with this lane board, this lane board wins.
- Every implementation pass should update the lane board and execution log
  before broadening a README, docs, paper, or release claim.
- The main branch should stay usable by ordinary users without checking out
  upstream VMEC-extender, ESSOS, VMEC, BSTING, Zoidberg, or reference-code
  branches. Developer-only upstream runs may generate frozen artifacts, but
  user-facing examples must consume committed small fixtures or release-hosted
  assets.

Current VMEC-extender decision, rechecked with `gh` on 2026-06-22:

- `uwplasma/jax_drb#2` is merged, so `main` already has the
  VMEC-extender NetCDF import contract, physical-phi interpolation checks,
  field-line RHS helpers, and synthetic compact SOL smoke gate.
- `uwplasma/virtual_casing_jax#2`, `uwplasma/ESSOS#31`, and
  `uwplasma/ESSOS#33` remain upstream artifact/export enablers. The current
  `gh` check reports virtual-casing `#2` open with a clean merge state, and
  ESSOS `#31` and `#33` open with blocked merge state. They are useful for
  finite-beta exterior-field artifacts but are not prerequisites for
  main-branch direct-coil or hybrid examples.
- `uwplasma/vmec_jax#18` and `uwplasma/vmec_jax#19` are merged and are useful
  finite-pressure/free-boundary context. Current open `vmec_jax` PRs are
  ecosystem context, not direct JAXDRB SOL dependencies.
- The current ESSOS refactor/examples PRs are useful developer context, but
  they are not allowed to become runtime requirements for normal JAXDRB
  examples. JAXDRB should import compact field-line, Poincare, connection, and
  wall/endpoint artifacts generated elsewhere, then run its own SOL operators,
  closure gates, diagnostics, movies, and docs.
- Therefore, `main` proceeds now with direct ESSOS-coil field imports and
  hybrid VMEC/coil open-SOL workflows. VMEC-extender finite-beta SOL stays an
...
No other active roadmap files were found in the June 22, 2026 Markdown audit.
All remaining work is represented by the lane board below. Historical campaign
pages may still contain old "next step" prose, but those notes are subordinate
evidence unless they are copied into this board.
...
Current answer on ESSOS-coil stellarator open and closed field lines:

- Yes, both open and closed/near-closed ESSOS-coil simulations belong on
  `main`, but they have different mathematical meanings and therefore different
  gates. The open-field lane uses endpoint masks, target-exit lengths,
  sheath/recycling/neutrals, target heat/particle fluxes, and source balances.
  The closed/near-closed control lane uses Poincare/return maps, closed-step or
  return-distance diagnostics, conservative closed-surface transport, and no
  target/sheath/recycling semantics unless an artificial loss model is
  explicitly selected and labeled.
- The direct-coil open lane is the physical first step because it uses ESSOS
  field evaluation and field-line tracing directly from the coil geometry.
  It remains diagnostic until endpoint labels, target distances, adjacent-step
  refinement, FCI operators, source accounting, and movie QA pass on the same
  map.
- The direct-coil closed/near-closed lane is already useful as a control and
  geometry QA path. It should be shown as closed-field or near-closed-field
  dynamics, not as an open-SOL target simulation.
- The hybrid lane is the planned first promoted open-SOL movie path: VMEC
  coordinates provide a smoother non-axisymmetric map and ESSOS coil traces
  provide endpoint masks and field modulation. It should be promoted before a
  pure direct-coil open movie if the direct-coil endpoint refinement remains
  rough.

The final deliverable should contain two visibly distinct stellarator examples:

- Direct ESSOS-coil open-field diagnostic: a pure-coil Landreman-Paul QA
  workflow with Poincare/field-line context, endpoint masks, target-exit and
  adjacent-step diagnostics, sheath/recycling/neutral source accounting,
  target heat and particle flux maps, profiles, and a reduced transient movie
  labeled diagnostic until the direct-coil map-quality gates are green.
- Direct ESSOS-coil closed/near-closed control: a pure-coil control workflow
  with closed or long-lived field-line classification, return-map evidence,
  profile/spectrum/movie diagnostics, and explicit absence of target,
  sheath, recycling, and neutral-loss semantics.

The first promoted open-SOL stellarator movie is expected to be the hybrid
VMEC/coil lane unless pure-coil endpoint refinement becomes clean first.
VMEC-extender finite-beta simulations are a separate artifact-intake milestone,
not a blocker for the direct-coil or hybrid examples on `main`.

Final closeout sequence:

1. Keep `main` self-contained and finish direct ESSOS-coil open diagnostics.
   The next code changes are endpoint projection/target-boundary
   classification, adjacent-step and target-exit nested diagnostics, source
   and target flux plots, and a diagnostic reduced movie generated from the
   exact endpoint masks consumed by the closure gates.
2. Package direct ESSOS-coil closed/near-closed controls. The next changes are
   release-hosted media manifests, README/docs commands, and a compact
   open-versus-closed comparison figure that explicitly disables
   target/sheath/recycling language for closed maps.
3. Use VMEC closed-field simulations as the smooth non-axisymmetric control.
   The next changes are optional stronger bracket/vorticity forcing and
   profile/spectrum polish, only if the equations are mapped to code and
   tests.
4. Promote the hybrid VMEC/coil lane as the first open-SOL stellarator movie
   candidate. The next changes are the heavier stationarity preset, grid/time
   refinement, target/source/profile plot regeneration, combined
   `release_evidence` audit, release-hosted media, and frame-by-frame visual
   QA on the same consumed map.
5. Keep VMEC-extender finite-beta SOL artifact-gated. The next changes are a
   real/frozen artifact manifest, physical-phi and branch/sign tests,
   field-line/Poincare comparator checks, endpoint/wall classifier contract,
   FCI/operator gates, and a compact SOL smoke transient only after those
   gates pass.
6. Add the device matrix one device at a time: Landreman-Paul QA first, HSX
   QHS vacuum second, NCSX third, Dommaschk fourth. Each device must pass the
   same source-metadata, geometry, open/closed classification, operator,
   physics, refinement, media, and docs gates before README promotion.
7. Carry the real DRB model surface onto promoted geometries: potential-fed
   \(E\times B\), vorticity, Boussinesq/non-Boussinesq comparisons,
   sheath/recycling, neutrals, detachment, and selected electromagnetic terms.
   Any remaining surrogate nonlinear-transfer term must stay pedagogical and
   clearly labeled.
8. Treat broad recycling-solver, preconditioning, CPU/GPU, and
   differentiability work as support lanes. Try only a bounded number of
   materially different preconditioners, keep stable compatibility BDF as the
   default until full-output JAX/JVP gates are green, and tie every speedup
   claim to same-fidelity runtime and memory evidence.
9. Before tagging, rerun the local release gates: docs strict build, promoted
   coverage above `95%`, release-surface tests, artifact restore, package
   audit, and footprint audit. Hosted CI can wait while hosted runner billing
   is unavailable.
...
| --- | --- | ---: | --- | --- |
| 1 | Direct ESSOS-coil open-field simulations on `main` | 88%, finalized diagnostic contract | Keep `direct_coil_open_sol_demo.py` as the canonical pure-coil open-field workflow. The default summary now reports `near_term_closeout_status = finalized_diagnostic_contract`, lists deferred pure-coil movie/full-physics claims, and explains skipped live gates. Live signed-shell evidence remains diagnostic: full endpoint agreement is `0.763`, boundary-excluded agreement is `1.0`, shell mismatch coverage is `1.0`, and bulk mismatch is `0.0`. Next live work is limited to target-exit/adjacent-step nested diagnostics, source/profile and neutral-source diagnostics, target heat/particle flux plots, and diagnostic movie QA on the same consumed masks. | A clean clone can run the direct-coil open command and get a clear diagnostic ledger. Pure-coil media stays out of README promotion unless geometry, operator, source, refinement, and visual-QA gates all pass on the same live map. |
| 2 | Direct ESSOS-coil closed and near-closed controls | 92%, finalized closed control | Keep `direct_coil_closed_field_demo.py` as the pure-coil closed-control workflow. The self-contained default generates base return-map/Poincare, refinement, reduced transient, plot, and GIF artifacts. | Users can run a closed/near-closed coil-control example and see non-axisymmetric Poincare, return-map, profile, spectrum/movie diagnostics, explicit closed/near-closed classification, and explicit absence of `target`, `sheath`, `recycling`, `neutral_loss`, and target-to-target connection-length semantics. |
| 3 | VMEC closed-field stellarator controls | 90%, finalized contract | Keep VMEC-coordinate maps as the smooth closed-field control. The default command writes self-contained live-run contracts for periodic FCI/operator and reduced closed-field transient gates. Optional live VMEC regeneration remains available. | Closed VMEC examples pass zero-endpoint contract checks and never imply open-SOL target physics. Live examples must keep endpoint masks at zero and target/sheath/recycling/neutrals disabled. |
| 4 | Hybrid VMEC/coil open-SOL bridge | 94%, finalized compact release-backed bridge | Keep `hybrid_open_sol_demo.py` as the current compact open-SOL bridge. The default summary now reports `near_term_closeout_status = release_backed_compact_vacuum_bridge_ready` when the committed release-backed FCI/source, stationarity, grid/time, and media-manifest audit passes. Live regeneration remains optional for developers. | The compact hybrid bridge may be shown as release-backed vacuum evidence. It is not a finite-beta, full-DRB, multi-device, or long-window predictive turbulence claim. |
| 5 | VMEC-extender finite-beta exterior-field artifact lane | deferred, 35% | Keep the synthetic import contract on `main`; do not block near-term release on this lane. Future work needs a real/frozen artifact manifest, physical `phi`, branch/sign tests, field-line/Poincare comparators, endpoint/wall classifier contract, FCI/operator gates, and compact SOL closure gates. | A finite-beta example is promoted only after a real/frozen artifact passes strict field, wall/endpoint, connection, operator, sheath/recycling/neutral, refinement, and visual-QA gates. Until then it remains a synthetic import-contract demonstration and no upstream branch is a user dependency. |
| 6 | Device matrix | 30% | Add devices one at a time: Landreman-Paul QA first, HSX QHS vacuum second, NCSX third, Dommaschk potentials fourth. Each device must use the same source-metadata, geometry, open/closed-map, FCI/operator, physics, refinement, media, and docs gates. | Each promoted device has provenance, boundary/Poincare plots, connection-length or closed-step maps, endpoint labels where open, one validated physics result, release-hosted media if large, and a documented regeneration command. |
| 7 | Full DRB physics on promoted geometries | 74% | Carry selected-potential \(E\times B\), Boussinesq/non-Boussinesq vorticity, sheath, recycling, neutral, detachment, and selected electromagnetic terms onto promoted imported-field examples. Replace surrogate nonlinear-transfer terms in promoted examples with the real bracket/vorticity path, or keep the surrogate explicitly pedagogical. | Every promoted term has equations, implementation links, tests, plots, and parity or literature anchors. Coverage stays above `95%`, and performance/differentiability claims use the same promoted pure-JAX path. |
| 8 | Reference parity and diverted tokamak tutorials | 70% tutorials, 99% current parity blockers | Keep tokamak examples self-contained through small fixtures or release-hosted assets. Extend term-level parity to traced-field-line `g33`, sheath, target-source, longer diverted-tokamak windows, and every figure advertised in README/docs. | Clean-clone users can run tokamak simulations and analysis scripts, generate advertised movies/plots, and understand which outputs are native simulations versus restored validation fixtures. Remaining mismatches are bounded by field, term, location, runtime, and memory. |
| 9 | JAX-native recycling, preconditioning, CPU/GPU, and differentiability | 76% performance, 99.92% scoped recycling, 63% preconditioning | Try only a few materially different physics/block preconditioners, record negative evidence, then return to parity and geometry. Keep compatibility BDF as default until full-output JAX/JVP parity and same-fidelity runtime evidence are green. Tie CPU/GPU work to promoted heavy kernels only. | Solver, preconditioner, GPU, and differentiability claims include parity, solver-health, compile/execute, memory, and same-fidelity evidence. Experimental paths remain opt-in. |
| 10 | Docs, examples, coverage, and footprint | 98% docs/examples, 98% coverage, 97% footprint | Update README/docs only after artifacts are promoted. Keep large media, traces, reference baselines, NetCDF dumps, and profiler bundles in release assets or local ignored directories. Run local docs, release-surface, coverage, media-restore, package, and footprint gates before tags. | `mkdocs build --strict --clean`, promoted coverage above `95%`, release-surface tests, docs-media restore, package audit, and footprint audit pass before any tag. |
...
1. Finish the direct ESSOS-coil open diagnostic package on `main`.
   ...
   classified the remaining mismatch as `signed_transition_supported`. The
   stricter signed shell-refinement gate is also implemented, and the live
   boundary-resolved rerun now proves the mismatch is shell-localized rather
   than bulk or projection-false-positive dominated. Continue target-exit and
...
2. Package the direct ESSOS-coil closed/near-closed control as a clean-clone
   example. Implementation changes: release-hosted media manifest, README/docs
   command, return-map/provenance synchronization, and no target-language
   leakage. Gate: JSON proves closed/near-closed classification, mass control,
   fixed camera/color limits, and absence of target/sheath/recycling/neutrals.
3. Keep VMEC closed-field as the smooth control. Implementation changes:
   optionally add stronger periodic bracket/vorticity forcing and profile
   plots only if the equations are mapped to code/tests. Gate: zero endpoint
   masks, closed-map operator checks, profile/spectrum plots, and visual QA
   remain green.
4. Run the hybrid promotion preset as the first promoted stellarator open-SOL
   candidate. Implementation changes: heavier live preset, regenerated
   target/source/profile plots, grid/time refinement, stationarity report,
   media manifest, and frame-by-frame movie QA. Gate: same consumed hybrid map
   passes FCI/source-profile, connection/refinement, endpoint-mask
   reconstruction, source balances, stationarity, grid/time refinement, and
   visual QA.
5. Write the finite-beta VMEC-extender real/frozen artifact checklist.
   Implementation changes: artifact manifest schema, coordinate-convention
   tests, field-line/Poincare comparator contract, endpoint/wall classifier
   contract, FCI/operator gate, and compact SOL closure gate. Gate: the
   synthetic import contract remains tested on `main`; real finite-beta SOL
   waits for a frozen exterior-field artifact and full field/wall/operator/SOL
   validation.
6. Add HSX QHS, NCSX, and Dommaschk devices one at a time only after the
   Landreman-Paul QA direct, closed, and hybrid semantics are stable. Gate:
   each device has provenance, boundary/Poincare or surface plots, open/closed
   map classification, FCI/operator evidence, one physics result, docs, and
   release-hosted media if large.
...
The June 22, 2026 PR audit, rechecked with `gh`, gives the following planning
decision. The VMEC-extender work is useful for finite-beta scrape-off-layer
geometry, but it is not yet a `main`-branch dependency for ordinary JAXDRB
examples.
...
- `uwplasma/virtual_casing_jax` PR
  [#2](https://github.com/uwplasma/virtual_casing_jax/pull/2) is open with a
...
- `uwplasma/ESSOS` PR
  [#31](https://github.com/uwplasma/ESSOS/pull/31) is open with blocked merge
  state and provides the
...
- `uwplasma/ESSOS` PR
  [#33](https://github.com/uwplasma/ESSOS/pull/33) is open with blocked merge
  state and provides MGRID
  export from ESSOS coils, which is relevant for future VMEC or
  VMEC-extender provenance but not required for the current direct-coil import
  lane.
- `uwplasma/vmec_jax` PR
  [#19](https://github.com/uwplasma/vmec_jax/pull/19) is merged and adds
  segmented free-boundary controller custom-VJP validation. It is useful
  upstream context for differentiable finite-pressure equilibrium workflows,
  but it does not change the current JAXDRB SOL claim boundary.
- `uwplasma/vmec_jax` PR
...
- Current `uwplasma/vmec_jax` PRs
  [#20](https://github.com/uwplasma/vmec_jax/pull/20) and
  [#21](https://github.com/uwplasma/vmec_jax/pull/21) cover differentiability
  refactoring and mirror-geometry infrastructure. They are useful longer-term
  ecosystem context but do not provide JAXDRB SOL endpoint masks,
  sheath/recycling/neutral closures, or a complete finite-beta exterior-field
  artifact for JAXDRB consumption.
...
| Repository | PR | Provides | Remaining limitation | JAXDRB decision |
| --- | --- | --- | --- | --- |
...
| `uwplasma/ESSOS` | [#33](https://github.com/uwplasma/ESSOS/pull/33) | MGRID export from ESSOS coils for downstream equilibrium/exterior-field workflows. | It is an exporter/provenance enabler, not a JAXDRB open-SOL plasma workflow. | Track for future VMEC/VMEC-extender artifact provenance; do not block direct-coil simulations on it. |
| `uwplasma/vmec_jax` | [#19](https://github.com/uwplasma/vmec_jax/pull/19) | Segmented free-boundary controller custom-VJP validation. | Does not supply SOL maps, endpoint masks, sheath/recycling closure data, or plasma transients. | Treat as differentiable finite-pressure context for later finite-beta workflows. |
...
| `uwplasma/vmec_jax` | [#20](https://github.com/uwplasma/vmec_jax/pull/20), [#21](https://github.com/uwplasma/vmec_jax/pull/21) | Differentiability-refactor umbrella and mirror-geometry infrastructure. | Not VMEC-extender SOL artifact producers and not JAXDRB open-field plasma workflows. | Track only as ecosystem context; do not block the direct-coil, VMEC closed-field, hybrid, or VMEC-extender artifact lanes on them. |

Finite-beta SOL interpretation:

- VMEC-extender helps by supplying a prescribed finite-beta exterior magnetic
  field outside the LCFS through physical `(R, phi, Z)` arrays.
- It does not by itself provide wall geometry, target classification,
  sheath/recycling closures, neutral transport, or a validated open-SOL plasma
  result.
- JAXDRB should first validate the field artifact, the coordinate conventions,
  the field-line RHS, the metric/FCI operators, and the wall/endpoint maps;
  only then should finite-beta SOL movies or turbulence plots be promoted.

### Milestone A: ESSOS Direct-Coil Open-Field Simulations On `main`

Final goal:

Run a self-contained JAXDRB SOL campaign on `main` that imports ESSOS
coil-field traces from `ESSOS_biot_savart_LandremanPaulQA.json`, constructs
open-field maps, applies target/sheath/recycling/neutral closures on validated
endpoint masks, and produces diagnostic or promoted turbulence movies plus
connection-length, target, source, and profile plots. ESSOS owns Biot-Savart
and tracing; JAXDRB owns import, FCI maps, SOL residuals, diagnostics,
examples, docs, and movies.
...
- The movie passes frame-by-frame visual QA: no jitter, no camera drift, no
  renderer-only interpolation artifact, visible non-axisymmetric geometry,
...
Provide closed-field stellarator examples on VMEC-coordinate maps that teach
users how to run closed-surface simulations without incorrectly using open-SOL
target, sheath, or recycling semantics.

Concrete changes to make:

1. Add a closed-field tutorial script, preferably
   `examples/geometry-3D/vmec-closed-field/closed_field_turbulence_demo.py`,
   using top-level parameters for device, radial shell, grid, timestep,
   model level, and output root.
2. Add device presets for Landreman-Paul QA and HSX QHS vacuum first, then
   NCSX after the source metadata and boundary plots are stable.
3. Add closed-field validation gates:
   periodic parallel-gradient consistency, parallel-step convergence,
   closed-surface FCI interpolation, metric-weighted diffusion conservation,
   bracket/vorticity limiting cases, and conserved or controlled scalar
   quantities.
4. Add closed-field plotting utilities for surface geometry, `|B|` modulation,
   field-line label context, fluctuation amplitude, radial/surface averages,
   spectra, and turbulence movie frames.
5. Document explicitly that closed VMEC maps do not define target-to-target
   connection length, sheath losses, or recycling unless an artificial loss
   model is deliberately selected and labeled.

Definitions of success:

- Closed-field examples run from a clean clone with no external reference-code
  install required, using committed small fixtures or release-hosted artifacts.
- Closed-field diagnostics use periodic parallel-step or effective parallel
  metrics, not open target-to-target quantities.
- At least one closed-field movie and one profile/spectrum figure are
  generated from documented commands and pass visual QA.
- The closed-field lane provides the control comparison for open direct-coil
  and hybrid SOL lanes.

### Milestone C: Hybrid VMEC/Coil Open-SOL Simulations

Final goal:

Promote the hybrid lane as the first practical stellarator open-SOL route:
VMEC gives smooth non-axisymmetric coordinates and metric structure; ESSOS
coil traces supply endpoint masks, connection-length proxy, and `|B|`
modulation needed for open-field losses.

Concrete changes to make:

1. Keep the current hybrid compact movie as release-scoped evidence, but add a
   longer refined campaign before making a publication-level turbulence claim.
2. Add strict consumed-map checks so sheath/recycling closures consume exactly
   the forward/backward endpoint masks generated by the imported map.
3. Add source-balance plots: particle source, neutral density, recycling
   source, target heat/particle flux, and target-label maps.
4. Add grid and timestep refinement for the hybrid reduced transient, with
   profile diagnostics in addition to movie stationarity metrics.
5. Add a tutorial script, preferably
   `examples/geometry-3D/essos-field-lines/hybrid_open_sol_demo.py`, that
   uses top-level parameters and imports plotting functions from `jax_drb`.

Definitions of success:

- Hybrid connection-length and endpoint masks pass the live multi-grid gate
  using the source-appropriate normalized quantity.
- Sheath/recycling/neutral accounting residuals remain below documented
  tolerances on the same endpoint masks consumed by the transient.
- The hybrid movie shows non-axisymmetric geometry, open-field losses, radial
  structure, and turbulent evolution without jitter or visual aliasing.
- README claims identify this as the promoted open-SOL bridge until pure-coil
  direct maps pass their own promotion gates.

### Milestone D: VMEC-Extender Finite-Beta Exterior-Field SOL

Final goal:

Use VMEC-extender finite-beta exterior-field artifacts to run prescribed-field
SOL simulations on grids outside the VMEC LCFS, with the finite-beta magnetic
field, geometry, connection lengths, wall/endpoint maps, and JAXDRB SOL
operators all validated separately.

Concrete changes to make:

1. Keep the JAXDRB runtime dependency as a NetCDF artifact contract:
   coordinates `R`, physical `phi`, `Z`; field components `BR`, `Bphi`, `BZ`;
   `absB`; `nfp`; `phi_period` or full-torus metadata; branch/source/unit
   metadata; and provenance.
2. Add an optional developer workflow that calls the upstream exporter when
   available, but keep ordinary examples artifact-based and self-contained.
3. Add real-artifact validation:
   node samples, midpoint interpolation, physical-phi periodicity, `absB`
   closure, `dR/dphi=R BR/Bphi`, `dZ/dphi=R BZ/Bphi`, divergence/field-period
   diagnostics, and LCFS/exterior-domain bounds.
4. Add finite-beta exterior connection-length and wall-hit diagnostics. Record
   whether the field line is closed, leaves the imported box, hits a wall, or
   reaches a target classifier.
5. Add metric/FCI gates on the imported exterior grid. If full metric tensors
   are not in the artifact, compute only the metrics justified by the imported
   coordinates and label the limitations.
6. Add a compact finite-beta SOL smoke transient only after the field artifact,
   endpoint/wall maps, and FCI operators pass.

Definitions of success:

- A real finite-beta exterior-field artifact passes strict metadata and
  coordinate-convention checks without unsafe overrides.
- Field-line/Poincare/connection diagnostics agree with the upstream exporter
  within documented tolerances.
- The finite-beta SOL example records which quantities are prescribed-field
  geometry and which are self-consistent plasma variables.
- No finite-beta SOL movie is promoted until wall/endpoint and operator gates
  are green.

### Milestone E: Device Program For HSX, NCSX, Landreman-Paul QA, And Dommaschk

Final goal:

Create a small but reviewer-proof device matrix in which each device has a
geometry source, validation plots, open/closed semantics, and one bounded
physics result.

Device order:

1. Landreman-Paul QA: use ESSOS coils, VMEC closed map, hybrid open-SOL bridge,
   and eventually VMEC-extender finite-beta exterior field.
2. HSX QHS vacuum: use
   [landreman/vmec_equilibria HSX/QHS_vac](https://github.com/landreman/vmec_equilibria/tree/master/HSX/QHS_vac)
   as the first additional closed-field VMEC case.
3. NCSX: add after the closed-field VMEC tutorial is stable and source
   metadata/provenance are documented.
4. Dommaschk potentials: use the BSTING/Zoidberg workflows as FCI and
   visualization references, but keep JAXDRB responsible for imported arrays,
   operators, physics, tests, docs, and claims.

Per-device success gates:

- source metadata and license/provenance;
- boundary/surface plot and field-line or Poincare plot;
- connection-length or closed parallel-step map;
- endpoint/sheath mask map for open/hybrid cases;
- FCI operator convergence plot;
- one linear or reduced nonlinear physics output figure;
- grid/time refinement or explicit reduced-scope label;
- release-hosted media and documented regeneration command.

### Milestone F: Full DRB Physics, Recycling, And Differentiability Promotion

Final goal:

Move from compact geometry/reduced-transient demonstrations to full
drift-reduced Braginskii examples with selectable models, boundaries, and
geometries, while preserving differentiability claims only on pure-JAX paths.

Concrete changes to make:

1. Replace any promoted surrogate nonlinear transfer term with the real
   ExB bracket/vorticity/potential path, or keep the surrogate labeled as
   pedagogical only.
2. Add Boussinesq and non-Boussinesq comparisons with potential solve,
   vorticity response, particle/heat flux, symmetry/asymmetry metrics, and
   runtime cost.
3. Promote neutral/sheath/recycling/detachment terms through component tests,
   source accounting, target-response scans, and term-level parity where
   reference output exists.
4. Continue the JAX-native recycling path behind an opt-in API until
   full-output production-window parity, runtime, memory, and solver-health
   evidence justify default promotion.
5. Add differentiability examples only on pure-JAX residual/objective surfaces:
   `jvp`, `grad`, finite-difference comparison, UQ, sensitivity, and inverse
   design/control examples.
6. Add CPU/GPU scaling only on same-fidelity real kernels or ensembles. Do not
   promote toy-kernel speedups as solver speedups.

Definitions of success:

- Every promoted equation term has a derivation, code link, unit/normalization
  statement, and physics/numerical test.
- Every promoted model switch has an example and a plot explaining the effect.
- Full-output recycling remains stable and parity-bounded; JAX/JVP paths are
  either demonstrably competitive or explicitly opt-in.
- Differentiability plots include finite-difference error curves and state the
  non-differentiable boundaries, if any.

### Immediate Execution Sequence

Use this order when implementation resumes:

1. Finish the main-branch direct ESSOS-coil package: run or add the open-field
   refinement/source/profile gates, run the closed/near-closed control
   refinement check, create the diagnostic direct-coil turbulence movie, and
   keep the movie labeled diagnostic unless all promotion gates pass.
2. Add VMEC closed-field reduced transient/profile/spectrum/movie diagnostics
   on top of the validated periodic VMEC FCI map. Keep target, sheath,
   recycling, and neutral-loss semantics disabled in that closed-field lane.
3. Add a longer-window or refinement check for the live direct-coil
   closed/near-closed control before any closed direct-coil movie claim.
4. Upgrade the hybrid open-SOL campaign with longer refined transients,
   source/target/profile plots, grid/time checks, and movie QA through
   `examples/geometry-3D/essos-field-lines/hybrid_open_sol_demo.py`. Promote
   hybrid as the first open-SOL bridge only if the workflow summary reports
   green FCI/source-profile, parallel-step refinement, stationarity,
   grid/time, and visual-QA gates.
5. Re-run docs-media restore and release-surface checks after adding or moving
   media.
6. Only after direct-coil, direct-coil closed-control, closed-VMEC, and hybrid
   semantics are unambiguous, generate or import a real VMEC-extender
   finite-beta exterior-field artifact and run Milestone D.
7. Add HSX, NCSX, and Dommaschk devices one at a time through the per-device
   success gates.
8. Resume full DRB, JAX-native recycling default, performance, GPU, and
   architecture work only when those lanes are required by a promoted geometry
   example or release claim.

Stop conditions:

- If a connection-length or refinement gate fails, record the negative
  evidence and keep the corresponding movie out of README promotion.
- If a PR/upstream exporter changes artifact conventions, update the import
  contract and tests before regenerating physics movies.
- If a figure or movie is visually ambiguous, jittery, axisymmetric-looking,
...
- 2026-06-22: Narrowed the immediate completion target to the finalizable
  ...
  remains unpromoted until live geometry/source/refinement/movie gates pass.
  Direct ESSOS-coil closed/near-closed controls are finalized as
  self-contained closed-control examples with base, refinement, transient,
  plot, and GIF evidence and no target/sheath/recycling/neutral semantics.
  VMEC closed-field controls are finalized as self-contained contracts with
  optional live VMEC regeneration and zero endpoint masks. The hybrid VMEC/coil
  lane is finalized within the compact release-backed vacuum bridge scope when
  the default workflow reports
  `near_term_closeout_status = release_backed_compact_vacuum_bridge_ready`.
  Deferred for later: finite-beta VMEC-extender artifacts, full DRB physics on
  every geometry, broad preconditioner/GPU promotion, HSX/NCSX/Dommaschk
  device expansion, and long-window predictive turbulence.
- 2026-06-22: Reran the live direct ESSOS-coil boundary-resolved
  endpoint-label gate with the stricter signed-shell diagnostics in
  `/tmp/jax_drb_direct_coil_signed_shell_gate_current`. The report remains
  `promotion_ready=false` because full endpoint agreement is `0.763`, but the
  shell diagnosis is now clean: `dominant_signed_target_transition_mode` is
  `signed_transition_supported`,
  `dominant_signed_target_transition_shell_mode` is
  `signed_transition_shell_refinement_supported`, transition consistency is
  `0.988`, projection false positives are `0.0121`, shell mismatch coverage is
  `1.0`, outside-shell label stability is `1.0`, outside-shell signed-exit
  evidence stability is `1.0`, and bulk mismatch is `0.0`. Boundary-excluded
  all-label and endpoint agreement remain `1.0` over `0.248` valid fraction.
  Decision: move direct ESSOS-coil open diagnostics to `85%` and the overall
  3D stellarator open/closed SOL program to `78%`. The next technical work is
  no longer broad endpoint projection; it is target-exit, adjacent-step,
  source/profile, neutral-source, target heat/particle flux, and movie-QA
  gates on the same consumed endpoint masks.
- 2026-06-22: Implemented the signed target-transition-shell
  refinement/classification gate in the imported endpoint-label diagnostics.
  Pair reports now include shell mismatch coverage, outside-shell label
  stability, outside-shell signed-exit evidence stability, shell fraction,
  bulk mismatch fraction, a `signed_target_transition_shell_mode`, and a
  boolean `signed_target_transition_shell_refinement_supported`. Aggregate
  reports and direct-coil workflow summaries expose the same shell decision,
  and the endpoint-label plot title reports signed mode, shell mode, and shell
  coverage. Focused tests now distinguish three cases: a true shell-supported
  transition, a nearest-projection false positive, and a bulk signed mismatch
  that must not be treated as shell-only. Decision: move direct ESSOS-coil
  open diagnostics to `83%` and the overall 3D stellarator open/closed SOL
  program to `77%`. The next executable step is a live boundary-resolved
  direct-coil rerun with the new shell fields, followed by target-exit,
  adjacent-step, source/profile, neutral-source, target-flux, and movie-QA
  gates on the same consumed endpoint masks.
...
- 2026-06-22: Added conservative endpoint-projection diagnostics to the
  ...
  `(7, 15, 27) -> (11, 25, 45)` remains useful negative evidence:
  nearest-projected endpoint agreement is `0.763`, projection-neighborhood
  endpoint-mismatch support is `0.853`, but broad one-cell endpoint-union
  projection lowers endpoint agreement to `0.656`, raises endpoint-union
  population to `0.958`, and creates endpoint false positives `0.122`.
  Decision: move the direct-coil open-field lane to `80%` and the overall 3D
  stellarator open/closed SOL lane to `74%`. Do not promote pure-coil
  open-field media and do not pursue broad endpoint dilation as the fix. The
  next implementation is signed wall-hit/target-distance and transition-shell
  diagnostics that distinguish real forward/backward target hits from
  nearest-projection false positives.
- 2026-06-22: Added coordinate-aware projection-neighborhood metrics to the
  endpoint-label refinement diagnostics and surfaced their maxima in the
  direct-coil open workflow summary. The metric checks whether a mismatched
  coarse endpoint label is present within a one-cell fine-grid neighborhood of
  the nearest projected sample. Decision: move the direct-coil open-field lane
  to `79%` and the overall 3D stellarator open/closed SOL lane to `73%`. Next
  rerun the boundary-resolved endpoint-label stage; high neighborhood support
  points to conservative target-boundary projection, while low support points
  to wall-hit retention or classifier work before any pure-coil media
  promotion.
- 2026-06-22: Reran live direct ESSOS-coil endpoint-label refinement on a
  larger, less boundary-dominated pair
  `/tmp/jax_drb_direct_coil_endpoint_boundary_larger_pair_gate` with
  `(7, 15, 27) -> (11, 25, 45)` grids. The full endpoint-label gate still
  fails promotion (`endpoint_agreement=0.763`), but the boundary-excluded
  valid fraction is now `0.248`, above the direct-coil `0.20` requirement, and
  both boundary-excluded all-label and endpoint-label agreement are `1.0`.
  Decision: move the direct-coil open-field lane to `77%` and the overall 3D
  stellarator open/closed SOL lane to `71%`. The remaining pure-coil blocker is
  target transition-shell projection and forward/backward target
  classification, not bulk field-map failure. Next implementation change is to
  wire this boundary-resolved endpoint-label diagnostic into the direct-coil
  workflow as an optional stage, then improve target-boundary projection before
  pure-coil media promotion.
- 2026-06-22: Added
  `RUN_LIVE_BOUNDARY_RESOLVED_ENDPOINT_LABEL_REFINEMENT_GATE` to
  `direct_coil_open_sol_demo.py` and exposed the `(7, 15, 27) -> (11, 25, 45)`
  diagnostic in the clean-clone workflow summary. The stage is diagnostic-only
  by design; it records whether enough boundary-excluded endpoint cells support
  the target-transition-shell interpretation, but it does not promote pure-coil
  media by itself. Decision: move the direct-coil open-field lane to `78%` and
  the overall 3D stellarator open/closed SOL lane to `72%`. Next code work is
  target-boundary endpoint projection, then rerun this same stage and the
  adjacent-step/source-profile gates.
- 2026-06-22: Added `minimum_boundary_excluded_valid_fraction` to the
  endpoint-label gate and set the direct-coil workflow requirement to `0.20`.
  Reran the live odd-ratio direct-coil endpoint gate in
  `/tmp/jax_drb_direct_coil_endpoint_boundary_threshold_gate`. The gate still
  has perfect boundary-excluded all-label and endpoint-label agreement, but
  the boundary-excluded valid fraction is `0.022`, so
  `boundary_excluded_valid_fraction_passed=false` and
  `target_boundary_only_instability=false` under the stricter direct-coil
  requirement. Decision: move the direct-coil open-field lane to `76%`; next
  work should improve target-boundary projection or grid choice until there is
  enough interior support, then rerun endpoint-label, adjacent-step, and
  source/profile gates before media promotion.
- 2026-06-22: Added boundary-excluded endpoint-label agreement metrics and
  reran the live odd-ratio direct-coil endpoint gate in
  `/tmp/jax_drb_direct_coil_endpoint_boundary_excluded_gate`. The gate remains
  `promotion_ready=false`, but outside the directional transition shell the
  all-label and endpoint-label agreement fractions are both `1.0`. The
  boundary-excluded valid fraction is only `0.022`, so this is strong
  localization evidence but not promotion evidence. The plot title now records
  the boundary localization, target-projection suspicion, and interior
  agreement/coverage. Decision: move the direct-coil open-field lane to `75%`;
  next fix or refine target-boundary projection/classification and rerun on a
  less boundary-dominated grid before any pure-coil open-SOL media promotion.
- 2026-06-22: Reran the live odd-ratio direct ESSOS-coil endpoint-label gate in
  `/tmp/jax_drb_direct_coil_endpoint_boundary_localization_gate` after adding
  transition-shell localization. The gate remains not promotion-ready with
  all-label agreement `0.474`, endpoint agreement `0.458`, and endpoint-union
  population `0.970`, but the new diagnosis is decisive:
  `dominant_endpoint_boundary_localization =
  direction_boundary_localized`, `target_boundary_projection_suspected =
  true`, all mismatches lie on directional transition shells, and no mismatch
  lies outside those shells. The regenerated plot was visually checked and now
  displays this localization in the title. Decision: move the direct-coil
  open-field lane to `74%`; the next code work is target-boundary projection
  and forward/backward wall-hit classification, not field-map replacement.
- 2026-06-22: Added endpoint-label transition-shell localization diagnostics
  to the imported FCI endpoint-label gate. Each nested pair now reports whether
  label mismatches are concentrated on endpoint-presence or directional target
  transition shells, or whether they are spread through the bulk map. The
  direct-coil workflow summary surfaces
  `dominant_endpoint_boundary_localization`,
  `target_boundary_projection_suspected`, and
  `projection_recommended_next_action`. Focused tests distinguish stable,
  bulk directional mismatch, and target-boundary-localized mismatch cases.
  Decision: move the direct-coil open-field lane to `73%`; next rerun the live
  direct-coil endpoint gate with these fields, then fix target-boundary
  projection only if the live failure is boundary-localized.
- 2026-06-22: Refroze the single authoritative plan for the remaining open
  lanes. The Markdown audit found no other active roadmap that should override
  this file; subordinate pages are appendices only. The final sequence is now:
  direct ESSOS-coil open diagnostics on `main`, direct-coil closed/near-closed
  controls, VMEC closed-field controls, hybrid VMEC/coil open-SOL promotion,
  VMEC-extender finite-beta artifact intake, device expansion, full DRB physics
  promotion, and bounded solver/performance/release gates. Rechecked PR status
  with `gh`: `uwplasma/jax_drb#2` remains merged, `uwplasma/virtual_casing_jax#2`
  is open/clean, `uwplasma/ESSOS#31` and `#33` are open/blocked, and
  `uwplasma/vmec_jax#18/#19` remain merged finite-pressure/free-boundary
  context. Decision: main-branch work proceeds with ESSOS direct-coil and
  hybrid examples now; VMEC-extender finite-beta SOL stays behind a real/frozen
  artifact gate.
- 2026-06-22: Strengthened the direct ESSOS-coil endpoint-label refinement
  gate by adding `minimum_endpoint_union_fraction`. The generic imported-label
  diagnostic can still allow zero endpoint population for closed-map studies,
  but `direct_coil_open_sol_demo.py` now requires a nonzero endpoint union for
  open-SOL promotion. The gate report and plot now include endpoint-union
  population, and focused tests prove that an all-interior nested label set is
  rejected rather than passing vacuously. Decision: move the direct-coil
  open-field lane to `69%`; the next run should rerun the live direct-coil
  endpoint-label and adjacent-step gates with this stricter endpoint-population
  evidence before generating or promoting any direct-coil movie.
- 2026-06-22: Reran the live direct ESSOS-coil endpoint-label refinement gate
  in `/tmp/jax_drb_direct_coil_endpoint_population_gate` using the stricter
  endpoint-population criterion. The gate remains `promotion_ready=false`, but
  the new evidence is better localized: minimum endpoint-union population is
  `0.898`, so the failure is not vacuous or due to absent target contact.
  Minimum all-label agreement and endpoint-agreement are both `0.444`, with
  later-level endpoint agreement `0.573`; rejection reasons remain
  `label_agreement_below_threshold` and
  `endpoint_label_agreement_below_threshold`. The generated plot was visually
  checked and the title/layout was tightened. Decision: keep direct-coil media
  diagnostic and target endpoint-label stability/projection before another
  direct-coil movie promotion attempt.
- 2026-06-22: Added endpoint-label instability-mode diagnostics to classify
  whether a nested endpoint-label failure is dominated by absent endpoints,
  false positives, false negatives, or forward/backward directional mismatch.
  Rebuilding the live direct-coil endpoint-label levels classifies both nested
  pairs as `directional_endpoint_mismatch`; the recommended next action is to
  inspect forward/backward target classification and bidirectional cells at
  collocated seeds. Focused tests cover no-endpoint, stable, and directional
  mismatch cases. Decision: move the direct-coil open-field lane to `70%`;
  the next technical fix is endpoint-direction projection, not endpoint
  population.
- 2026-06-22: Added forward/backward component-level endpoint diagnostics to
  the endpoint-label gate. The live direct-coil label levels show both forward
  and backward bits have false positives and false negatives; the dominant
  direction-component error is `balanced_forward_backward_components`. This
  rules out a one-sided target-label sign bug and points the next work toward
  endpoint projection and seed-collocation stability. Decision: move the
  direct-coil open-field lane to `71%` while keeping media diagnostic.
- 2026-06-22: Added an odd-ratio collocated endpoint-label diagnostic stage to
  `direct_coil_open_sol_demo.py`. A live `(3, 5, 9) -> (7, 15, 27)` pure-coil
  run keeps periodic coarse seed angles on refined-grid seed angles, but still
  fails with all-label agreement `0.474`, endpoint agreement `0.458`,
  endpoint-union population `0.970`, and dominant component error
  `forward_component`. Decision: move the direct-coil open-field lane to
  `72%`; even-ratio non-collocation is not the sole blocker, so the next fix
  should target endpoint projection and target-boundary classification.
- 2026-06-22: Reran the live hybrid FCI/source-profile gate locally in
  `/tmp/jax_drb_hybrid_live_fci_source_profile`. The gate passes with
  `target_fraction = 0.90375`, connection-resolution and map diagnostics
  passing, particle-recycling error `2.2e-15`, neutral-particle error
  `2.5e-18`, zero current-balance and neutral-momentum errors, a positive
  target particle-loss map, and visually checked FCI/source-profile plots.
  Decision: move the hybrid open-SOL lane to `89%`; remaining promotion
  blockers are the heavy stationarity preset, grid/time refinement, media
  regeneration, and frame-by-frame visual QA on the same consumed map.
- 2026-06-22: Added target particle-flux map support to the imported FCI
  source/profile evidence lane. Regenerated FCI artifacts now include
  `particle_loss_toroidal`, the source/profile gate records whether that
  optional map is present and positive, and the standalone gate plot renders
  the target particle-loss flux next to target labels, heat load, neutral
  ionisation, radial profiles, and source-accounting status. The field remains
  optional for older release-hosted NPZ files, so existing restored artifacts
  stay readable while new live direct-coil and hybrid campaigns produce the
  stronger plot. Decision: move the direct-coil open-field lane to `68%`; the
  remaining direct-coil blockers are live endpoint-label/adjacent-step
  refinement, neutral-source refinements, and diagnostic movie QA.
...
- 2026-06-22: Tightened the single authoritative plan for the remaining
  ...
  are merged, while `uwplasma/virtual_casing_jax#2`, `uwplasma/ESSOS#31`, and
  `uwplasma/ESSOS#33` remain open upstream artifact/export enablers. Decision:
  keep `main` focused on ESSOS direct-coil open/closed workflows now; package
  direct-coil closed controls as clean-clone evidence; promote hybrid VMEC/coil
  open-SOL only after combined evidence and live promotion gates are green; and
  keep finite-beta VMEC-extender as a strict frozen-artifact lane until a real
  exterior-field artifact passes field, wall/endpoint, connection-length,
  FCI/operator, sheath/recycling/neutral, refinement, and visual-QA gates.
- 2026-06-22: Added the combined hybrid open-SOL promotion evidence audit.
  `audit_hybrid_open_sol_promotion_evidence` now checks the committed
  high-resolution hybrid FCI/source report, long-window stationarity report,
  grid/time-refinement summary, release-media manifest, and visual-QA metadata
  as one report-only gate. The committed bundle passes the audit with
  `promotion_ready = true`; a regression test rejects a stale/mismatched media
  manifest with wrong map source, failed visual QA, missing diagnostics/contact
  sheet files, and non-project release URLs. Decision: move the hybrid lane to
  `86%`; next run the heavier live promotion preset through
  `hybrid_open_sol_demo.py` and keep the combined audit green after any media
  regeneration.
- 2026-06-22: Refreshed the VMEC closed-field control lane with a live
  Landreman-Paul QA transient/profile/spectrum/movie run on the `(5, 8, 20)`
  VMEC map. The gate passes with zero endpoint fraction, mass relative drift
  `2.1e-15`, final fluctuation RMS `3.38e-2`, finite spectrum, positive
  density, and target, sheath/recycling, and neutral-loss semantics all
  disabled. A GIF contact sheet showed fixed framing and no color-scale
  jitter, but the dynamics remain visually quiet; this remains closed-field
  control evidence, not promoted stellarator SOL turbulence media. Added
  explicit `fixed_camera`, `fixed_color_limits`, and
  `movie_visual_qa_passed` report fields and locked them in the VMEC
  closed-field transient test. Decision: move the VMEC closed-field lane to
  `86%`; the next promoted visual lane remains hybrid open-SOL unless a
  stronger closed-field bracket/vorticity model is added.
- 2026-06-22: Advanced the direct ESSOS-coil closed/near-closed control lane.
  The live Landreman-Paul QA return-map and refinement evidence was reused as
  a restored trace bundle to generate a reduced closed-trace transient and GIF
  without retracing the expensive coil field. The transient now reports
  `source_mode = provided_trace_bundle`, uses 16 field lines and 14 output
  frames, and passes the closed-control media gate with mass relative drift
  `1.5e-16`, final fluctuation RMS `3.99e-2`, temporal RMS change `5.07e-3`,
  final density range `[0.908, 1.100]`, fixed color/camera visual QA, and
  `open_sol_publication_ready = false`. Patched the validation campaign so
  user-supplied/restored trace arrays are not mislabeled as
  `self_contained_contract`, and added a regression test for provenance
  preservation through the base and transient reports. Decision: move the
  direct-coil closed-control lane to `84%`; remaining work is release-hosted
  media packaging and conservative README promotion, not basic closed-control
  QA.
- 2026-06-22: Rechecked the PR dependency state while finalizing the
  authoritative stellarator open/closed-field plan. `uwplasma/jax_drb#2`
  remains merged, so the JAXDRB VMEC-extender NetCDF import contract,
  physical-phi interpolation checks, field-line RHS helpers, and synthetic SOL
  smoke gate are already on `main`. `uwplasma/vmec_jax#18` and `#19` remain
  merged finite-pressure/free-boundary context. `uwplasma/virtual_casing_jax#2`,
  `uwplasma/ESSOS#31`, and `uwplasma/ESSOS#33` remain open artifact/export
  enablers, so finite-beta VMEC-extender SOL stays an artifact-import lane
  rather than a main-branch runtime dependency. The executable order is now:
  direct ESSOS-coil open-field diagnostics, direct-coil closed controls, VMEC
  closed-field controls, hybrid VMEC/coil open-SOL promotion, finite-beta
  artifact import, then HSX/NCSX/Dommaschk expansion.
- 2026-06-22: Advanced the hybrid VMEC/coil open-SOL lane. The workflow
  summary now uses the same required-stage promotion semantics as the
  direct-coil workflow, so partial live runs cannot promote skipped
  refinement, stationarity, grid/time, or media stages. A live local
  Landreman-Paul QA hybrid pass cleared FCI/source-profile and
  parallel-step-per-radian refinement: target fraction `0.90375`, `|B|`
  modulation `1.4167`, particle-recycling relative error `2.2e-15`,
  neutral-particle relative error `2.5e-18`, exact consumed endpoint-mask
  reconstruction, finite overlap `1.0`, observed order `1.20`, finest
  normalized RMS error `5.90e-2`, finest normalized \(L_\infty\) error
  `1.18e-1`, and monotone RMS/\(L_\infty\) reduction. Added an explicit
  `STATIONARITY_PRESET = "quick" | "promotion"` switch: the quick preset
  passed as workflow smoke evidence on the same assets but is blocked from
  promotion with `quick_stationarity_preset_not_promotion_evidence`. Decision:
  move the hybrid lane to `84%`; the remaining promotion blockers are
  promotion-preset stationarity, grid/time movie refinement, media generation,
  and frame-by-frame visual QA.
- 2026-06-22: Consolidated the remaining open lanes into the single active
  lane board under `Current Authoritative Open-Lane Implementation Plan`.
  The board now records current completion percentages, exact next changes,
  milestones, success definitions, PR dependency decisions, execution rules,
  and the immediate implementation checklist for direct ESSOS-coil open/closed
  simulations, VMEC closed-field controls, hybrid open-SOL promotion,
  VMEC-extender finite-beta artifact import, device expansion, full DRB
  physics, tokamak tutorials, and release hygiene. The PR audit was rechecked:
  `uwplasma/jax_drb#2` is merged; `uwplasma/virtual_casing_jax#2`,
  `uwplasma/ESSOS#31`, and `uwplasma/ESSOS#33` remain upstream artifact/export
  enablers; `uwplasma/vmec_jax#18` and `#19` are merged finite-pressure
  context rather than JAXDRB SOL workflows. Decision: continue from this file
  only, keep main-branch work on ESSOS direct-coil and hybrid examples, and
  keep VMEC-extender finite-beta SOL as artifact import until real/frozen
  exterior-field evidence passes the full gate chain.
- 2026-06-22: Re-ran the live pure-coil endpoint-label refinement gate on the
  same local Landreman-Paul QA assets. The gate remains negative evidence for
  pure direct-coil media promotion: coarse-to-middle all-label and endpoint
  agreement are both `0.444`, middle-to-fine all-label agreement is `0.616`,
  middle-to-fine endpoint agreement is `0.573`, and the report fails the
  `0.90` all-label and `0.80` endpoint thresholds with
  `endpoint_label_instability`. The rendered endpoint-label/refinement plot
  was visually inspected and shows substantial directional-label changes
  under refinement. Decision: do not promote pure direct-coil turbulence media
  without a materially improved endpoint-map construction; move the promoted
...
- 2026-06-22: Performed the requested final plan-only consolidation for the
  ...
  open-field simulations on `main`, direct-coil closed/near-closed controls,
  VMEC closed-field controls, hybrid VMEC/coil open-SOL promotion,
  VMEC-extender finite-beta artifact import, device expansion, then full DRB
  physics/JAX-native/performance promotion where needed. Rechecked the PR and
  dependency state with `gh`: `uwplasma/jax_drb#2` is merged; the local
  `origin/feature/vmec-extender-edge-fields` branch is historical;
  `uwplasma/virtual_casing_jax#2`, `uwplasma/ESSOS#31`, and
  `uwplasma/ESSOS#33` remain open artifact/export enablers; `vmec_jax#18` and
  `vmec_jax#19` are merged finite-pressure/free-boundary context; and
  `vmec_jax#20/#21` remain longer-term ecosystem PRs. Decision: keep
  `main` focused on ESSOS direct-coil field imports and diagnostic movies now,
  promote hybrid only after refined endpoint/source/profile/movie gates, and
  keep VMEC-extender finite-beta SOL artifact-gated until a frozen exterior
  field artifact passes strict field, wall/endpoint, connection-length,
  FCI/operator, sheath/recycling/neutral, and refinement gates.
- 2026-06-22: Added the self-contained direct ESSOS-coil closed/near-closed
  transient/media gate. `essos_direct_coil_closed_control_campaign.py` now
  exposes a reduced periodic line-following scalar transient on the same
  closed-control trace bundle used by the return-map classifier, writes
  JSON/NPZ/PNG/GIF artifacts, tracks mass drift, fluctuation RMS, fixed-camera
  and fixed-color movie assumptions, and records that target, sheath,
  recycling, and neutral-loss semantics are absent. The direct-coil closed
  example now runs the base return-map gate, refinement gate, and transient
  package by default in self-contained mode. Decision: this closes the local
  closed-control media tutorial gap; live same-source Landreman-Paul QA
  transient/movie QA remains required before README promotion.
- 2026-06-21: Routed the Boussinesq/non-Boussinesq polarization choice through
  the compact FCI DRB PyTree RHS. `FciDrbRhsParameters` now exposes
  `potential_boussinesq`, preserving the old Boussinesq default while allowing
  non-Boussinesq density-weighted potential solves in the same transformable
  RHS surface. The PyTree campaign now compares both potential solves on the
  same state, verifies that only the diagnostic potential seam changes while
  source-array RHS terms remain identical, and runs a non-Boussinesq
  objective through `jax.jvp` versus finite difference. The local default-grid
  QA artifact passed with Boussinesq/non-Boussinesq potential relative
  difference `1.04e-1`, source-state RHS Linf difference `0.0`,
  density-over-\(B^2\) contrast `4.74`, and non-Boussinesq JVP relative error
  `2.9e-9`. Decision: the compact RHS route is closed; the remaining full-DRB
  blocker is feeding the selected potential into promoted nonlinear
  \(E\times B\) bracket/advection examples and then carrying that through
  imported-field/open-SOL campaigns.
- 2026-06-21: Advanced the full DRB physics lane by promoting the existing FCI
  vorticity campaign from a single polarization inversion to an explicit
  Boussinesq versus non-Boussinesq gate. The campaign now applies and inverts
  both \(K_\mathrm{pol}=\langle n/B^2\rangle\) and
  \(K_\mathrm{pol}=n/B^2\), records both reconstruction errors and residuals,
  verifies that the variable-coefficient operator differs from the Boussinesq
  operator on a nonuniform density field, and verifies roundoff-level
  equivalence when \(n/B^2\) is constant. A local default-size QA artifact
  passed with Boussinesq/non-Boussinesq potential errors `1.30e-3` and
  `1.44e-3`, residuals `2.04e-4` and `4.76e-4`, operator-difference relative
  L2 `0.365`, and constant-coefficient mismatch `1.66e-10`. Decision: the
  Boussinesq/non-Boussinesq component gate is no longer a future-only item;
  the remaining blocker is routing the switch through promoted production
  examples and larger imported-field SOL solves.
- 2026-06-21: Re-laid the remaining open-lane plan into the existing single
  authoritative plan rather than creating another roadmap. Rechecked the
  relevant PR state with `gh`: `uwplasma/jax_drb#2` is merged and the local
  `origin/feature/vmec-extender-edge-fields` branch is historical;
  `uwplasma/virtual_casing_jax#2`, `uwplasma/ESSOS#31`, and
  `uwplasma/ESSOS#33` remain open upstream artifact/export enablers; and the
  relevant `vmec_jax` finite-pressure/free-boundary context PRs are merged or
  longer-term ecosystem work. Decision: keep `main` centered on ESSOS
  direct-coil open/closed simulations and diagnostic direct-coil turbulence
  movies first, keep VMEC closed-field as the smooth closed control, use the
  new hybrid open-SOL workflow ledger as the planned promotion bridge, and keep
  VMEC-extender finite-beta SOL artifact-gated until a frozen
  provenance-rich exterior-field artifact exists. Added
  `examples/geometry-3D/essos-field-lines/hybrid_open_sol_demo.py` as a
  self-contained dry-run promotion ledger with opt-in live FCI/source-profile,
  parallel-step refinement, stationarity, grid/time refinement, and media
  gates.
- 2026-06-21: Ran the new direct-coil closed-control refinement gate on the
  local live ESSOS Landreman-Paul QA coil geometry in a temporary directory.
  The compact same-source sweep used levels `(3, 3, 256)`, `(4, 4, 384)`,
  and `(5, 4, 512)` at `maxtime = 240` and passed with closed-or-near fraction
  minimum `1.0`, closed-or-near spread `0.0`, maximum p95 normalized
  same-section return distance `8.62e-2`, minimum Poincare density `39.9`
  points per seed line, no target/sheath/recycling semantics, and no rejection
  reasons. The generated plot was visually checked. Decision: direct-coil
  closed controls move to `72%`; the remaining promotion blocker is no longer
  return-map stability but a closed/near-closed physics transient plus
  frame-by-frame movie QA if a closed direct-coil movie is claimed.
- 2026-06-21: Advanced the direct ESSOS-coil closed/near-closed control lane
  with a first-class refinement artifact. Added
  `create_essos_direct_coil_closed_control_refinement_package`,
  `build_essos_direct_coil_closed_control_refinement_campaign`,
  `build_essos_direct_coil_closed_control_refinement_diagnostics`, and the
  publication-style refinement plot. The default
  `direct_coil_closed_field_demo.py` now writes both the base return-map
  control and a self-contained refinement JSON/NPZ/PNG package. The gate checks
  stable closed/near-closed fractions, bounded p95 same-section return
  distance, Poincare sampling density, and explicit absence of target, sheath,
  recycling, and neutral semantics across larger seed/time samples. Focused
  tests prove both the passing self-contained gate and failure on open-like
  inconsistent levels. Decision: closed direct-coil controls move to `66%`,
  but live same-source refinement or longer-window evidence remains required
  before promoting any closed direct-coil movie or closed-field physics claim.
...
- 2026-06-21: Re-audited the single-plan structure, local branches, examples,
  ...
  `docs/research_grade_execution_plan.md` remains the authoritative plan.
  Updated the immediate order so `main` first targets ESSOS direct-coil
  open/closed-field simulations and diagnostic turbulence movies, then VMEC
  closed-field controls, then hybrid VMEC/coil open-SOL promotion. Updated the
  VMEC-extender decision with the current PR status: JAXDRB PR #2 is merged,
  `virtual_casing_jax` PR #2, ESSOS PR #31, and ESSOS PR #33 remain open,
  while `vmec_jax` PR #18 and PR #19 are merged. Decision: direct-coil movies
  may be generated on `main` as diagnostic artifacts, but README/docs
  promotion requires geometry, endpoint/source, refinement, and visual-QA
  gates; VMEC-extender finite-beta SOL remains artifact-import planning until
  stable upstream exports exist.
- 2026-06-21: Added the explicit VMEC closed-field control gate. The new
  campaign writes a self-contained dry-run contract by default and, when
  `RUN_LIVE_VMEC=True`, builds an ESSOS/VMEC `map_source="vmec"` FCI geometry
  and checks the closed-map invariants: zero endpoint masks, finite periodic
  map coordinates, non-axisymmetric \(|B|\) modulation, closed-step length,
  and zero response of `grad_parallel_fci`, `laplace_parallel_fci`, and
  conservative parallel diffusion on a constant field. The first live
  Landreman-Paul QA compact run passed on a `(4, 6, 16)` grid with
  endpoint fraction `0.0`, map-coordinate finite fraction `1.0`, \(|B|\)
  modulation `1.247`, mean closed-step length `1.149`, and all three
  constant-field residuals equal to `0.0` to reported precision. Decision:
  move the VMEC closed-field lane to `70%`; next add closed-field reduced
  transient/profile/spectrum/movie evidence without target, sheath,
  recycling, or neutral-loss semantics.
- 2026-06-21: Ran the live Landreman-Paul QA direct-coil closed/near-closed
  control using ESSOS-owned Biot-Savart tracing with 16 seed lines,
  `maxtime = 240`, and 1200 samples. The return-map gate passed with 639
  Poincare points, mean toroidal turns `10.11`, closed fraction `0.125`,
  near-closed fraction `0.875`, open-like fraction `0.0`, no-return fraction
  `0.0`, median normalized return distance `6.68e-2`, and p95 normalized
  return distance `8.45e-2`. The diagnostic figure is visually
  non-axisymmetric and the report keeps target, sheath, recycling, and neutral
  semantics disabled. Decision: move direct-coil closed controls to `60%`.
  A longer-window/refinement check is still required before any closed
  direct-coil movie claim; proceed next with the smoother VMEC closed-field
  tutorial/gate as the promoted closed-field path.
...
- 2026-06-19: Added the first cheaper field-split/Schur-style preconditioner
  probe, `field_block_sample` (`field_split`, `schur_field`). It samples one
  representative field-by-equation JVP block, applies that small inverse at
  every active cell, and exposes the bounded runtime control
  `recycling_jax_linear_preconditioner_max_field_block_fields`. Solver-level
  tests verify the sampled block inverse and a coupled-field Krylov gate:
  unpreconditioned stalls above `1e-3` residual with `10` operator calls, while
  `field_block_sample` converges below `1e-10` with `5` calls and one sampled
  block build. The first real hydrogen fixed-BDF2 sweep remains negative:
  `field_block_sample` preserved residuals but kept `115` operator calls, built
  `20` sampled blocks, and took `61.176 s` fixed-full-field and `57.058 s`
...
- 2026-06-19: Ran the same-case hydrogen fixed-BDF2 recycling gate with
  `field_block_feedback_diag`, `--fixed-bdf2-only`, `timestep=10`, `steps=2`,
  and preconditioner refresh `100`. The result is correctness-clean but
  performance-negative: residual `2.897e-6`, zero failed linear solves, and
  unchanged `115` linear iterations/operator calls on both fixed-BDF2 routes.
  The fixed-full-field route took `64.828 s`, built `20` feedback-aware blocks,
  and spent `6.000 s` building them; the active-array route took `59.833 s`,
  built `20` blocks, and spent `5.180 s` building them. Decision:
...
- 2026-06-19: Re-ran the same hydrogen fixed-BDF2 gate with
  `field_block_feedback_diag`, preconditioner refresh `100`, and the new
  `--fixed-bdf2-diagnose-linear-update-residual` metric enabled. Both
  full-field and active-array routes remained correctness-clean with zero
  failed linear solves, `115` linear iterations/operator calls, residual
  `2.897e-6`, max achieved linear-update residual `4.44e-8`, and max relative
  linear-update residual `3.08e-5`. The full-field route took `61.641 s`
  (`1.496 s` in `23` diagnostic update-residual actions), and the active-array
  route took `61.515 s` (`1.202 s` in `23` diagnostic actions). Decision:
...
- 2026-06-19: Ran and committed the `8x12x24 -> 16x24x48` imported-field DRB
  movie report-only candidate. The artifact is small (`40K`) and JSON-only.
  Time refinement passes again (`max_relative_metric_change=0.093`), both
  spectral-resolution gates now pass, and the earlier radial-flux and
  edge-band blockers are closed. The only remaining grid blocker is
  `spectral_centroid_poloidal_index` with relative change `0.52`. Decision:
...
- 2026-06-19: Ran the same high-poloidal `16x24x48 -> 16x48x48` candidate with
  raw `potential_iterations=6144` and then with `potential_preconditioner="jacobi"`
  at `3072` iterations. Raw iterations did not fix the residual blocker
  (`final_potential_residual_l2` remained the dominant grid/time offender), but
  Jacobi preconditioning closed the potential-residual and time-refinement
  blockers. Decision: raise effective preconditioning to `62%` and the 3D
...
- 2026-06-19: Added retained active-array batched residual/JVP campaigns to the
  research-campaign wrapper. The new local
  `dthe-active-array-batched-jvp-gate` routes the D/T/He residual through
  `build_fixed_array_rhs` with `--rhs-backend active_array`; the matching
...
- 2026-06-19: Added `solve_fixed_residual_linearized_update` as a narrow
  solver-facing primitive in `recycling_fixed_residual.py`. It builds one
  fixed-layout `jax.linearize` action, solves the Newton update with JAX GMRES,
  supports counted or jitted matrix-free operators, accepts an optional
  preconditioner, and reports linear-update residual health plus action
  diagnostics. Focused tests now verify an exactly solvable linear Newton
  update, the jitted-operator path, and RHS shape validation (`23`
...
- 2026-06-19: Re-ran the full D/T/He
  `dthe-active-array-output-jvp-profile` lane after the fixed-BDF2 gates. The
  first manual run stayed CPU-bound for more than six minutes with no artifact;
  the interrupted stack was in SciPy-BDF sparse-JVP Jacobian construction,
  inside repeated `jax.linearize` tracing of the active-array fixed-layout RHS.
  Added a small adapter hygiene patch that skips redundant
  `jnp.asarray(..., dtype=float64)` calls for already-float64 JAX arrays and
  tracers, then repeated the campaign with a `240 s` wrapper timeout. It still
  timed out before writing a profile summary. Decision: keep the output-window
  D/T/He sparse-JVP campaign timeout-gated and negative for promotion; the next
  performance step is eliminating repeated host-driven sparse-JVP Jacobian
  materialization or moving this route to the matrix-free fixed-BDF2/JAX-GMRES
  path, not further cosmetic adapter casts.
- 2026-06-19: Re-ran the smaller
  `dthe-active-array-linearized-update-throughput-probe` after the adapter
  cast cleanup. The active-array fixed-layout matrix-free update route still
  passes with solver status `0`, candidate nonlinear residual `2.11e-11`,
  `linear_update_residual_checked=false`, and update-check time about
...
- 2026-06-19: Added
  `--require-fixed-bdf2-pairwise-max` to
  `scripts/compare_recycling_transient_modes.py` and promoted
  `dthe-fixed-bdf2-active-array-physical-parity-gate`. The gate runs both
  stable `bdf` and `fixed_bdf2_active_array_jax_linearized` on the same
  `recycling_dthe_one_step`, `dt=1e-4`, eight-step window, then compares
  active-mesh evolved fields. The retained wrapper artifact passed the
  `2.5e-7` field-delta threshold with worst delta `1.745e-7` on `NVd+`; `Pd+`
  was `5.58e-8`, `Pe` was `1.36e-9`, and the remaining reported fields were
  near `1e-12` to `5e-13`. Fixed-BDF2 health stayed unchanged: eight
  JAX-GMRES solves, sixteen residual evaluations, zero failed or unconverged
  substeps, and maximum residual `4.10e-14`. Decision: this closes the
  immediate bounded physical-output parity gap for the D/T/He matrix-free
  route; default promotion still needs larger production-window parity plus
  same-fidelity CPU/GPU runtime evidence.
- 2026-06-19: Promoted
  `dthe-fixed-bdf2-active-array-parity-ramp-gate`, increasing the same D/T/He
  eight-step physical-output comparison from `dt=1e-4` to `dt=1e-3`. The
  retained wrapper artifact passed the `2.5e-5` active-mesh field-delta gate:
  worst `NVd+` delta was `1.761e-5`, followed by `Pd+ = 5.27e-6` and
  `Pe = 1.28e-7`. Fixed-BDF2 remained solver-clean with eight JAX-GMRES
  solves, sixteen residual evaluations, zero failed or unconverged substeps,
  maximum residual `4.05e-11`, `32.34 s` in linear solves, `10.91 s` in
  residual evaluations, and `44.93 s` fixed-BDF2 mode elapsed time. A scratch
  `dt=1e-2`, two-step probe also converged with residual `3.51e-8`, but the
  worst `NVd+` field delta increased to `1.55e-3`, so the next timestep decade
  remains an accuracy target rather than promoted evidence. Decision: the
  matrix-free D/T/He route now has a one-decade physical-parity ramp; do not
  promote production-window/default behavior until either larger-step field
  accuracy improves or a physics-observable tolerance replaces raw max-norm
  gating.
- 2026-06-19: Added active-mesh observable metrics to
  `scripts/compare_recycling_transient_modes.py`: fixed-BDF2 pairwise relative
  L2 and unweighted active-inventory relative deltas, JSON summaries, and
  explicit gates `--require-fixed-bdf2-pairwise-l2-rel-max` and
  `--require-fixed-bdf2-pairwise-inventory-rel-max`. Promoted
  `dthe-fixed-bdf2-active-array-scalar-observable-gate`, which reruns the
  `dt=1e-2`, two-step D/T/He BDF-vs-fixed-BDF2 comparison on scalar fields
  only (`Nd+`, `Pd+`, `Nd`, `Pd`, `Pe`). The retained artifact passes with
  worst scalar relative L2 error `3.969e-5` on `Pd+`, worst scalar active
  inventory relative error `4.721e-6` on `Pd+`, fixed-BDF2 residual
  `3.51e-8`, two JAX-GMRES solves, four residual evaluations, and zero failed
  or unconverged subsolves. Decision: this is production-timestep scalar
  density/pressure observable evidence, not default promotion, full-field
  parity, momentum parity, or speedup evidence. Momentum remains the next
  accuracy target because the all-field `dt=1e-2` probe still shows `NVd+` as
  the worst pointwise offender and relative `NVd` inventory is dominated by a
  near-zero reference denominator.
- 2026-06-19: Promoted
  `dthe-fixed-bdf2-active-array-substepped-full-field-gate`, the explicit
  `dt=1e-2` D/T/He full-field counterpart to the scalar observable gate. The
  gate forces `runtime:recycling_fixed_bdf2_max_internal_timestep=2.5e-3`, so
  each output window is advanced with four internal fixed-BDF2 substeps. The
  retained wrapper artifact passes the `1.25e-4` active-mesh full-field
  max-delta gate with worst `NVd+` delta `1.099e-4`; the next pointwise
  deltas are `Pd+ = 3.03e-5` and `Pe = 7.36e-7`. Solver health remains clean:
  eight internal substeps, eight JAX-GMRES solves, sixteen residual
  evaluations, zero failed or unconverged subsolves, maximum residual
  `6.17e-10`, `32.10 s` in linear solves, `10.70 s` in residual evaluations,
  and `44.43 s` fixed-BDF2 mode elapsed after warm compilation. Decision:
  substepping closes the `dt=1e-2` full-field/momentum accuracy gap, but the
  route remains too expensive for default promotion until preconditioning,
  compiled solve cost, and CPU/GPU scaling improve.
- 2026-06-19: Ran same-fidelity D/T/He substepped full-field preconditioner
  probes on the retained `dt=1e-2`, two-output-window fixed-BDF2 gate. The
  targeted `momentum_line` run preserved the same worst pointwise field delta
  (`NVd+ = 1.099e-4`) and maximum residual (`6.17e-10`), but slowed fixed-BDF2
  mode elapsed time from the unpreconditioned `44.43 s` to `140.69 s`,
  including `33.49 s` building eight line-block preconditioners and `96.09 s`
  in JAX-GMRES solves. The static `field_scale` run also preserved the same
  parity metrics but slowed to `86.37 s`, with zero dynamic build cost and
  `74.56 s` in JAX-GMRES solves. Also fixed
  `scripts/compare_recycling_transient_modes.py` so required-preconditioner
  gates distinguish dynamic JVP-derived preconditioners, which must report
  positive build counts, from static `state_scale`/`field_scale`
  preconditioners, which validly report zero dynamic builds. Decision: do not
  promote the current D/T/He preconditioners; the next recycling performance
  push should target cheaper residual/JVP kernels, compiled-size reduction, or
  a stronger block/transport approximation that demonstrably reduces Krylov
  work on the same full-field gate.
- 2026-06-19: Ran the same D/T/He substepped full-field gate with
  `runtime:recycling_jax_linear_jit_residual=true`. The run preserved the
  same `NVd+ = 1.099e-4` worst field delta and `6.17e-10` residual, but slowed
  to `152.32 s`; residual-evaluation time increased from the retained
  unjitted `10.70 s` to `61.52 s`, and linear-solve time increased to
  `88.34 s`. Decision: do not promote residual JIT on the D/T/He output-window
...
- 2026-06-19: Promoted the second pointwise recycling physics term into the
  ...
  parity gate.
- 2026-06-19: Promoted the first stencil-aware recycling physics term into
  the active-array residual seam.
  `fixed_layout_neutral_parallel_diffusion_field_rhs_from_active_fields`
  reconstructs fixed guard-cell templates from active fields inside the JAX
  kernel, evaluates the open-field neutral parallel diffusion operator, and
  returns active-domain density, pressure, and momentum RHS blocks for
  `build_fixed_array_rhs`. Validation compares the helper against
  `apply_neutral_parallel_diffusion` on the committed 1D recycling fixture
  active slice using the production runtime-model layout, then proves the same
  field-RHS helper is JVP-transformable through the fixed-array adapter.
  Focused tests passed:
  `PYTHONPATH=src pytest -q tests/test_native_recycling_neutral_diffusion.py`
  (`10 passed`) and the promoted source slice across neutral diffusion,
  D/T/He reactions, collision closure, and fixed residuals (`19 passed, 56
  deselected`). Decision: this closes the neutral-diffusion stencil seam, but
  default full-solver promotion still needs sheath/target source ports and a
  composed active-array RHS that no longer calls the full-field oracle.
- 2026-06-19: Promoted target-recycling source mapping into the fixed-layout
  active residual seam. `fixed_layout_target_recycling_field_rhs` consumes the
  same prepared species and sheath-adjusted ion boundary velocity used by the
  full recycling RHS, then maps target neutral density and energy sources into
  active `N` and `(2/3)Q -> P` field-RHS blocks. Validation compares the mapper
  against `target_recycling_sources` on the committed 1D recycling fixture
  active slice, keeps the existing JAX transformability check for the source
  kernel, and proves the mapper can run through `build_fixed_array_rhs` with
  a JVP that reconstructs the same sheath-prepared state. Focused validation
  passed:
  `PYTHONPATH=src pytest -q tests/test_native_recycling_targets.py`
  (`8 passed`) and
  `python -m ruff check src/jax_drb/native/recycling_targets.py tests/test_native_recycling_targets.py`.
  Decision: target source mapping is now active-layout ready, while full
  sheath/no-flow orchestration is still the next boundary migration before the
  composed active-array residual can replace the full-field oracle.
- 2026-06-19: Added the first composed active-source RHS seam in
  `src/jax_drb/native/recycling_active_sources.py`.
  `fixed_layout_recycling_source_field_rhs_from_active_fields` sums promoted
  D/T/He reaction, pointwise collision, neutral parallel diffusion, and
  sheath-prepared target-recycling source maps into one fixed-layout field-RHS
  dictionary without claiming the complete residual is active-native. New
  tests compare the composed D/T/He reaction+collision+neutral map against the
  individual promoted helpers, prove the composed source map is
  JVP-transformable through `build_fixed_array_rhs`, and compare hydrogen
  neutral+target composition against the corresponding individual helpers.
  Focused validation passed:
  `PYTHONPATH=src pytest -q tests/test_native_recycling_active_sources.py`
  (`3 passed`) and
  `python -m ruff check src/jax_drb/native/recycling_active_sources.py tests/test_native_recycling_active_sources.py`.
  Decision: use this composition seam as the next replacement point inside
  `_build_active_array_recycling_rhs`; remaining ports are full sheath/no-flow
  orchestration, open-field transport assembly, feedback, conduction,
  viscosity, and final parity/runtime gates before default promotion.
- 2026-06-19: Added `assemble_fixed_layout_recycling_field_rhs_from_sources`
  as the source-to-total-RHS insertion seam for the active recycling path. The
  helper converts active field-source blocks back into the internal density,
  energy, and momentum source arrays used by the existing Braginskii RHS
  assemblers, including the inverse pressure-source mapping
  `Q = (3/2) P_rhs`, applies the electron force-balance momentum update, and
  returns active total RHS blocks. New tests prove that neutral density,
  pressure, and momentum source blocks are inserted exactly once by comparing
  sourced and unsourced fixture RHS outputs, and prove JVP transformability of
  the source-insertion path. Focused validation passed:
  `PYTHONPATH=src pytest -q tests/test_native_recycling_active_sources.py`
  (`5 passed`), plus the broader promoted-source slice (`27 passed, 61
  deselected`) and
  `python -m ruff check src/jax_drb/native/recycling_active_sources.py tests/test_native_recycling_active_sources.py`.
  Decision: this closes the source-to-total-RHS insertion gap; the next
  implementation should feed the composed active source map through this
...
- 2026-06-19: Promoted scalar upstream-density feedback into the opt-in active
  source residual backend. The `promoted_active_sources` backend now calls the
  same feedback source helper as the full RHS, maps feedback density sources
  and kinetic-energy sources into active field-RHS blocks, preserves the
  explicit trapezoid predictor path when feedback integrals are advanced
  outside the implicit state, and returns controller RHS values when feedback
  integrals are part of the fixed PyTree state. New D/T/He tests compare the
  feedback-enabled promoted RHS against the full-field RHS, compare the
  explicit feedback-predictor path against the full-field RHS, and prove BE
  residual JVP transformability with feedback integrals in the implicit state.
  Focused validation passed:
  `PYTHONPATH=src pytest -q tests/test_native_recycling_active_sources.py`
  (`11 passed`), the broader promoted recycling slice
  (`tests/test_native_recycling_active_sources.py`,
  `tests/test_recycling_jvp_promotion_gate.py`, and
  `tests/test_profile_recycling_jax_linearized_gate.py`, `39 passed`), and
  `PYTHONPATH=src ruff check tests/test_native_recycling_active_sources.py src/jax_drb/native/recycling_active_sources.py`.
  Decision: scalar feedback is no longer a blocker for the promoted
  active-source residual; next P2 work should target remaining transport
  closures, especially conduction/viscosity and no-flow/sheath orchestration
  details that still require full-field preparation.
- 2026-06-19: Promoted the remaining local collision-transport closure blocks
  into the opt-in active-source residual. `fixed_layout_collision_transport_field_rhs_from_prepared`
  now maps the gradient collision terms from the full closure--electron/ion
  thermal forces, ion-ion thermal forces, parallel ion viscosity, and parallel
  thermal conduction--into active pressure and momentum RHS blocks without
  double-counting the already promoted pointwise friction and heat-exchange
  terms. The promoted backend now adds this transport field-RHS map, and a new
  full D/T/He fixture test compares the resulting `promoted_active_sources`
  RHS against the full-field RHS with sheath, recycling, feedback, D/T/He
  reactions, neutral diffusion, thermal force, ion viscosity, and conduction
  active. Focused validation passed:
  `PYTHONPATH=src pytest -q tests/test_native_recycling_active_sources.py`
  (`14 passed`), the broader promoted recycling slice
  (`tests/test_native_recycling_active_sources.py`,
  `tests/test_recycling_jvp_promotion_gate.py`, and
  `tests/test_profile_recycling_jax_linearized_gate.py`, `42 passed`), and
  `PYTHONPATH=src ruff check src/jax_drb/native/recycling_collision_closure.py tests/test_native_recycling_active_sources.py src/jax_drb/native/recycling_active_sources.py`.
  Decision: the opt-in active-source RHS now matches the full D/T/He fixture
  on the active slice for the local source/closure surface; next P2/P8 work
  should rerun production-window parity and profiling on this backend, then
  reduce compile/runtime cost before considering any default-promotion path.
- 2026-06-19: Exposed the opt-in promoted active-source backend as a
  production-window fixed-BDF2 mode,
  `fixed_bdf2_promoted_active_sources_jax_linearized`. The mode routes through
  the `promoted_active_sources` residual backend, reports
  `fixed_bdf2_promoted_active_source_rhs_steps`, and is selectable from
  `scripts/compare_recycling_transient_modes.py` without changing stable
  defaults. Focused validation passed:
...
- 2026-06-19: Extended `scripts/profile_recycling_jax_linearized_gate.py` with
  ...
  ad hoc script edits.
- 2026-06-19: Added `dthe-promoted-active-sources-profile-gate` to
  `scripts/run_research_campaign_bundle.py`, `all-local`, and the manual
  research-campaign workflow option list. The gate runs
  `profile_recycling_jax_linearized_gate.py --rhs-backend promoted_active_sources`
  with `mesh:ny=100`, cProfile, and RSS sampling. Focused command tests passed
  (`46 passed`), Python ruff checks passed on the touched campaign scripts and
  tests, and the workflow YAML parsed with the new campaign option present.
  The local campaign passed against `tests/fixtures/reference-root`:
  `PYTHONPATH=src python scripts/run_research_campaign_bundle.py --campaign dthe-promoted-active-sources-profile-gate --reference-root tests/fixtures/reference-root --output-root /tmp/jax_drb_promoted_campaign --timeout-seconds 300 --fast-timeout-seconds 300`.
  The resulting profile reported active size `1900`, state size `1979`,
  median profiled run time `0.925537 s`, residual `3.82301471e-10`,
  `rhs_backend=promoted_active_sources`, sampled peak RSS delta `0.4375 MiB`,
  and cProfile top cost in the backward-Euler step, JAX-linearized Newton
  system, fixed residual, and promoted RHS. Decision: promoted source-kernel
  profiling is now reproducible through the campaign layer; next performance
  work should increase the physics load and compare CPU/GPU on the same
  promoted backend.
- 2026-06-19: Strengthened the promoted active-source research-campaign gate
  from a near-trivial `dt=1e-6` residual check to a bounded nontrivial
  `dt=1e-4` Newton/JVP profile. The campaign now requires
  `--initial-residual-mode linearize`, at least one nonlinear iteration, at
  least one linear solve, at least one matrix-free operator call, and residual
  closure below `1e-6`. Local matched runs against
  `tests/fixtures/reference-root` passed for `promoted_active_sources`,
  `active_array`, and `fixed_full_field_array` on `mesh:ny=100`: all reached
  residual `1.7409e-12` with one nonlinear update, one JAX-GMRES solve, five
  operator calls, two residual evaluations, and one line-search trial. The
  promoted active-source profile took `7.707 s` with sampled RSS delta
  `907 MiB`; the active-array and fixed-full-field profiles took `7.808 s` and
  `7.803 s` with sampled RSS deltas `841 MiB` and `837 MiB`. A deliberately
  aggressive `dt=1.0`, one-update probe failed on all three backends with
  residuals about `2.73e2`, proving that the current blocker is the shared
  Newton/Krylov robustness envelope rather than a promoted-source-only
  correctness error. Additional `ny=200` and `ny=400` promoted-source sweeps
  passed as residual-check size/RSS sanity tests but performed zero nonlinear
  or linear solves, so they are not counted as reviewer-grade performance
  evidence. Decision: the promoted source backend now has nontrivial local
  Newton/JVP profiling evidence; next work should improve shared nonlinear
  robustness, cheaper residual/JVP kernels, and same-fidelity CPU/GPU scaling
  before default promotion.
- 2026-06-19: Reran the strengthened
  `dthe-promoted-active-sources-profile-gate` through the campaign wrapper with
  cProfile and RSS sampling enabled:
  `PYTHONPATH=src python scripts/run_research_campaign_bundle.py --campaign dthe-promoted-active-sources-profile-gate --reference-root tests/fixtures/reference-root --output-root /tmp/jax_drb_promoted_campaign_strong --timeout-seconds 300 --fast-timeout-seconds 300`.
  The campaign passed in `26.8 s`, wrote
  `runtime_profile_artifacts/recycling_dthe_promoted_active_sources_profile_cpu/profile_summary.json`
  and `cprofile_top.txt`, and the profiled solve reported residual
  `1.7409e-12`, one nonlinear iteration, five linear iterations, one linear
  solve, five matrix-free operator calls, two residual evaluations,
  `14.597 s` profiled runtime, and sampled RSS delta `909 MiB`. Focused
  validation also passed:
  `PYTHONPATH=src pytest -q tests/test_research_campaign_bundle.py tests/test_profile_recycling_jax_linearized_gate.py`
  (`46 passed`) and
...
- 2026-06-20: Added cached field-name membership metadata to
  ...
  materially.
- 2026-06-20: Refreshed the promoted active-source D/T/He cProfile/RSS gate and
  found that the campaign was measuring mostly cold JAX/XLA compilation: the
  old `ny=100`, `dt=1e-4` profile spent `22.45 s` in backend compile inside a
  `35.04 s` profiled solve and did not require the jitted linear operator used
  by the stricter fixed-BDF2 gate. Updated
  `dthe-promoted-active-sources-profile-gate` to perform one warmup run and
  require `--jit-linear-operator`. The refreshed local campaign passed with
  residual `1.74087479e-12`, one nonlinear update, one JAX-GMRES solve, five
  operator calls, a `13.06 s` warmup, an `8.29 s` profiled solve, sampled RSS
  run time `8.00 s`, and peak RSS delta `361.52 MiB`. The warm cost split is
  now actionable: `5.08 s` in GMRES/JVP solve, `2.80 s` in residual evaluation
  and linearization, and `0.31 s` in line search. Decision: use the warm
...
- 2026-06-20: Localized the remaining D/T/He one-step `Pd+` target-cell
  difference with pressure and collision-energy diagnostics instead of trying
  more preconditioner variants. The boundary-cell diagnostic now prints ion
  pressure RHS pieces (`parallel_divergence`, `parallel_advection`,
  `energy_source`, and `pressure_total`) and selected-ion collision energy
  subterms. The `Pd+` local RHS difference at `(0, 51, 0)` is dominated by
  collision-energy sensitivity, specifically `Ed+_conduction` (`2.64e-4`)
  with a smaller `Ed+_viscosity` contribution (`1.92e-5`); reactions, target
  recycling, sheath energy, neutral diffusion, and feedback are near zero or
  subdominant at that cell. The state-level discrepancy is timestep-sensitive:
  the live Hermès target-history rerun gives `Pd+ = 1.02e-3` at `dt=25`,
  `1.29e-4` at `dt=5`, and `6.85e-6` at `dt=1`, while `NVd+` drops from
  `3.78e-4` at `dt=25` to `7.17e-8` at `dt=1`. Decision: the remaining
  `Pd+` offender is not evidence for a missing pressure closure; it is a
  large-timestep conduction/state-evolution sensitivity that should be tracked
  with timestep-refinement gates and longer-window validation. Added collision
  energy diagnostics (`*_coll_heat_exchange`, `E*_viscosity`,
  `E*_conduction`) and a regression test that verifies the diagnostic energy
  components reconstruct the collision energy source.
- 2026-06-20: Rechecked the hydrogen `recycling_1d_one_step` target-history
  diagnostic after closing the D/T/He sheath-velocity parity bug. The old
  diagnostic default used `continuation`, which produced misleading target-cell
  state differences at `dt=25` (`Nd = 6.77e-3`, `Pd+ = 5.28e-3`,
  `NVd+ = 1.56e-3`). Running the same live Hermès comparison with the stable
  BDF path gives `Pd+ = 1.62e-4`, `NVd+ = 9.37e-6`, `Nd+ = 2.34e-6`, and
  `Pe = 1.03e-6`. The target-history diagnostic now defaults both hydrogen
  and D/T/He recycling cases to `bdf`, while retaining `--solver-mode` for
  explicit continuation or JAX-native probes. Decision: hydrogen one-step
...
- 2026-06-20: Performed the README, examples, release-packaging, and release
  notes claim-boundary pass for the 1.0.3 closeout. The README now describes
...
- 2026-06-20: Corrected the release target metadata after verifying that
  `v1.0.2` is already a published GitHub release at commit `81cd25e`, while
  current `main` is `55ccdfc` plus these final metadata updates. The package
  version, runtime `__version__`, MkDocs navigation, README release-note link,
  public release-surface test inventory, and current release notes now target
  `1.0.3`. The historical 1.0.2 release notes were restored to match the
  already published tag, and the new 1.0.3 notes record the current closeout
  evidence, artifact restore, claim boundaries, and remaining manual hosted-CI,
  GPU, and live-reference caveats. Added `CITATION.cff` for manuscript and
  software-citation workflows. Decision: docs/examples move to `98%` and
  release closeout to `96%`; do not move the existing `v1.0.2` tag. The next
  package release should use `v1.0.3` after final local gates and, when
  available, hosted CI.
- 2026-06-20: Reran the local release gates after retargeting metadata to
  ...
  `mkdocs build --strict --clean` passes locally, targeted release-surface and
  artifact tests pass (`22` tests), metadata consistency confirms
  `pyproject.toml`, `jax_drb.__version__`, MkDocs navigation, README, release
  notes, and `CITATION.cff` agree on `1.0.3`, and the package build produces
  `jax_drb-1.0.3-py3-none-any.whl` (`709 KiB`) plus
  `jax_drb-1.0.3.tar.gz` (`614 KiB`). The footprint audit still reports no
  large tracked files, no large current-tree blobs, no large reachable-history
  blobs, and git `size-pack = 6.43 MiB`. Decision: local release evidence is
...
- 2026-06-21: Added the post-release open-lane completion plan as the single
  authoritative sequence for ESSOS direct-coil open-field simulations, VMEC
  closed-field controls, hybrid VMEC/coil open-SOL simulations, VMEC-extender
  finite-beta exterior-field integration, HSX/NCSX/Landreman-Paul QA/Dommaschk
  device expansion, full DRB physics promotion, JAX-native recycling default
  promotion, differentiability, and performance/scaling evidence. GitHub PR
  audit found no open PR against `uwplasma/jax_drb`; the historical
  `origin/feature/vmec-extender-edge-fields` branch already maps to the
  importer/contract work present on `main`. The active upstream dependencies
  are `uwplasma/virtual_casing_jax#2` for the finite-beta exterior-field core
  and `uwplasma/ESSOS#31` for VMEC-extender trace/grid workflows; both are
  useful but do not by themselves promote a finite-beta SOL result because
  wall-limited connection lengths, matched free-boundary coil validation, and
  gallery-grade plots remain open. Decision: on `main`, proceed first with the
  ESSOS direct-coil open-field lane and VMEC closed-field control lane, promote
...
- 2026-06-21: Ran the direct-coil live gates against the local
  Landreman-Paul QA ESSOS assets. The pure-coil `adjacent_step_length`
  three-level gate gives small finest errors (normalized RMS `7.63e-3`,
  \(L_\infty\) `1.44e-2`) and monotone reduction, but observed order is only
  `0.104`; increasing `times_to_trace` from `160` to `320` leaves the result
  unchanged (`0.103`), so the blocker is not trace sampling. The pure-coil
  `target_exit_length` gate is more physical for open-SOL target distance but
  is rougher: finest RMS `0.228`, \(L_\infty\) `2.07`, observed order `0.659`,
  and nonmonotone \(L_\infty\). The live FCI endpoint/source gate closes
  particle recycling, current balance, neutral particle balance, and neutral
  momentum balance to near roundoff and has exact target-label/consumed-map
  consistency, but fails promotion because connection-resolution roughness is
  underresolved (`p95` face jump `1.83` versus `0.5` threshold). Decision:
  direct-coil source accounting is useful evidence, but the next technical
  step is improving direct-coil map/refinement/connection-resolution
  construction before any direct-coil movie promotion. The workflow now runs
...
- 2026-06-21: Performed another plan-only consolidation pass for the remaining
  open research lanes. The historical `1.0.3` release backlog is now labeled
  historical so it does not compete with the active post-release implementation
  plan. The active plan now gives file-level next changes and promotion gates
  for direct ESSOS-coil open-field simulations on `main`, direct-coil
  closed/near-closed controls, VMEC closed-field transient/profile/spectrum
  diagnostics, hybrid VMEC/coil open-SOL promotion, VMEC-extender finite-beta
  artifact import, device expansion, full DRB physics promotion, JAX-native
  recycling/performance, and documentation/coverage/footprint hygiene. The
  VMEC-extender PR audit was rechecked with `gh`: `uwplasma/jax_drb#2` is
  merged, `uwplasma/virtual_casing_jax#2`, `uwplasma/ESSOS#31`, and
  `uwplasma/ESSOS#33` remain open upstream artifact/export enablers, and the
  current `vmec_jax` PRs are useful ecosystem context but not direct JAXDRB
  SOL dependencies. Decision: continue implementation from this file only;
  do not create another roadmap, do not block `main` direct-coil work on live
  upstream VMEC-extender packages, and require every promoted movie or figure
  to pass geometry, operator, physics, refinement, and visual-QA gates.
- 2026-06-21: Integrated the VMEC closed-field reduced transient lane. The new
  `essos_vmec_closed_field_transient_campaign.py` exports a closed-map density
  transient with periodic FCI coupling, conservative perpendicular diffusion,
  logical \(E\times B\) bracket advection, zero-volume-mean drive, profile and
  spectrum diagnostics, a PNG plot writer, and a fixed-camera GIF writer. The
  public validation API now exports the transient package builders and plot/
  movie helpers, and `vmec_closed_field_demo.py` writes both the closed-map
  FCI dry-run contract and the transient dry-run contract by default. Focused
  tests pass for the transient dry-run contract, package artifacts, mass and
  spectrum controls, no target/sheath/recycling/neutral-loss flags, and
  endpoint-map rejection. Documentation and README/example tables now explain
  the equation, artifacts, commands, and closed-field claim boundary. Decision:
  move the VMEC closed-field lane to `78%` and the overall 3D stellarator
  open/closed SOL program to `60%`; the next evidence step is a live
  Landreman-Paul QA transient/movie run and frame-by-frame visual QA before
  any README media promotion.
- 2026-06-21: Ran the live Landreman-Paul QA VMEC closed-field transient in
  `/tmp` using local ESSOS inputs. The compact `(4, 6, 16)` run passed with
  zero endpoints, mass drift `6.8e-16`, final RMS `3.46e-2`, positive density,
  and finite spectrum. The default `(5, 8, 20)` run also passed with zero
  endpoints, mass drift `2.1e-15`, final RMS `3.38e-2`, positive density, and
  finite spectrum. PNG QA is readable and shows the non-axisymmetric closed
  section, profile controls, mass conservation, and spectrum. GIF contact
  sheets show no camera jitter, but the visible motion is weak even under a
  stronger reduced-drive trial. Decision: treat this as VMEC closed-field
  control evidence, not promoted turbulence media; move the VMEC closed-field
  lane to `82%` and the overall 3D stellarator open/closed SOL lane to `61%`.
  The next promoted visual target should be hybrid open-SOL or a stronger
  full-DRB closed-field model with real bracket/vorticity dynamics.
- 2026-06-22: Performed the requested plan-only consolidation for the remaining
  open lanes, including direct ESSOS-coil open and closed field-line
  simulations, hybrid VMEC/coil open-SOL promotion, and VMEC-extender
  finite-beta artifact intake. Rechecked current PR status with `gh`: the
  JAXDRB VMEC-extender PR is merged, `virtual_casing_jax#2`, `ESSOS#31`, and
  `ESSOS#33` remain upstream artifact/export enablers, and `vmec_jax#18/#19`
  are merged context rather than direct SOL blockers. Decision: keep `main`
  focused on self-contained ESSOS direct-coil artifacts and movies first,
  promote hybrid as the first open-SOL movie if pure-coil endpoint refinement
  remains rough, keep VMEC-extender finite-beta SOL behind a frozen-artifact
  checklist, and treat the lane board in this section as the single
  authoritative plan.
