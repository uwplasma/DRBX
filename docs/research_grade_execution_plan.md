...
| Reference-backed parity | Many RHS, one-step, and campaign gates exist; neutral-mixed direct `SNVh_*` source formulas are now closed. The accepted-step reference-monitor path now auto-builds a clean patched checkout, links the patched `hermes-3`, runs the neutral-mixed reference case, writes JSONL, and passes the schema gate. The June 5, 2026 local rerun with optional `Dnnh`, `Vh`, and `eta_h` diagnostics produced `148` accepted-step records. Native traces replay the reference adaptive accepted-step time grid, and the current comparison matches `148/148` accepted points. With timestamp mismatch removed, the highest matched-time input drift is now `Dnnh` (`4.46e-3` target/guard maximum), followed by `eta_h` (`3.23e-3` target/guard maximum) and `SNVh_parallel_viscosity` (`5.35e-5` active/target maximum). The comparator emits `parallel_viscosity_input_register`, and the final-state input-closure diagnostic verifies that native `Dnn`, `Vh`, and `eta_h` reconstructed from a reference dump match the reference final-state fields to roundoff in active, target-adjacent, and guard cells. The register now ranks `Dnn`/`V`/`eta` closure inputs separately from `Nh`, `Ph`, and `NVh` state inputs; on the real `148/148` matched trace, `Nh` is the dominant state input, but `Dnnh` is the dominant closure-input drift and `eta_h` target drift is about `99x` larger than the largest state-input drift. Native accepted-step traces now also emit the `Dnnh` preparation ladder (`Tnlimh`, `logPnlimh`, `grad_logPnlimh`, raw diffusion, flux-limit maximum, flux-limited diffusion, and diffusion-limited pre-boundary diffusion). The remaining `NVh` mismatch is therefore localized to accepted-step `Dnn` diffusion-coefficient preparation or target-boundary sequencing before the viscosity and parallel-viscosity stencil, not to the final-state neutral input-closure formulas. Target-adjacent accepted-step state/history drift and longer recycling windows remain the main scientific offenders. | 90% | Extend the reference accepted-step monitor with the same `Dnnh` preparation ladder, rerun the accepted-step comparator, then patch the identified `Dnn` preparation or boundary-sequencing stage. |
