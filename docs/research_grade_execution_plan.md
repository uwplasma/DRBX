...
| Reference-backed parity | Many RHS, one-step, and campaign gates exist; neutral-mixed direct `SNVh_*` source formulas are now closed. The accepted-step reference-monitor path auto-builds a clean patched checkout, links the patched `hermes-3`, runs the neutral-mixed reference case, writes JSONL, and passes the schema gate. The June 8, 2026 local rerun with optional `Dnnh`, `Vh`, `eta_h`, guard payload diagnostics, and CVODE solver order produced `148` accepted-step records. Native traces replay the reference adaptive accepted-step time grid, and the current comparison matches `148/148` accepted points. The contextual reference-monitor patch now deep-copies the `Dnnh` preparation ladder (`Tnlimh`, `logPnlimh`, `grad_logPnlimh`, raw diffusion, flux-limit maximum, flux-limited diffusion, and diffusion-limited pre-boundary diffusion), emits pointwise target-adjacent and guard payloads, and a live rerun with those fields produced no missing ladder diagnostics. The native `grad_logPnlim*` norm now carries the structured metric terms (`g11`, `g22`, `g33`, and supported `g23`) used by the reference, removing metric-norm semantics as a formula-level blocker. The new `neutral_diffusion_ladder_register` ranks `Dnnh_flux_max` as the leading real pointwise target-cell offender: native `2.74471293` versus reference `2.73944` at local target index `[0, 3, 0]`, a `5.27e-3` drift. Final boundary-applied `Dnnh` follows at `4.46e-3`, while raw diffusion is much smaller (`6.07e-4`). The guard payload shows large pre-boundary guard deltas in ladder diagnostics but final `Dnnh` guard drift equals its target drift; a direct algebraic check closes the cap formula at the worst cell. The new solver-order trace reports `147` comparable points, `137` native/reference order mismatches, and the worst `Dnnh_flux_max` point at `t = 2.9901189387441356` compares native BDF2 against reference order `5` with identical `dt`. The remaining production mismatch is therefore localized to variable-order accepted-step state/history replay feeding near-target `Grad(logPnlim)` and the flux-limit cap rather than final diffusion guard application or a local closure formula. The `parallel_viscosity_input_register` still shows `Dnnh` as the dominant closure input drift, `eta_h` target drift about `49x` larger than the largest pointwise state-input drift (`99x` by the legacy zone metric), and `SNVh_parallel_viscosity` at `1.29e-4` pointwise (`5.35e-5` by the legacy zone metric). The final-state input-closure diagnostic verifies that native `Dnn`, `Vh`, and `eta_h` reconstructed from a reference dump match the reference final-state fields to roundoff. | 97% | Either implement variable-order accepted-step replay for the neutral history gate or add a controlled max-order-2 reference campaign; then rerun the pointwise accepted-step comparator and longer recycling windows. |
| JAX-native recycling solver | Fixed-layout residual, JVP checks, and fixed-BDF2 diagnostics are present and gated on lightweight fixtures. The fixed residual contract now rejects packed-state rank/length mismatches, layout field-size inconsistencies, unknown active-array RHS keys, wrong RHS field shapes, wrong feedback shapes, and field/feedback shape mismatches before they can enter traced residuals. The opt-in `bdf_active_array_jvp` full-output bridge routes SciPy BDF through `rhs_backend="active_array"` and grouped JVPs, while `bdf_fixed_full_field_jvp` remains the compatibility bridge. A new opt-in `fixed_bdf2_active_array_jax_linearized` path routes the non-SciPy fixed-BDF2 output-window lane through `build_fixed_array_rhs`; focused tests and a fixture command verify the active-array backend and JAX-linearized diagnostics. Full output-window default promotion is not yet complete because the full `timestep = 5000` fixture still exposes a large fixed-BDF2 nonlinear residual in both fixed-full-field and active-array routes. | 64% | Fix fixed-BDF2 timestep/nonlinear convergence on the full output-window fixture, then rerun parity, cProfile/RSS, and JAX trace evidence showing a runtime win rather than only transformability. |
...
| 3D imported-field and VMEC-extender SOL | Import, selected-field, reduced FCI, and smoke campaign gates exist. The VMEC-extender synthetic import example now runs from a clean temporary directory, and the analytic FCI SOL lane remains the strongest self-contained 3D evidence. The imported-FCI example now has CLI routing for coil, VMEC, and hybrid map sources, including dry-run coverage. Broad imported-field turbulence claims still require independent connection-length validation, consumed-map/source-fieldline agreement, and imported-field grid/time refinement. | 55% | Add imported connection-length and refinement campaigns, then regenerate smooth non-axisymmetric movies only from gates that pass those checks. |
...
| Documentation and examples | ReadTheDocs and equation/code/validation docs exist; neutral-mixed docs now describe pointwise target and guard accepted-step payloads, pre-boundary ladder scopes, and the current flux-cap offender. Examples need to stay synchronized with every promoted artifact and geometry lane. | 78% | One clean-clone tokamak and one clean-clone stellarator tutorial that regenerate the advertised plots/movies without private reference checkouts. |
