...
| Reference-backed parity | Many RHS, one-step, and campaign gates exist; neutral-mixed direct `SNVh_*` source formulas are now closed. The accepted-step reference-monitor path auto-builds a clean patched checkout, links the patched `hermes-3`, runs the neutral-mixed reference case, writes JSONL, and passes the schema gate. The June 8, 2026 local rerun with optional `Dnnh`, `Vh`, `eta_h`, guard payload diagnostics, and CVODE solver order produced `148` accepted-step records. Native traces replay the reference adaptive accepted-step time grid, and the current comparison matches `148/148` accepted points. The contextual reference-monitor patch now deep-copies the `Dnnh` preparation ladder (`Tnlimh`, `logPnlimh`, `grad_logPnlimh`, raw diffusion, flux-limit maximum, flux-limited diffusion, and diffusion-limited pre-boundary diffusion), emits pointwise target-adjacent and guard payloads, and a live rerun with those fields produced no missing ladder diagnostics. A controlled June 8, 2026 max-order-2 reference campaign now matches `309/309` accepted points with zero solver-order mismatches, observed reference max order `2`, and no configured order-ceiling violations. The leading offender remains `Dnnh_flux_max`, with target-adjacent drift about `5.13e-3`, followed by final `Dnnh` and the diffusion-limited/flux-limited ladder. This closes CVODE variable order as the primary blocker and localizes the remaining neutral NVh parity work to accepted-step state/history sequencing, near-target `Grad(logPnlim)` reconstruction, and flux-limit-cap preparation rather than the pressure-gradient, viscosity, or local cap algebra. The final-state input-closure diagnostic verifies that native `Dnn`, `Vh`, and `eta_h` reconstructed from a reference dump match the reference final-state fields to roundoff. The parity report now also records state-input and limiter-input amplification for each neutral diffusion ladder, so the next rerun can distinguish state/history drift from a pure `Grad(logPnlim)` stencil mismatch. | 98% | Rerun the max-order-2 accepted-step trace with the state-to-limiter amplification register, then patch either accepted-state sequencing or the near-target stencil based on that evidence. |
