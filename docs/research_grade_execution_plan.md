...
| Reference-backed parity | Many RHS, one-step, and campaign gates exist; neutral-mixed direct `SNVh_*` source formulas are now closed. The accepted-step reference-monitor path now auto-builds a clean patched checkout, links the patched `hermes-3`, runs the neutral-mixed reference case, writes JSONL, and passes the schema gate. The June 5, 2026 local rerun produced `148` accepted-step records with the required 10 state/RHS/source fields plus optional `Vh` and `eta_h` input diagnostics, and the monitor patch now also emits optional `Dnnh` diffusion diagnostics for the next rerun. Native traces replay the reference adaptive accepted-step time grid, and the current comparison matches `148/148` accepted points. With timestamp mismatch removed, the highest matched-time input drift is `eta_h` (`3.23e-3` target/guard maximum), followed by `SNVh_parallel_viscosity` (`5.35e-5` active/target maximum). The comparator emits `parallel_viscosity_input_register`, and the new final-state input-closure diagnostic verifies that native `Dnn`, `Vh`, and `eta_h` reconstructed from a reference dump match the reference final-state fields to roundoff in active, target-adjacent, and guard cells. The register now also ranks `Dnn`, `Nh`, `Ph`, and `NVh` inputs; on the real `148/148` matched trace, `Nh` is the dominant state input but `eta_h` target drift is about `99x` larger than the largest state-input drift. The remaining `NVh` mismatch is therefore localized to accepted-step closure/boundary amplification before the parallel-viscosity stencil, not to the neutral input-closure formulas. Target-adjacent accepted-step state/history drift and longer recycling windows remain the main scientific offenders. | 88% | Rerun the accepted-step trace with optional `Dnnh`, then fix the neutral-viscosity target-boundary or closure-stage amplification path. |
