...
| 3D imported-field and VMEC-extender SOL | Import, selected-field, reduced FCI, and smoke campaign gates exist. The VMEC-extender synthetic import example now runs from a clean temporary directory, and the analytic FCI SOL lane remains the strongest self-contained 3D evidence. The imported-FCI example now has CLI routing for coil, VMEC, and hybrid map sources, including dry-run coverage, plus advisory single-grid connection-length roughness diagnostics in the imported FCI report. Broad imported-field turbulence claims still require independent connection-length validation, consumed-map/source-fieldline agreement, and imported-field grid/time refinement. | 56% | Add true multi-grid imported connection-length and refinement campaigns, then regenerate smooth non-axisymmetric movies only from gates that pass those checks. |
