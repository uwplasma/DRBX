...
| Documentation and examples | ReadTheDocs and equation/code/validation docs exist; neutral-mixed docs now describe pointwise target and guard accepted-step payloads, pre-boundary ladder scopes, max-order-2 reference control, and the current flux-cap offender. A new SIMSOPT-style `examples/model_selection_guide.py` teaches users how to choose diffusion, drift-reduced Braginskii, 2D turbulence, and 3D FCI lanes while writing parse-checked starter decks. The imported connection-length refinement demo is now indexed as a self-contained manufactured validation example and its plot is shown in the imported-FCI docs. The new connection-length page gives equations, history, code paths, and promotion rules for parallel connection length. Examples need to stay synchronized with every promoted artifact and geometry lane. | 84% | One clean-clone tokamak and one clean-clone stellarator tutorial that regenerate the advertised turbulence plots/movies without private reference checkouts. |
