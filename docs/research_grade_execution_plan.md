...
1. Finish the direct ESSOS-coil open diagnostic package on `main`.
   Implementation changes: improve endpoint-label restriction/refinement at
   target transition shells, rerun the boundary-resolved optional stage using
   `(7, 15, 27) -> (11, 25, 45)` grids, target-exit and adjacent-step nested
   diagnostics, neutral-source refinements, consumed target heat/particle flux
   plots, diagnostic movie generation, and a report field that explains
