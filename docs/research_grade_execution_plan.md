...
| Reference-backed parity | Many RHS, one-step, and campaign gates exist; neutral-mixed direct `SNVh_*` source formulas are now closed. The accepted-step reference-monitor path auto-builds a clean patched checkout, links the patched `hermes-3`, runs the neutral-mixed reference case, writes JSONL, and passes the schema gate. The June 5, 2026 local rerun with optional `Dnnh`, `Vh`, and `eta_h` diagnostics produced `148` accepted-step records. Native traces replay the reference adaptive accepted-step time grid, and the current comparison matches `148/148` accepted points. The contextual reference-monitor patch now deep-copies the `Dnnh` preparation ladder (`Tnlimh`, `logPnlimh`, `grad_logPnlimh`, raw diffusion, flux-limit maximum, flux-limited diffusion, and diffusion-limited pre-boundary diffusion), emits pointwise target-adjacent payloads, and a live rerun with those fields produced no missing ladder diagnostics. The native `grad_logPnlim*` norm now carries the structured metric terms (`g11`, `g22`, `g33`, and supported `g23`) used by the reference, removing metric-norm semantics as a formula-level blocker. The new `neutral_diffusion_ladder_register` ranks `Dnnh_flux_max` as the leading real pointwise target-cell offender: native `2.74471293` versus reference `2.73944` at local target index `[0, 3, 0]`, a `5.27e-3` drift. Final `Dnnh` follows at `4.46e-3`, while raw diffusion is much smaller (`6.07e-4`). The same cell shows the mismatch is driven by `grad_logPnlimh` near the upper target, not by temperature or raw diffusion. The `parallel_viscosity_input_register` still shows `Dnnh` as the dominant closure input drift, `eta_h` target drift about `49x` larger than the largest pointwise state-input drift (`99x` by the legacy zone metric), and `SNVh_parallel_viscosity` at `1.29e-4` pointwise (`5.35e-5` by the legacy zone metric). The final-state input-closure diagnostic verifies that native `Dnn`, `Vh`, and `eta_h` reconstructed from a reference dump match the reference final-state fields to roundoff. The remaining `NVh` mismatch is therefore localized to accepted-step near-target `Grad(logPnlim)`/flux-limit cap sequencing, plus target-adjacent state-history drift over longer recycling windows. | 95% | Patch the near-target `Grad(logPnlim)` stencil or pressure-guard sequencing for the flux cap, then rerun the pointwise accepted-step comparator and longer recycling windows. |
| JAX-native recycling solver | Fixed-layout residual, JVP checks, and fixed-BDF2 diagnostics are present and gated on lightweight fixtures. The new opt-in `bdf_active_array_jvp` full-output bridge routes SciPy BDF through `rhs_backend="active_array"` and grouped JVPs, while `bdf_fixed_full_field_jvp` remains the compatibility bridge. The default promotion wrapper now runs two output windows and requires prebuilt JVP direction reuse plus actual fixed-BDF2 corrector steps; the active-array bridge is an explicit `--include-active-array-jvp` experiment because the current local fixture run timed out before completion. Full output-window default promotion is not yet complete. | 59% | Full open-field recycling transient through the fixed-layout JAX residual, with parity, cProfile/RSS, and JAX trace evidence showing a runtime win rather than only transformability. |
...
| 3D imported-field and VMEC-extender SOL | Import, selected-field, reduced FCI, and smoke campaign gates exist. The VMEC-extender synthetic import example now runs from a clean temporary directory, and the analytic FCI SOL lane remains the strongest self-contained 3D evidence. Broad imported-field turbulence claims still require independent connection-length validation, consumed-map/source-fieldline agreement, and imported-field grid/time refinement. | 52% | Add imported connection-length and refinement campaigns, then regenerate smooth non-axisymmetric movies only from gates that pass those checks. |
