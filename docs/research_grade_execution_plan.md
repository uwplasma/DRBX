...
| Reference-backed parity | 99.2% | Keep the closed neutral `NVh` source split locked while extending the same term-level parity discipline to traced-field-line `g33`, hydrogen recycling, sheath, target-source, and longer-window diverted-tokamak campaigns. D/T/He target momentum parity is closed; the remaining one-step `Pd+` pressure difference is now localized to conduction-energy state sensitivity at the large `dt=25` gate and shrinks under timestep refinement. |
