...
| JAX-native recycling solver | Fixed-layout residual, JVP checks, and fixed-BDF2 diagnostics are present and gated on lightweight fixtures. The fixed residual contract now rejects packed-state rank/length mismatches, layout field-size inconsistencies, unknown active-array RHS keys, wrong RHS field shapes, wrong feedback shapes, and field/feedback shape mismatches before they can enter traced residuals. The opt-in `bdf_active_array_jvp` full-output bridge routes SciPy BDF through `rhs_backend="active_array"` and grouped JVPs, while `bdf_fixed_full_field_jvp` remains the compatibility bridge. The self-contained promotion wrapper now separates the full-output BDF/JVP parity phase from the non-SciPy fixed-BDF2 residual phase: full-output parity still runs on hydrogen and D/T/He, while fixed-BDF2 now runs on hydrogen at `timestep = 10` and D/T/He at `timestep = 1` with `runtime:recycling_fixed_bdf2_max_internal_timestep=0.5`. The D/T/He `timestep = 1` route now passes in both fixed-full-field and active-array modes with `fixed_bdf2_max_residual_inf_norm = 3.77e-9`, four internal substeps, three BDF2 correctors, zero unconverged steps, and zero failed linear solves; the corrected solver-health gate still prevents large residuals from passing as convergence. The adaptive-BDF active-array JAX-linearized path now reaches the full controller and, on a local `timestep=1` fixture, completed with `61` active-array RHS/JAX-linearized trial solves, no fallbacks, no unconverged substeps, no failed linear solves, and max accepted error ratio `9.31e-1`; rejected trials still push the all-trial max error above the strict promotion threshold, so it remains an opt-in research gate. Full output-window default promotion is not yet complete because full `timestep = 5000` fixed-BDF2 still needs a substep policy and the local `timestep = 1` D/T/He gate is slow (`98.9 s` fixed-full-field, `117.6 s` active-array, `5200` inner linear iterations). A June 9, 2026 active-array GMRES-control probe with `restart=10`, `maxiter=20`, and the same internal substep policy converged cleanly but slowed to `136.8 s`, so simple restart reduction is not a viable promotion path. The JAX-linearized solver now uses the selected jitted residual consistently in line search and final residual checks, removing one host/path inconsistency for the fixed-BDF2 JAX lane. A June 15, 2026 opt-in `state_scale` JAX-GMRES left-preconditioner hook is implemented and unit-tested, but the first real hydrogen fixed-layout probe is negative evidence: the unpreconditioned gate ran in `2.96 s`, while `runtime:recycling_jax_linear_preconditioner=state_scale` ran in `7.25 s` with the same residual and solver status. | 78% | Do not default-promote the current preconditioner. Reduce fixed-BDF2 linear iteration cost with a physics-informed block/Jacobi preconditioner, damping/startup policy, or cheaper residual/JVP kernel; then rerun parity, cProfile/RSS, and JAX trace evidence showing a runtime win rather than only transformability. |
