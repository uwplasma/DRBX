...
| Meaningful promoted coverage | Local closeout and promoted-solver coverage gates exceed the `95%` target on promoted surfaces. The June 5, 2026 promoted slice now includes fixed residual active-array `jit`/`jvp`/`vmap` gates, the neutral diffusion-preparation diagnostic helper, and direct neutral-mixed option-matrix gates for conduction, viscosity, Lax wave speed, flux limiting, and diffusion caps. It passes with `527` tests and reports `95.44%` unrounded total coverage on the promoted target files, while the bounded closeout slice remains at `97%`. | 96% | Keep the gates green after every solver/geometry promotion; avoid adding coverage-only smoke tests. |
...
| JAX-native recycling solver | Fixed-layout residual, JVP checks, and fixed-BDF2 diagnostics are present and gated on lightweight fixtures. The fixed residual contract now rejects packed-state rank/length mismatches, layout field-size inconsistencies, unknown active-array RHS keys, wrong RHS field shapes, wrong feedback shapes, and field/feedback shape mismatches before they can enter traced residuals. The opt-in `bdf_active_array_jvp` full-output bridge routes SciPy BDF through `rhs_backend="active_array"` and grouped JVPs, while `bdf_fixed_full_field_jvp` remains the compatibility bridge. The self-contained promotion wrapper now separates the full-output BDF/JVP parity phase from the bounded fixed-BDF2 residual phase: full-output parity still runs on hydrogen and D/T/He, while bounded fixed-BDF2 currently runs only on the hydrogen fixture at `timestep = 10`, where both fixed-full-field and active-array JAX-linearized routes pass the residual/status gate. The D/T/He fixed-BDF2 route remains a blocker after a local `timestep = 1` run reported `fixed_bdf2_unconverged_solver_steps = 1` and `fixed_bdf2_max_residual_inf_norm = 7.32`; this now reflects real solver health instead of a false small-step convergence flag. The adaptive-BDF active-array JAX-linearized path now reaches the full controller and, on a local `timestep=1` fixture, completed with `61` active-array RHS/JAX-linearized trial solves, no fallbacks, no unconverged substeps, no failed linear solves, and max accepted error ratio `9.31e-1`; rejected trials still push the all-trial max error above the strict promotion threshold, so it remains an opt-in research gate. Full output-window default promotion is not yet complete because the full `timestep = 5000` fixed-BDF2 fixture still exposes a large nonlinear residual in both fixed-full-field and active-array routes. | 71% | Improve D/T/He fixed-BDF2 nonlinear convergence, add adaptive substepping or damping for full output-window fixed-BDF2, then rerun parity, cProfile/RSS, and JAX trace evidence showing a runtime win rather than only transformability. |
