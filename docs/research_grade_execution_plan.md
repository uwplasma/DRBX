...
| Meaningful promoted coverage | Local closeout and promoted-solver coverage gates exceed the `95%` target on promoted surfaces. The June 5, 2026 promoted slice now includes fixed residual active-array `jit`/`jvp`/`vmap` gates and the neutral diffusion-preparation diagnostic helper, passes with `527` tests, and reports `95.44%` unrounded total coverage on the promoted target files, while the bounded closeout slice remains at `97%`. | 96% | Keep the gates green after every solver/geometry promotion; avoid adding coverage-only smoke tests. |
