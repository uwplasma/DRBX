...
| ESSOS direct-coil closed-field controls | A self-contained direct-coil closed/near-closed control package now exists with manufactured non-axisymmetric traces, Poincare sections, same-section return-distance classification, JSON/NPZ/PNG artifacts, tests, docs, and an example script. The example now also writes a self-contained closed-control refinement gate that checks stable closed/near-closed classification, bounded same-section return distance, sufficient Poincare sampling, and explicit absence of target, sheath, recycling, and neutral semantics across larger seed/time samples. The first live Landreman-Paul QA base run passes the return-map gate: 16 seed lines, 639 Poincare points, mean toroidal turns `10.11`, closed fraction `0.125`, near-closed fraction `0.875`, no open-like/no-return lines, and normalized return-distance p95 `8.45e-2`. A compact live same-source refinement with levels `(3, 3, 256)`, `(4, 4, 384)`, and `(5, 4, 512)` also passes with closed-or-near fraction `1.0`, spread `0.0`, p95 normalized return distance max `8.62e-2`, and Poincare density min `39.9` points per seed line. | 72% | Add a closed/near-closed reduced or full physics transient and frame-by-frame movie QA before promoting any closed direct-coil movie; keep the VMEC lane as the smoother closed-field physics tutorial path. |
...
| Direct ESSOS-coil closed control | `examples/geometry-3D/essos-field-lines/direct_coil_closed_field_demo.py`, `src/jax_drb/validation/essos_direct_coil_closed_control_campaign.py`, `docs/essos_direct_coil_closed_control.md` | The self-contained refined return-map control is implemented. Next add a live same-source refinement or longer-window control and optional closed/near-closed reduced transient only after return-distance and Poincare diagnostics remain stable. | Closed-control and closed-refinement JSON/NPZ/PNG artifacts, no endpoint/sheath/recycling flags, return-distance and field-period checks, visual QA. |
