...
scrape-off-layer calculations in `drbx`. The goal is not only to produce a
...
- `src/drbx/geometry/metric_tensor.py`, which stores covariant and
...
- `src/drbx/geometry/fci_maps.py`, which stores forward and backward traced
  plane-intersection maps plus boundary masks and plane spacing.
- `src/drbx/geometry/stellarator.py`, which builds an analytic
...
- `src/drbx/native/fci.py`, which implements JAX-native bilinear
  ...
  gate.
- `src/drbx/native/fci_sheath_recycling.py`, which implements the first
...
- `src/drbx/native/fci_neutral.py`, which implements the first
  non-axisymmetric neutral diffusion, ionisation, recombination, and
  charge-exchange reaction gate.
- `src/drbx/native/fci_vorticity.py`, which implements the first
...
- `src/drbx/native/fci_drb_rhs.py`, which assembles the first
...
- `src/drbx/geometry/essos_import.py`, which imports ESSOS-produced
  ...
  tracer in `drbx`.
- `src/drbx/validation/stellarator_fci_geometry_campaign.py`, which writes
  a metric/map/connection-length validation bundle.
- `src/drbx/validation/stellarator_fci_suite_campaign.py`, which runs the
  same metric/map gate on baseline island, stronger island-map, and high
  mirror/shear analytic configurations.
- `src/drbx/validation/stellarator_fci_operator_campaign.py`, which writes
  interpolation convergence, parallel-gradient convergence, and diffusion
  dissipation diagnostics.
- `src/drbx/validation/stellarator_metric_mms_campaign.py`, which writes
...
- `src/drbx/native/fci.py::logical_exb_bracket_xz`, which provides the
  ...
  advection.
- `src/drbx/validation/stellarator_drb_pytree_campaign.py`, which writes
  the fixed-layout PyTree RHS, JVP, batched-objective, and multi-device
  profiling artifact.
- `src/drbx/validation/stellarator_sol_showcase.py`, which writes a reduced
  3D SOL dynamics package with snapshots, an opened toroidal 3D poster, and a
  README-ready GIF.
- `src/drbx/validation/essos_fieldline_import_campaign.py`, which writes
...
- `src/drbx/validation/essos_imported_fci_campaign.py`, which converts a
  scaled VMEC Landreman-Paul QA shell into fixed-shape FCI maps through
  `coil`, `vmec`, or `hybrid` map sources, then runs DRBX sheath/recycling
...
- `src/drbx/validation/essos_imported_pytree_campaign.py`, which drives
  the fixed-layout DRBX PyTree RHS, `jax.jvp`, and `jax.vmap` diagnostics
  from the same ESSOS-imported field-line maps.
- `src/drbx/validation/essos_imported_drb_movie_campaign.py`, which
...
through ESSOS, then stores only sanitized arrays and metadata in `drbx`.
...
masks to a DRBX fixed-layout DRB transient with sheath, recycling, neutral
...
The external field-line import gate uses the same downstream map abstraction,
...
the magnetic field inside `drbx`. With cylindrical projections
...
and maps the endpoint back to the chosen geometry coordinates. This gives the
next bridge from analytic non-axisymmetric geometry to imported coil fields:
first an external ESSOS import gate, then Poincare/connection-length
...
4. Field-line/Poincare/connection-length promotion:
   compare the `drbx`-consumed map against the source geometry artifact for
...
7. Grid/timestep refinement for the imported QA turbulence lane. A movie is not
   ...
   are available, `require_observed_order=True` for promotion runs, and at
...
6. every user-facing example can be run from a clean `drbx` clone plus
