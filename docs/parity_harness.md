...
- [diagnose_recycling_timeline.py](scripts/diagnose_recycling_timeline.py) now gives a compact target-band timeline report for `recycling_1d_one_step` and `recycling_dthe_one_step`, so the first-output mismatch can be tracked field-by-field without writing large artifacts;
...
- the current transient blocker is now localized by the short-step reference probe in [diagnose_recycling_transient_step.py](scripts/diagnose_recycling_transient_step.py):
...
```bash
PYTHONPATH=src .venv/bin/python scripts/diagnose_integrated_2d_recycling_parity.py --reference-root /path/to/reference-suite

PYTHONPATH=src .venv/bin/python scripts/diagnose_integrated_2d_production_parity.py --reference-root /path/to/reference-suite
```
