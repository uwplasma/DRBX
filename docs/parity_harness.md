...
| `tokamak_recycling_one_step` | `live-reference target` | Direct tokamak-2D recycling first-output rung on the same example and launcher split. The reference-side initialization conflict is no longer the blocker; the next gating step is to promote the corresponding native transient path on top of the now-locked direct tokamak RHS lane. |
