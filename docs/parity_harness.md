...
| `tokamak_recycling_dthe_drifts_rhs` | `native-scaffolded target` | Direct drift-enabled multispecies tokamak-2D recycling RHS rung on `examples/tokamak-2D/recycling-dthe-drifts`. Committed summary/array baselines plus a committed direct-tokamak snapshot cache are now in-tree, and the native RHS path matches them exactly on the compact ion-pressure/momentum plus `Pe`, `phi`, and `Vort` surface. The manifest now carries deterministic `sound_speed` plus `solver:type=cvode` curation so this rung no longer depends on local deck quirks. |
