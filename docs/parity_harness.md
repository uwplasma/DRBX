...
| `tokamak_recycling_dthe_one_step` | `native operational target` | Curated direct multispecies tokamak-2D recycling first-output rung on the same geometry path, staged with `timestep=0.1` so the reference loop is practical. Committed summary/array baselines and a committed optional-history cache are now in-tree, and the native one-step path stays in a small operational band led by `Pd+` about `9.23e-3`, `NVd+` about `3.60e-2`, `Pt+` about `1.12e-2`, `NVt+` about `5.37e-2`, and `Pe` about `1.89e-4`. The blocker is now more specific than before: the script pair [diagnose_tokamak_recycling_one_step.py](scripts/diagnose_tokamak_recycling_one_step.py) and [diagnose_tokamak_recycling_ion_viscosity.py](scripts/diagnose_tokamak_recycling_ion_viscosity.py) shows that the lower-target-corner D/T mismatch tightened materially once the missing lower neutral guard was reconstructed, while the worst remaining one-step residual moved to the upper active edge of the local slab. On this rank `mesh.has_upper_y_target` is false there, so the next guard row is a communicated neighbor state rather than a local sheath boundary. That keeps the rung honest as `native_operational`: more local sheath heuristics are not enough, and the remaining promotion path is the broader native/distributed recycling transient backbone. |
