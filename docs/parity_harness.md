...
- the next native open-field milestone is still `recycling_dthe_rhs`; live reference comparisons now show that the remaining gap is specifically in the multispecies ion-viscosity / neutral-pressure source bookkeeping, not in the shared sheath/recycling scaffolding or the summary/array harness;
