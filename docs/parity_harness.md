...
| `tokamak_recycling_rhs` | `native-scaffolded target` | First direct tokamak-2D recycling RHS rung on `examples/tokamak-2D/recycling` with `process_count = 6`. Live reference execution is now stable again on the real tokamak geometry, and the native RHS path reaches full species/reaction assembly on the direct dump-backed lane. The next work is to lock committed baselines and tighten parity on this rung before promoting the transient ladder. |
| `tokamak_recycling_one_step` | `live-reference target` | Direct tokamak-2D recycling first-output rung on the same example and launcher split. The reference-side initialization conflict is no longer the blocker; the next gating step is to promote the corresponding native transient path on top of the now-running direct tokamak RHS lane. |
| `tokamak_recycling_dthe_one_step` | `live-reference target` | Direct multispecies tokamak-2D recycling first-output rung on `examples/tokamak-2D/recycling-dthe` with the same shared tokamak mesh staging. It should follow only after the single-species tokamak recycling rung is locked. |
...
- the first direct tokamak-2D geometry rung is now also in the same harness through `tokamak_diffusion_flow_one_step`, staged on `examples/tokamak-2D/diffusion-flow-evolveT` with `process_count = 6`. The harness now stages shared `mesh:file` inputs from parent directories deterministically, so this case can be launched through the same curated workflow and its current native dump-backed one-step scaffold matches the committed summary and array baselines exactly.
- the direct tokamak recycling manifest lane is no longer blocked on reference-side geometry startup. `tokamak_recycling_rhs` now launches live through the shared harness at `process_count = 6`, and `tokamak_recycling_one_step` is the next direct-geometry transient rung to promote on top of that revived reference path;
