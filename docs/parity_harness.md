...
| `tokamak_recycling_dthe_one_step` | `native operational target` | Curated direct multispecies tokamak-2D recycling first-output rung on the same geometry path, staged with `timestep=0.1` so the reference loop is practical. Committed summary/array baselines and a committed optional-history cache are now in-tree, and the native one-step path stays in a small operational band led by `Pd+` about `9.23e-3`, `NVd+` about `3.60e-2`, `Pt+` about `1.12e-2`, `NVt+` about `5.37e-2`, and `Pe` about `1.89e-4`. The remaining blocker is now localized by the script pair [diagnose_tokamak_recycling_one_step.py](scripts/diagnose_tokamak_recycling_one_step.py) and [diagnose_tokamak_recycling_ion_viscosity.py](scripts/diagnose_tokamak_recycling_ion_viscosity.py): the bad lower-target-corner cell is dominated by `DivPiPar_d+` and `DivPiPar_t+`, the native D/T Coulomb `K*_coll` inputs already match the reference diagnostics there to roundoff, the neutral-collision subtotal at those same cells is zero, and a live one-step dump check shows the reconstructed guard-cell state already matches the reference dump there. That leaves D/T charge exchange or the remaining viscosity collisionality closure as the main fix surface rather than generic guard-state assembly. |
