...
| `alfven_wave_rhs` | `native-scaffolded target` | First Stage 4 electromagnetic rung on the finite-electron-mass Alfvén-wave benchmark, now routed through a dump-backed native scaffold that reproduces the committed diagnostic RHS fields exactly. |
| `alfven_wave_one_step` | `native-scaffolded target` | First-output electromagnetic state rung on the same Alfvén-wave benchmark, now routed through the same dump-backed native scaffold before longer dispersion-history comparisons are ported. |
