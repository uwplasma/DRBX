...
| `neutral_mixed_short_window` | `open native runner target` | Baseline exists and the native implicit runner path now executes this lane. A bounded native prefix gate now covers the first three saved intervals inside the local ten-minute policy, but the broader short-window surface remains open. |
...
- the neutral runner path is now enabled for `neutral_mixed_one_step` and `neutral_mixed_short_window`, and the latest bounded one-step compare tightened materially after matching the reference neutral density wall extrapolation (`center Nh ≈ 8.0e-3`, `center Ph ≈ 5.7e-4`, `center NVh ≈ 8.6e-4`, `center T ≈ 2.9e-4`, `momentum RMS ≈ 1.7e-3` max-abs error at the locked `(5,3,5)` probe). The same native backbone now also has a bounded short-window prefix gate on the first three saved intervals (`center Nh ≈ 9.24e-2`, `center Ph ≈ 9.24e-3`, `center NVh ≈ 2.66e-3`, `center T ≈ 1.17e-4`, `momentum RMS ≈ 1.93e-3`), but this family still stays out of the promoted matrix until the heavier broader short-window surface is actually clean;
