...
| `alfven_wave_rhs` | `native-scaffolded target` | First Stage 4 electromagnetic rung on the finite-electron-mass Alfvén-wave benchmark, routed through a dump-backed native scaffold that still reproduces the committed diagnostic RHS fields exactly after replacing `Ajpar` with a native charged-current sum. |
| `alfven_wave_one_step` | `native-scaffolded target` | First-output electromagnetic state rung on the same Alfvén-wave benchmark, routed through the same dump-backed native scaffold, with `Ajpar` now computed natively from the charged momentum field plus the saved outer-radial zeroing convention while the other EM fields remain staged. |
...
- the first real electromagnetic operator is now ported inside that scaffold: `Ajpar` is no longer read from the dump, but reconstructed natively from the charged-species momentum sum, with the same zeroed outer-radial planes observed in the saved reference field. The live `alfven_wave_rhs` and `alfven_wave_one_step` summary and array comparisons remain exact after that replacement.
