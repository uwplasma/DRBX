...
| `tokamak_heat_transport_short_window` | `native-scaffolded target` | First multi-output direct tokamak electron-pressure transport rung on the same `heat-transport` example. The manifest, direct tokamak dump-backed runner path, and focused scaffold tests are in place; committed summary/array baselines are still pending because the longer reference history is materially slower than the current one-step rung. |
| `tokamak_diffusion_conduction_one_step` | `native-scaffolded target` | Next direct tokamak pressure-transport family on `examples/tokamak-2D/diffusion-conduction`, adding sheath-coupled parallel conduction without parallel momentum evolution. This rung is curated with `h+:diagnose=false` and `e:diagnose=false` because the upstream Hermes diagnostic-only `particle_flow_ylow` path aborts on empty fields at some ranks; with those overrides in place, baseline staging is practical on the shared tokamak geometry lane. |
