...
| `recycling_1d_one_step` | `native_operational` | Native transient solve now clears the bounded operational relative band (`rtol <= 2e-1`) used to unblock the 2D closure path. |
| `recycling_dthe_one_step` | `native_operational` | Native transient solve now clears the bounded operational relative band (`rtol <= 2e-1`) used to unblock the 2D closure path. |
...
| `integrated_2d_recycling_one_step` | `scaffolded_reference_backed` | Stable integrated 2D recycling first-output target with staged grid artifact, 10-rank launch, committed one-step baselines, and a dump-backed transient scaffold that now stays inside a bounded operational band while computing `Sd_target_recycle` and `Ed_target_recycle` natively at all saved times, including `t=0`. |
| `integrated_2d_recycling_short_window` | `scaffolded_reference_backed` | Stable integrated 2D recycling short-window target with staged grid artifact, 10-rank launch, committed reference baselines, and a dump-backed transient scaffold that stays inside a bounded operational band over `nout=5`; saved recycling diagnostics are now native across the whole window. |
| `integrated_2d_recycling_medium_window` | `scaffolded_reference_backed` | Stable integrated 2D recycling medium-window target with staged grid artifact, 10-rank launch, committed reference baselines, and a dump-backed transient scaffold that also stays inside a bounded operational band over the staged `nout=20` window; this is still scaffolded because the transient depends on dump-backed source and target-state preservation. |
...
| `tokamak_recycling_dthe_one_step` | `native operational target` | Curated direct multispecies tokamak-2D recycling first-output rung on the same geometry path, staged with `timestep=0.1` so the reference loop is practical. Committed summary/array baselines and a committed optional-history cache are now in-tree, and the one-step residual is materially tighter after restricting replay to the committed communicated non-owned Y-guard rows inside the implicit solve: the current compare surface is led by `NVd+` about `6.75e-3`, `NVt+` about `5.87e-3`, `Pd+` about `5.22e-3`, `Pt+` about `4.04e-3`, and `Pe` about `1.67e-4`. The blocker is therefore narrower and more honest than before: the lower-target-corner D/T mismatch tightened materially once the missing lower neutral guard was reconstructed, while the surviving one-step residual now sits on the upper active edge of the local slab. On this rank `mesh.has_upper_y_target` is false there, so the next guard row is a communicated neighbor state rather than a local sheath boundary. That keeps the rung honest as `native_operational`: more local sheath heuristics are not enough, and the remaining promotion path is the broader native/distributed recycling transient backbone. |
...
- the open-field one-step parity probes now follow the same bounded operational rule: both `recycling_1d_one_step` and `recycling_dthe_one_step` clear a relative-error gate of `rtol <= 2e-1` when `JAX_DRB_RUN_RECYCLING_ONE_STEP_PARITY=1` is enabled, so both lanes are tracked as `native_operational` rather than as exact closure;
...
- the integrated 2D recycling transient family now has a complete committed baseline surface too: `integrated_2d_recycling_one_step.json` / `.npz` are now in-tree, and the one-step, short-window, and medium-window paths all clear bounded operational tests against those committed baselines on the dump-backed transient scaffold. That closes the staged 2D recycling lane as operational evidence without overstating it as a fully native claim-bearing lane;
