...
- the neutral module now also has active-domain implicit stepping helpers and a residual-convergence regression, but the public runner still keeps `neutral_mixed_one_step` and `neutral_mixed_short_window` disabled until that substrate matches the committed reference transients closely enough to be reviewer-safe;
- the same module now also has an active-domain Jacobian sparsity builder for the neutral stencil, so the next implicit transient iteration can reuse sparse solver structure without rediscovering the coupling pattern from scratch;
- the same branch now also carries grouped sparse Jacobian assembly plus sparse Newton/GMRES and BDF2 helper paths, but the validated step helpers still default to the matrix-free nonlinear solve until the sparse direct path is benchmark-clean;
