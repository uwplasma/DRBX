...
| `integrated_2d_recycling_one_step` | `scaffolded_reference_backed` | Stable integrated 2D recycling first-output target with staged grid artifact, 10-rank launch, committed one-step baselines, and a dump-backed transient scaffold that now stays inside a bounded operational band while computing `Sd_target_recycle` and `Ed_target_recycle` natively at all saved times, including `t=0`, without replaying dump-backed density, pressure, or momentum source fields during the transient. |
| `integrated_2d_recycling_short_window` | `scaffolded_reference_backed` | Stable integrated 2D recycling short-window target with staged grid artifact, 10-rank launch, committed reference baselines, and a dump-backed transient scaffold that stays inside a bounded operational band over `nout=5`; saved recycling diagnostics are now native across the whole window and the transient no longer replays dump-backed source fields. |
| `integrated_2d_recycling_medium_window` | `scaffolded_reference_backed` | Stable integrated 2D recycling medium-window target with staged grid artifact, 10-rank launch, committed reference baselines, and a dump-backed transient scaffold that also stays inside a bounded operational band over the staged `nout=20` window; this is still scaffolded because the transient depends on dump-seeded initial state and target-state preservation. |
...
| `tokamak_recycling_dthe_one_step` | `native operational target` | Curated direct multispecies tokamak-2D recycling first-output rung on the same geometry path, staged with `timestep=0.1` so the reference loop is practical. Committed summary/array baselines and a committed optional-history cache are now in-tree, and the current compare surface is led by `NVd+` about `6.75e-3`, `NVt+` about `5.87e-3`, `Pd+` about `5.22e-3`, `Pt+` about `4.04e-3`, and `Pe` about `1.67e-4`. The active solve no longer depends on committed communicated-guard replay, so the remaining blocker is the native transient itself on the upper active edge of the local slab, not cached guard restoration. That keeps the rung honest as `native_operational`: more local sheath heuristics are not enough, and the remaining promotion path is the broader native/distributed recycling transient backbone. |
