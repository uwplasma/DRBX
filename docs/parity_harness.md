...
- the open-field one-step parity probes are now split on purpose: `recycling_1d_one_step` is treated as a bounded operational probe and clears a relative-error gate of `rtol <= 2e-1` when `JAX_DRB_RUN_RECYCLING_ONE_STEP_PARITY=1` is enabled, while `recycling_dthe_one_step` remains `xfail`-guarded until the multispecies transient solve is actually clean;
