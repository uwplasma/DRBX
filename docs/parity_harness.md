...
| `alfven_wave_rhs` | `native-scaffolded target` | First Stage 4 electromagnetic rung on the finite-electron-mass Alfvén-wave benchmark, routed through a partially native scaffold that now reconstructs `Ajpar`, `Apar`, and the physical/inner-radial `ddt(NVe)` core natively, with the `ddt(NVe)` core expressed as the benchmark’s periodic central-difference closure on `Vort` while preserving exact committed parity for the remaining staged diagnostic RHS guard rows. |
