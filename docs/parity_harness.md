...
- the staged integrated production runner now also understands compact committed snapshot caches in [reference_snapshots](/Users/rogerio/local/jax_drb/references/baselines/reference_snapshots). The first committed cache is the shared `integrated_2d_production_rhs` snapshot, which removes the repeated live-reference bootstrap for the broader production transient rungs. Optional-history caches for the broader production ladder can be layered on incrementally without changing the physics path.
- the same broader production lane now also has `integrated_2d_production_short_window`, widening Step 3 from a single production output into a multi-output production transient. It is still a scaffold target rather than a parity win, but the same ion-only target-preservation policy improves the longer window too: `Pe` is now much closer, and the live residuals are led by `NVd+` and `Sd_target_recycle`, with `Pd` already comparatively tight.
