...
- [compare_recycling_transient_modes.py](scripts/compare_recycling_transient_modes.py) now compares the available native one-step transient modes (`continuation`, `bdf`, `bdf_fixed_full_field_jvp`, `bdf_active_array_jvp`, `fixed_bdf2_jax_linearized`, `fixed_bdf2_jax_linearized_lineax`, `adaptive_be`, `adaptive_bdf`, `adaptive_bdf_sparse_jvp`, `adaptive_bdf_jax_linearized`, and `adaptive_bdf_jax_linearized_lineax`) directly against the committed `recycling_*_one_step` array baselines using the curated deck, and the same mode can now be pinned from a deck through `[runtime].recycling_transient_solver_mode`;
