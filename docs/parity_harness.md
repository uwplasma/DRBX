...
- [compare_recycling_transient_modes.py](scripts/compare_recycling_transient_modes.py) now compares the available native one-step transient modes (`continuation`, `bdf`, `adaptive_be`, `adaptive_bdf`) directly against the committed `recycling_*_one_step` array baselines using the curated deck, and the same mode can now be pinned from a deck through `[runtime].recycling_transient_solver_mode`;
