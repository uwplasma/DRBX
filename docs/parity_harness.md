...
| `integrated_2d_recycling_one_step` | `native_operational` | Stable integrated 2D recycling first-output target with staged grid artifact, 10-rank launch, committed one-step baselines, and a native transient path that stays inside a bounded operational band while computing `Sd_target_recycle` and `Ed_target_recycle` natively at all saved times, including `t=0`, without replaying dump-backed density, pressure, or momentum source fields or preserving dump target state during the transient. |
| `integrated_2d_recycling_short_window` | `native_operational` | Stable integrated 2D recycling short-window target with staged grid artifact, 10-rank launch, committed reference baselines, and a native transient path that stays inside a bounded operational band over `nout=5`; saved recycling diagnostics are native across the whole window and the transient no longer replays dump-backed source fields or preserves dump target state. |
| `integrated_2d_recycling_medium_window` | `native_operational` | Stable integrated 2D recycling medium-window target with staged grid artifact, 10-rank launch, committed reference baselines, and a native transient path that also stays inside a bounded operational band over the staged `nout=20` window. This still starts from a dump-seeded initial state, so it is operational rather than exact. |
...
- the integrated 2D recycling transient family now has a complete committed baseline surface too: `integrated_2d_recycling_one_step.json` / `.npz` are now in-tree, and the one-step, short-window, and medium-window paths all clear bounded operational tests against those committed baselines on a native transient path that no longer replays dump-backed source fields or preserves dump target state. The remaining scaffold is the dump-seeded initial state, so the family is now tracked as `native_operational` rather than `scaffolded_reference_backed`;
