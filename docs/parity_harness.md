...
- the recycling transient branch now has both a shared-implicit backward-Euler residual path and an adaptive BDF probe around the packed active-domain state plus controller integrals; the packed RHS now reuses a cached runtime model and the sparse Newton path can use a direct sparse linear solve on these small active systems, but the generic adaptive BDF wrapper is still too slow to promote the public `recycling_1d_one_step` / `recycling_dthe_one_step` cases yet;
