...
| `neutral_mixed_one_step` | `native-operational` | Baseline exists and the native implicit runner path now executes this lane. Connected-y guard reconstruction plus the eight-substep one-step default reduce the bounded centerline errors to about `Nh = 5.39e-5`, `Ph = 5.90e-6`, `NVh = 4.24e-6`, and momentum RMS at roundoff scale; the release-media figure still needs regeneration from this code path. |
