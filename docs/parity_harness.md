...
- the next production diagnostic rung is now also in-tree: [diagnose_integrated_2d_production_step.py](/Users/rogerio/local/jax_drb/scripts/diagnose_integrated_2d_production_step.py) compares the native one-step state and the native RHS-on-reference-state against a fresh one-step reference run. The current result is decisive: on the reference-evolved production state, `Sd_target_recycle` and `Ed_target_recycle` are exact, `ddt(NVd+)` is already small, but `ddt(Pe)` is still badly wrong at the target-band cell and `ddt(Nd+)` / `ddt(Pd+)` / `ddt(Nd)` are also still off there. That means the remaining Step 3 work should focus on local target-band pressure/density RHS terms rather than the recycling diagnostics.
...
- the same broader production lane now also has `integrated_2d_production_short_window`, widening Step 3 from a single production output into a multi-output production transient. It is still a scaffold target rather than a parity win, but the same ion-only target-preservation policy improves the longer window too: the live short-window target-band residuals are now led by `Pe` (about `1.43` max abs diff), then `NVd+` (about `9.8e-1`), then `Sd_target_recycle` (about `3.75e-1`), with `Pd+` and `Nd+` already much tighter.
