...
| `tokamak_heat_transport_short_window` | `native-scaffolded target` | First multi-output direct tokamak electron-pressure transport rung on the same `heat-transport` example. This rung is now curated with the harmless early-window history `nout=2` plus `e:diagnose=false` so the reference run stays practical; committed summary/array baselines are in-tree and the native direct tokamak path is now being locked against them on `Pe`. |
...
| `tokamak_linear_transport_one_step` | `native-scaffolded target` | Staged next low-risk direct tokamak fixed-density electron transport rung on `examples/tokamak-2D/linear-transport`. The manifest, direct tokamak dump-backed runner path, and focused scaffold tests are in place, with `e:diagnose=false` as the only harmless curation; the raw one-step output interval is still too slow to promote as an exact gate in the current pass. |
...
For native-supported inputs, `jax_drb input.toml` and `jax-drb run <BOUT.inp>` both write the corresponding JAX-side summary JSON, arrays NPZ, restart NPZ, and verbose run-log JSON. The runnable walkthrough at [examples/restartable_diffusion_tutorial.py](/Users/rogerio/local/jax_drb/examples/restartable_diffusion_tutorial.py) shows the same artifact flow plus restart/resume, TOML runtime-precision selection, and Matplotlib postprocessing from the saved arrays, and [examples/diffusion_precision_benchmark.py](/Users/rogerio/local/jax_drb/examples/diffusion_precision_benchmark.py) now measures the same native rung in isolated `float64` and `float32` subprocesses.
...
- the first direct tokamak-2D geometry lane is now broader than a single rung: `tokamak_diffusion_flow_one_step`, `tokamak_diffusion_transport_one_step`, `tokamak_diffusion_transport_short_window`, `tokamak_heat_transport_one_step`, `tokamak_heat_transport_short_window`, `tokamak_diffusion_conduction_one_step`, and `tokamak_diffusion_one_step` are all staged on the shared tokamak geometry harness with `process_count = 6`. The harness now stages shared `mesh:file` inputs from parent directories deterministically, the exact transport/conduction/diffusion rungs match their committed summary and array baselines on the native dump-backed path, and the next staged low-risk family is `tokamak_linear_transport_one_step`.
