...
- the first direct tokamak-2D geometry lane is now broader than a single rung: `tokamak_diffusion_flow_one_step`, `tokamak_diffusion_transport_one_step`, `tokamak_diffusion_transport_short_window`, `tokamak_heat_transport_one_step`, `tokamak_heat_transport_short_window`, `tokamak_diffusion_conduction_one_step`, `tokamak_diffusion_one_step`, `tokamak_linear_transport_one_step`, `tokamak_isothermal_rhs`, `tokamak_isothermal_one_step`, `tokamak_isothermal_short_window`, `tokamak_isothermal_medium_window`, `tokamak_turbulence_rhs`, `tokamak_turbulence_one_step`, and `tokamak_turbulence_short_window` are all staged on the shared tokamak geometry harness with `process_count = 6`. The harness now stages shared `mesh:file` inputs from parent directories deterministically, and the exact transport/conduction/diffusion/linear-transport/isothermal/turbulence rungs all match their committed summary and array baselines on the native dump-backed path. The cache-backed subset on this lane now includes the exact isothermal and turbulence operator rungs, the exact isothermal transient rungs, and the exact transient rungs `tokamak_diffusion_transport_short_window`, `tokamak_heat_transport_short_window`, and `tokamak_turbulence_short_window` plus the cheapest exact one-step families, so repeated parity checks are depending less and less on fresh Hermes launches.
