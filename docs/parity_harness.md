...
- the open-field one-step blocker is now narrower too: the implicit recycling residual no longer double-advances density-feedback integrals when those integrals are already part of the packed solve state, and that behavior is unit-locked; the open-field one-step runner also no longer seeds the transient from a synthesized final active-state template, and now explicitly starts from the committed RHS snapshot state. Bounded probes still rule out smaller continuation startup `dt` and forcing sanitized implicit residual fields as first-order fixes for the leading `recycling_1d_one_step` `Nd+` miss. The remaining blocker is still the active transient evolution itself near the target-adjacent cell.
- the current best open-field continuation controller is now explicit and kept in-tree: the first output interval on open-field recycling runs uses a startup warmup that splits the first `25` time units into `4 x 6.25` sparse implicit substeps before the usual continuation cadence resumes. On the bounded `timestep = 25` single-species blocker probe, that reduces the upper-cell drift from about `Nd+ ≈ 2.13e-2`, `Pd+ ≈ 2.03e-2`, `NVd+ ≈ 4.05e-2`, `Pe ≈ 2.13e-2` down to about `Nd+ ≈ 5.78e-3`, `Pd+ ≈ 5.28e-3`, `NVd+ ≈ 1.56e-3`, `Pe ≈ 5.72e-3`. The same warmup also materially tightens the multispecies `recycling_dthe_one_step` short-step probe, so it is now the default continuation startup on the open-field research lane rather than a throwaway experiment.
- the latest solver-side closure pass tightened that read further: forcing the continuation interval to stay backward-Euler-only makes `Pd+`, `NVd+`, and `Pe` worse even though `Nd+` shifts slightly, so the BE-to-BDF2 handoff is not the main blocker. An explicit-RHS startup predictor is now used as the sparse BE/BDF2 Newton initial guess and is unit-locked; it materially improves `Pd+` / `NVd+` on the committed one-step compare but still leaves `Nd+` as the dominant miss. A fresh full-interval `adaptive_be` probe still fails the five-minute local gate, so the next fix belongs in the active transient evolution itself rather than in another startup/controller variant.
- [diagnose_recycling_boundary_cell.py](scripts/diagnose_recycling_boundary_cell.py) now gives the short-step cell-local picture at the worst upper target-adjacent active cells. On the `timestep = 25` single-species probe, it shows that the remaining drift is not primarily in `SNVd+` / `SNVd` / `Sd_target_recycle`; the larger local mismatch is in the transient ion momentum path, with `Ve` and `Epar` low, `ddt(NVd+)` correspondingly high, and the ion-term split showing the dominant miss in `momentum_advection` at that upper active cell.
- the same bounded probe also shows that this local `momentum_advection` miss follows the velocity state much more strongly than the density state: swapping only the reference `NVd+` into the native short-step state moves the bad-cell `momentum_advection` much closer to the reference value, while swapping only `Nd+` barely changes it.
...
| `neutral_mixed_one_step` | `native-operational` | Baseline exists and the native implicit runner path now executes this lane. Connected-y guard reconstruction plus the eight-substep one-step default reduce the bounded centerline errors to about `Nh = 5.39e-5`, `Ph = 5.90e-6`, `NVh = 4.24e-6`, and momentum RMS at roundoff scale; the release-backed term-balance media now reflects this code path. |
...
| `recycling_1d_short_window` | `native operational target` | First curated repeated-output open-field recycling transient rung on the local native backbone, staged with `nout=5`; the latest bounded compare is led by about `NVd+ ≈ 1.05e-1`, `Pd+ ≈ 6.13e-2`, and `Nd+ ≈ 4.55e-2`, so this remains evidence for operational closure rather than an exact promotion. |
| `recycling_1d_one_step` | `native_exact` | Native transient solve now clears the promoted open-field one-step scaled-diff gate (`relative_to_expected_max < 5e-2`) on the committed compare surface. |
| `recycling_dthe_one_step` | `native_exact` | Native transient solve now clears the promoted multispecies open-field one-step scaled-diff gate (`relative_to_expected_max < 5e-2`) on the committed compare surface. |
| `recycling_1d_long` | `blocked` | Depends on the transient ladder. |
| `integrated_2d_recycling_rhs` | `native-scaffolded target` | Stable integrated 2D recycling geometry target with staged grid artifact, 10-rank launch, a native local-dump RHS scaffold, slab-local target routing, dump-state sheath preservation, dump-backed `SNd`/`SNd+` and `SPd`/`SPd+` staging for the integrated case, and a source-faithful `sheath_boundary_simple` electron boundary path. |
| `integrated_2d_recycling_one_step` | `native_exact` | Stable integrated 2D recycling first-output target with staged grid artifact, 10-rank launch, committed one-step baselines, and a native transient path that now clears the promoted mixed exact gate: non-negligible fields stay inside a tight scaled band while the effectively silent `NVd` channel stays inside a tiny absolute band. `Sd_target_recycle` and `Ed_target_recycle` are computed natively at all saved times, including `t=0`, and the transient no longer replays dump-backed density, pressure, or momentum source fields or preserves dump target state during the transient. |
| `integrated_2d_recycling_short_window` | `native_exact` | Stable integrated 2D recycling short-window target with staged grid artifact, 10-rank launch, committed reference baselines, and a native transient path that now clears the same promoted mixed exact gate over `nout=5`; saved recycling diagnostics are native across the whole window and the transient no longer replays dump-backed source fields or preserves dump target state. |
| `integrated_2d_recycling_medium_window` | `native_exact` | Stable integrated 2D recycling medium-window target with staged grid artifact, 10-rank launch, committed reference baselines, and a native transient path that also clears the promoted mixed exact gate over the staged `nout=20` window. |
...
| `tokamak_diffusion_conduction_short_window` | `native-scaffolded target` | First exact transient rung on the direct tokamak diffusion-conduction stack. Committed summary/array baselines plus committed snapshot/history caches are now in-tree, the harmless curation is `nout=5` with `h+:diagnose=false` and `e:diagnose=false`, and the native direct tokamak path matches the committed baseline exactly on `Nh+`, `Ph+`, and `Pe`. |
...
| `tokamak_linear_transport_short_window` | `native-scaffolded target` | First exact transient rung on the direct tokamak fixed-density electron transport stack. Committed summary/array baselines plus committed snapshot/history caches are now in-tree, the harmless curation is `nout=5` with `e:diagnose=false`, and the native direct tokamak path matches the committed baseline exactly on `Pe`. |
| `tokamak_isothermal_rhs` | `native-scaffolded target` | Direct tokamak electrostatic/vorticity operator rung on `examples/tokamak-2D/isothermal`. Committed summary/array baselines plus committed snapshot/history caches are now in-tree, the harmless curation is `timestep=0.1` plus `e:diagnose=true`, `i:diagnose=true`, and `vorticity:diagnose=true`, and the native direct tokamak dump-backed path matches the committed baseline exactly on `Ne`, `Ni`, `NVe`, `NVi`, `phi`, `Vort`, `ddt(Ne)`, `ddt(NVe)`, `ddt(NVi)`, and `ddt(Vort)`. |
| `tokamak_isothermal_one_step` | `native-scaffolded target` | First direct tokamak electrostatic/vorticity rung on `examples/tokamak-2D/isothermal`. Committed summary/array baselines plus committed snapshot/history caches are now in-tree, the current harmless curation is `timestep=0.1`, and the native direct tokamak dump-backed path matches the committed baseline exactly on `Ne`, `Ni`, `NVe`, `NVi`, `phi`, and `Vort`. |
| `tokamak_isothermal_short_window` | `native-scaffolded target` | First exact transient rung on the direct tokamak electrostatic/vorticity isothermal stack. Committed summary/array baselines plus committed snapshot/history caches are now in-tree, the harmless curation is `nout=2`, `timestep=0.1`, and the native direct tokamak path matches the committed baseline exactly on `Ne`, `Ni`, `NVe`, `NVi`, `phi`, and `Vort`. |
| `tokamak_isothermal_medium_window` | `native-scaffolded target` | Broader exact transient rung on the direct tokamak electrostatic/vorticity isothermal stack. Committed summary/array baselines plus committed snapshot/history caches are now in-tree, the harmless curation is `nout=5`, `timestep=0.1`, and the native direct tokamak path matches the committed baseline exactly on `Ne`, `Ni`, `NVe`, `NVi`, `phi`, and `Vort`. |
| `tokamak_turbulence_rhs` | `native-scaffolded target` | Direct tokamak turbulence operator rung on `examples/tokamak-2D/turbulence`, locking the first-output RHS surface before the transient history surfaces. Committed summary/array baselines plus committed snapshot/history caches are now in-tree, the harmless curation is `nout=0`, `timestep=0.1`, and the native direct tokamak path matches the committed baseline exactly on `Ne`, `Nd+`, `NVe`, `NVd+`, `Pe`, `Pd+`, `phi`, `Vort`, `ddt(Ne)`, `ddt(NVe)`, and `ddt(Pe)`. |
| `tokamak_turbulence_one_step` | `native_exact` | First direct tokamak turbulence rung on `examples/tokamak-2D/turbulence`, widening the electrostatic drift lane beyond the isothermal stack while staying on the same dump-backed exact path. Committed summary/array baselines plus committed snapshot/history caches are now in-tree, the current harmless curation is `timestep=0.1`, and the native direct tokamak path matches the committed baseline exactly on `Ne`, `Nd+`, `NVe`, `NVd+`, `Pe`, `Pd+`, `phi`, and `Vort`. |
| `tokamak_turbulence_short_window` | `native-scaffolded target` | First direct tokamak turbulence transient rung on `examples/tokamak-2D/turbulence`, curated with `nout=2` and `timestep=0.1` so the early history is practical while staying on the same exact dump-backed path. Committed summary/array baselines plus committed snapshot/history caches are now in-tree and the native direct tokamak path matches them exactly on `Ne`, `Nd+`, `NVe`, `NVd+`, `Pe`, `Pd+`, `phi`, and `Vort`. |
...
| `tokamak_recycling_dthe_drifts_rhs` | `native-scaffolded target` | Direct drift-enabled multispecies tokamak-2D recycling RHS rung on `examples/tokamak-2D/recycling-dthe-drifts`. Committed summary/array baselines plus a committed direct-tokamak snapshot cache are now in-tree, and the native RHS path matches them exactly on the current compact recycling surface `Nd+`, `Pd+`, `NVd+`, `Nt+`, `Pt+`, `NVt+`, `Nhe+`, `Phe+`, `NVhe+`, and `Pe`. The manifest now carries deterministic `sound_speed` plus `solver:type=cvode` curation so this rung no longer depends on local deck quirks. |
...
| `tokamak_recycling_dthe_drifts_one_step` | `native operational target` | Curated first-output direct tokamak drift-enabled multispecies recycling rung on the D/T/He divertor stack. Committed summary/array baselines and a committed optional-history cache are now in-tree, the manifest carries deterministic `sound_speed` plus `solver:type=cvode` curation, and the native one-step path stays in a small operational band on the current recycling surface `Nd+`, `Pd+`, `NVd+`, `Nt+`, `Pt+`, `NVt+`, `Nhe+`, `Phe+`, `NVhe+`, and `Pe`. `phi` and `Vort` are not yet part of the native recycling output for this lane and are tracked separately as a missing drift-field capability rather than silently tolerated. |
...
| `tokamak_recycling_dthe_one_step` | `native exact target` | Curated direct multispecies tokamak-2D recycling first-output rung on the same geometry path, staged with `timestep=0.1` so the reference loop is practical. Committed summary/array baselines and a committed optional-history cache are now in-tree, and the promoted compare surface now clears a mixed exact gate: the non-negligible ion/electron fields stay inside a `5e-2` scaled band while the near-zero neutral channels stay inside a small absolute band. The active solve no longer depends on committed communicated-guard replay or dump target preservation, and it is now routed through a dedicated direct-tokamak recycling runner entry rather than the integrated 2D dispatch path. The first richer direct-window probe is now bounded too: a live Hermes-backed `nout=2` check on the same D/T/He surface stays inside a mixed operational band with worst scaled diff `NVd+ ≈ 2.74e-2` and worst near-zero absolute diff `Pd ≈ 2.67e-5` under the current ten-minute local policy. |
...
For native-supported inputs, `jax_drb input.toml` and `jax-drb run <BOUT.inp>` both write the corresponding JAX-side summary JSON, arrays NPZ, restart NPZ, and verbose run-log JSON. The runnable walkthrough at [examples/restartable_diffusion_tutorial.py](examples/restartable_diffusion_tutorial.py) now shows the same artifact flow plus deck-declared `[output]`, `[restart]`, and `[runtime.logging]` control, including the boolean `[runtime.logging].verbose = true|false` switch, restart/resume, TOML runtime-precision selection, and Matplotlib postprocessing from the saved arrays. The same detailed-event switch is now available from both `jax_drb --verbose` and `run_input_case(..., verbose=True)`. The run-log JSON now also stores the ordered event stream that the rich terminal path prints during the run, along with execution-environment metadata (`jax_version`, `python_version`, platform, pid, working directory), and [examples/diffusion_precision_benchmark.py](examples/diffusion_precision_benchmark.py) measures the same native rung in isolated `float64` and `float32` subprocesses.
...
- the neutral RHS slice now also has a committed full-array regression with explicit max/RMS tolerances on `ddt(Nh)` and `ddt(Ph)`, plus a compact diagnosed-reference artifact in [neutral_mixed_rhs_diagnostics.json](references/baselines/reference_metrics/neutral_mixed_rhs_diagnostics.json) for the isolated parallel density term and advective-flow centerlines;
...
- the public runner no longer blocks `neutral_mixed_one_step` and `neutral_mixed_short_window` at the dispatch layer: both lanes now run through the shared implicit neutral backbone, with the one-step lane promoted to an eight-substep bounded native check while the short-window lane remains a bounded operational history gate;
...
- the neutral runner path is now enabled for `neutral_mixed_one_step` and `neutral_mixed_short_window`, and the latest bounded one-step compare tightened materially after connected-y guard reconstruction and the eight-substep one-step default (`center Nh ≈ 5.39e-5`, `center Ph ≈ 5.90e-6`, `center NVh ≈ 4.24e-6`, `center T ≈ 6.38e-7`, `momentum RMS ≈ 7.74e-10` max-abs error at the locked `(5,3,5)` probe). The same native backbone still clears a bounded full short-window metric gate on the four-substep matrix-free path (`center Nh ≈ 8.03e-3`, `center Ph ≈ 6.47e-4`, `center NVh ≈ 8.60e-4`, `center T ≈ 2.91e-4`, `total Nh ≈ 3.24e-1`, `total Ph ≈ 2.89e-2`, `momentum RMS ≈ 1.71e-3`) and a bounded trimmed active-domain full-array gate on `Nh`/`Ph`/`NVh` (`Nh max|Δ| ≈ 1.27e-2`, `Ph max|Δ| ≈ 1.21e-3`, `NVh max|Δ| ≈ 3.37e-3`), so the remaining neutral work is the longer-history total-density/pressure drift rather than the one-step `NVh` source closure;
...
- a compact recycling parity scaffold now exists in [parity/diff.py](src/jax_drb/parity/diff.py) and [parity/recycling.py](src/jax_drb/parity/recycling.py), giving per-variable max-absolute-difference and argmax-location reports plus staged reference controller snapshot extraction for the one-step recycling cases;
- the open-field one-step parity probes now follow a stricter promoted rule: both `recycling_1d_one_step` and `recycling_dthe_one_step` clear a scaled-diff gate of `relative_to_expected_max < 5e-2` when `JAX_DRB_RUN_RECYCLING_ONE_STEP_PARITY=1` is enabled, so both first-output open-field lanes are now tracked as `native_exact` on their committed compare surfaces;
- the promoted 2D recycling exact gate now also has an explicit bounded pytest path: when `JAX_DRB_RUN_RECYCLING_2D_PARITY=1` is enabled, `integrated_2d_recycling_one_step`, `integrated_2d_recycling_short_window`, `integrated_2d_recycling_medium_window`, and `tokamak_recycling_dthe_one_step` all clear a mixed exact gate that combines scaled-diff checks on the non-negligible fields with small absolute bands on the effectively silent neutral channels;
- `diagnose_recycling_timeline.py` now gives a compact target-band timeline report for `recycling_1d_one_step` and `recycling_dthe_one_step`, so the first-output mismatch can be tracked field-by-field without writing large artifacts;
- [compare_recycling_transient_modes.py](scripts/compare_recycling_transient_modes.py) now compares the available native one-step transient modes (`continuation`, `bdf`, `bdf_fixed_full_field_jvp`, `bdf_active_array_jvp`, `fixed_bdf2_jax_linearized`, `fixed_bdf2_jax_linearized_lineax`, `fixed_bdf2_active_array_jax_linearized`, `fixed_bdf2_active_array_jax_linearized_lineax`, `adaptive_be`, `adaptive_bdf`, `adaptive_bdf_sparse_jvp`, `adaptive_bdf_jax_linearized`, `adaptive_bdf_jax_linearized_lineax`, `adaptive_bdf_active_array_jax_linearized`, and `adaptive_bdf_active_array_jax_linearized_lineax`) directly against the committed `recycling_*_one_step` array baselines using the curated deck, and the same mode can now be pinned from a deck through `[runtime].recycling_transient_solver_mode`;
- the first bounded single-species sweep now shows that solver choice alone is not the remaining Step 2 fix: `continuation` takes about `66.7 s` and `bdf` about `34.5 s` on `recycling_1d_one_step`, but both still land in the same `O(1e-2)` band, with `bdf` only modestly reducing `NVd+`/`Pd+` while leaving `Nd+` essentially unchanged; a bounded `adaptive_bdf` probe did not finish within the five-minute local gate, so the next pass should stay on active transient/controller closure rather than on heavier solver variants;
- `diagnose_recycling_dense_history.py` now stages a dense-output reference run (for example `nout=40`, `timestep=125`) and compares that trajectory directly to the native transient history, which is the fastest way to see whether drift is already present at each accepted step or only in the coarse first-output comparison;
...
- the integrated 2D recycling transient family now has a complete committed baseline surface too: `integrated_2d_recycling_one_step.json` / `.npz` are now in-tree, and the one-step, short-window, and medium-window paths all clear a promoted mixed exact gate against those committed baselines on a native transient path that no longer replays dump-backed source fields or preserves dump target state;
...
- the next production diagnostic rung is now also in-tree: `diagnose_integrated_2d_production_step.py` compares the native one-step state and the native RHS-on-reference-state against a fresh one-step reference run. After tightening that script to use the evolved one-step source overrides from the final Hermes dump, the result is sharper: on the reference-evolved production state, `Sd_target_recycle` and `Ed_target_recycle` are exact, `ddt(Nd+)` is now effectively exact, and the remaining meaningful target-band RHS leaders are `ddt(Pe)` first, then `ddt(Pd+)`, then `ddt(NVd+)`, with `ddt(Nd)` smaller but still visible. That means the remaining Step 3 work should focus on local target-band pressure/ion-pressure/momentum RHS terms rather than the recycling diagnostics.
- those same Step 3 term-diagnostic scripts now also support `--use-committed-baselines`. The intended fast loop is now:
  - committed production summary/array baselines through `diagnose_integrated_2d_production_parity.py`;
  - committed final one-step diagnostic snapshot for `diagnose_integrated_2d_production_step.py`, `diagnose_integrated_2d_production_pe_terms.py`, and [diagnose_integrated_2d_production_ion_terms.py](scripts/diagnose_integrated_2d_production_ion_terms.py), or the new synthetic fallback built from the committed active-domain arrays plus optional-history cache when that exact snapshot is unavailable;
...
- `diagnose_integrated_2d_production_pe_terms.py` now decomposes the target-band production `ddt(Pe)` assembly on the reference-evolved state. The current decomposition shows that the dominant mismatch is in the electron parallel-compression term `-(5/3) * Div_par_mod(Pe, Ve)`, not in the explicit pressure source and not in the small parallel-advection correction.
...
- the ion-side preserve path has now been tightened in the matching way: when `preserve_dump_ion_target_state_only` is active, the preserved ion target cell no longer carries the sheath-generated ion `energy_source`, while the sheath-generated ion guard cells are still kept. That collapses the old one-step `Pd+` blocker. The target-recycling energy path now follows the fixed returning-neutral energy branch seen in the current Hermes reference output, and RHS-only dump `SPd` overrides are treated as total neutral pressure sources so they do not double-count native `Ed_target_recycle`. On the current committed-baseline target-band compare for `integrated_2d_production_one_step`, the leading residual is now `Pe` at about `1.63e-1`, followed by `Nd` at about `1.10e-2`, `Pd+` at about `5.0e-3`, `Nd+` at about `4.1e-3`, `Sd_target_recycle` at about `1.0e-3`, `NVd+` at about `6.2e-4`, and `Ed_target_recycle` at about `3.3e-4`.
...
- that operational Step 3 state is now locked in the test suite as well. The committed-baseline integrated production regressions explicitly gate:
  ...
  - `integrated_2d_production_short_window` on the current broader-window operational maxima for `Pe`, `NVd+`, `Nd`, `Nd+`, `Pd+`, `Pd`, `Sd_target_recycle`, and `Ed_target_recycle`.
...
- the harness now has two targeted blocker discriminators for the integrated production lane:
  - [diagnose_integrated_2d_production_anomalous_diffusion.py](scripts/diagnose_integrated_2d_production_anomalous_diffusion.py) runs Hermes with and without `anomalous_diffusion` on `d+` and `e` and compares both against the current native one-step target-band residual, so the next physics slice can be chosen from direct evidence;
  - `diagnose_integrated_2d_production_step.py` now performs a controlled boundary A/B sweep between the current production mixed-preserve mode and a full-sheath mode on the bad target-band cells, so boundary-state mismatch can be separated from missing operator/source-term mismatch.
...
- the first direct tokamak-2D geometry lane is now broader than a single rung: `tokamak_diffusion_flow_one_step`, `tokamak_diffusion_transport_one_step`, `tokamak_diffusion_transport_short_window`, `tokamak_heat_transport_one_step`, `tokamak_heat_transport_short_window`, `tokamak_diffusion_conduction_one_step`, `tokamak_diffusion_one_step`, `tokamak_linear_transport_one_step`, `tokamak_linear_transport_short_window`, `tokamak_isothermal_rhs`, `tokamak_isothermal_one_step`, `tokamak_isothermal_short_window`, `tokamak_isothermal_medium_window`, `tokamak_turbulence_rhs`, `tokamak_turbulence_one_step`, and `tokamak_turbulence_short_window` are all staged on the shared tokamak geometry harness with `process_count = 6`. The harness now stages shared `mesh:file` inputs from parent directories deterministically, and the exact transport/conduction/diffusion/linear-transport/isothermal/turbulence rungs all match their committed summary and array baselines on the native dump-backed path. The cache-backed subset on this lane now includes the exact isothermal and turbulence operator rungs, the exact isothermal transient rungs, and the exact transient rungs `tokamak_diffusion_transport_short_window`, `tokamak_heat_transport_short_window`, `tokamak_linear_transport_short_window`, and `tokamak_turbulence_short_window` plus the cheapest exact one-step families, so repeated parity checks are depending less and less on fresh Hermes launches.
- the neighboring drift-enabled recycling family, `examples/tokamak-2D/recycling-dthe-drifts`, is now promoted into the committed ladder at the curated `rhs` and `one_step` level. The manifest carries deterministic `sound_speed` plus `solver:type=cvode` curation, the compact `rhs` rung is exact, and the one-step rung is now in a small operational band on the drift-enabled D/T/He recycling surface. The current compare surface there is intentionally limited to ion density/pressure/momentum plus `Pe`; `phi` and `Vort` are still a follow-on recycling-path capability.
...
- that widened D/T/He/Ne lane now also has a practical transient ladder: `tokamak_recycling_dthene_one_step` has committed summary/array baselines plus a committed optional-history cache and already stays in a tight operational band on the native one-step path, and live Hermes-backed `nout=3` and `nout=5` short-window probes now also clear bounded mixed operational gates on the same neon-enabled surface. The next direct tokamak move should therefore be a longer-window transient or broader distributed-guard closure rather than more first-step tuning on this rung.
- shared open-field operator utilities are now available in [open_field.py](src/jax_drb/native/open_field.py), covering no-flow guard fills, limited free extrapolation, electron force balance, parallel electric-force deposition, and target-recycling source assembly before these terms are wired into the coupled native recycling runner;
...
- the current transient blocker is now localized by the short-step reference probe in `diagnose_recycling_transient_step.py`:
...
- parity for those first-output cases is still blocked, but the remaining defect is now tightly localized:
  ...
  - the newest dense-history diagnostics now separate that target-band drift into field history and controller/source history:
    - [diagnose_recycling_target_cell_history.py](scripts/diagnose_recycling_target_cell_history.py) shows the earliest visible single-species drift at `t = 25` in `NVd+`, followed by `Nd`, then `Nd+` / `Pe`;
    - [diagnose_recycling_controller_history.py](scripts/diagnose_recycling_controller_history.py) shows that the controller multiplier and target-recycling source do drift, but by much less than the target-cell state itself, so the remaining Step 2 blocker is not just the PI-source history in isolation;
  ...
  - the new committed-snapshot A/B probe closes the guard-template question as well: [cache_open_field_dump_case.py](scripts/cache_open_field_dump_case.py) now writes committed RHS snapshot caches for `recycling_1d_rhs` and `recycling_dthe_rhs`, and `diagnose_open_field_recycling_templates.py` compares the default native one-step path against a field-template replay probe that pins non-active cells to the reference-evolved state. That replay probe is much worse on both families than the default native solve, so the remaining Step 2 blocker is not non-active guard-template uncertainty; it is active transient evolution itself;
  ...
  - the new [diagnose_recycling_neutral_transient.py](scripts/diagnose_recycling_neutral_transient.py) report now makes the denominator issue explicit:
  ...
  - the latest localized recycling RHS fix removes most of that short-window neutral-side gap:
    ...
    - [neutral_mixed.py](src/jax_drb/native/neutral_mixed.py) now follows the reference `DDY / sqrt(g_22)` centered metric form for `Grad_par` instead of the older `1 / (J * Δy)` approximation, which materially reduces the target-adjacent ion-momentum remainder;
...
- the current staged `integrated_2d_recycling_rhs` surface is also not end-to-end differentiable yet:
  - the present `grad` barrier is a `TracerArrayConversionError` triggered by `np.asarray(..., copy=True)` in `_initialize_species()` inside [recycling_1d.py](src/jax_drb/native/recycling_1d.py);
  - an xfailed regression now tracks that limitation directly in [test_native_integrated_2d_recycling.py](tests/test_native_integrated_2d_recycling.py);
...
- [evolve_density_rhs.json](references/baselines/reference/evolve_density_rhs.json)
- [diffusion_one_step.json](references/baselines/reference/diffusion_one_step.json)
- [diffusion_short_window.json](references/baselines/reference/diffusion_short_window.json)
- [fluid_1d_mms_rhs.json](references/baselines/reference/fluid_1d_mms_rhs.json)
- [fluid_1d_mms_one_step.json](references/baselines/reference/fluid_1d_mms_one_step.json)
- [fluid_1d_mms.json](references/baselines/reference/fluid_1d_mms.json)
- [vorticity_rhs.json](references/baselines/reference/vorticity_rhs.json)
- [vorticity_one_step.json](references/baselines/reference/vorticity_one_step.json)
- [vorticity_short_window.json](references/baselines/reference/vorticity_short_window.json)
- [neutral_mixed_rhs.json](references/baselines/reference/neutral_mixed_rhs.json)
- [neutral_mixed_one_step.json](references/baselines/reference/neutral_mixed_one_step.json)
- [neutral_mixed_short_window.json](references/baselines/reference/neutral_mixed_short_window.json)
- [drift_wave_rhs.json](references/baselines/reference/drift_wave_rhs.json)
- [drift_wave_one_step.json](references/baselines/reference/drift_wave_one_step.json)
- [drift_wave_short_window.json](references/baselines/reference/drift_wave_short_window.json)
- [blob2d_rhs.json](references/baselines/reference/blob2d_rhs.json)
- [blob2d_one_step.json](references/baselines/reference/blob2d_one_step.json)
- [blob2d_short_window.json](references/baselines/reference/blob2d_short_window.json)

The compact benchmark-metric references are:

- [blob2d_short_window_metrics.json](references/baselines/reference_metrics/blob2d_short_window_metrics.json)
- [neutral_mixed_rhs_diagnostics.json](references/baselines/reference_metrics/neutral_mixed_rhs_diagnostics.json)
- [neutral_mixed_short_window_metrics.json](references/baselines/reference_metrics/neutral_mixed_short_window_metrics.json)
- [docs/data/alfven_wave_short_window_analysis.json](docs/data/alfven_wave_short_window_analysis.json)
- [docs/data/alfven_wave_short_window_parity.json](docs/data/alfven_wave_short_window_parity.json)
...
Future JAX runs should emit the same portable schema through the generic summary helpers in [portable.py](src/jax_drb/parity/portable.py), so that `jax-drb compare-summary` can be used unchanged for reference vs. JAX comparisons.
