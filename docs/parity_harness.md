...
| `neutral_mixed_short_window` | `open native runner target` | Baseline exists and the native implicit runner path now executes this lane. The matrix-free runner path now clears a bounded full short-window metric gate inside the local ten-minute policy, including total-density and total-pressure histories; the remaining gap is the broader full-array short-window field surface. |
...
- that widened D/T/He/Ne lane now also has a practical transient ladder: `tokamak_recycling_dthene_one_step` has committed summary/array baselines plus a committed optional-history cache and already stays in a tight operational band on the native one-step path, and a live Hermes-backed `nout=3` short-window probe now also clears a bounded mixed operational gate on the same neon-enabled surface. The next direct tokamak move should therefore be a longer-window transient or broader distributed-guard closure rather than more first-step tuning on this rung.
