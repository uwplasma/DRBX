...
| `tokamak_recycling_one_step` | `live-reference target` | Curated small-step direct tokamak-2D recycling transient rung on the same example and launcher split, currently staged with `timestep=1` so the first direct tokamak transient parity loop is practical before promoting the full default `timestep=5000` output interval. Optional-history cache support is now in-tree for this rung, and the next gating step is to commit its first summary/array baselines. |
