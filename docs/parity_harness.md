...
- focused production probes show that neither of the obvious staged fixes solves that remaining gap cleanly: carrying dump-backed `SPe` into the transient reduces the worst target-corner `Pe` miss but worsens the adjacent target-band cell, and disabling dump-target preservation improves `Pe` while substantially degrading `Nd+`, `Pd+`, and `NVd+`. That keeps the next Step 3 production work tightly focused on the target-band transient update itself.
