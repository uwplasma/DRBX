...
### 2026-03-10 literal edge-block neighbor-plane reconstruction and long-window diagnostic panel

A follow-on literal ExB runtime probe tested importing neighboring parallel
planes for the **advected field only** at the first and last processor-like
edge blocks. On the 1-step strict audit at
`runs/audit_literal_edge_neighbor_field_1step`,
that improves `n` and `Pe` ExB slightly:

- `n advection/exb`: `array_rel_diff 0.06090172614132968 -> 0.060766993509094175`
- `Pe advection/exb`: `array_rel_diff 0.0660107963883212 -> 0.06587254628876064`

But the same probe regresses the dump-backed literal vorticity ExB check in
`tests/hermes_literal/test_literal_vorticity.py`,
dropping the source-term correlation to `0.8494116815859967`. That probe was
therefore reverted and is not part of the active solver baseline.

The benchmark strict config
`examples/open_field_line/input_tokamak_bxcv_benchmark_hermes_strict.toml`
was also refreshed to use the current literal parity path:
...
- `geometry.coeff_path = "examples/open_field_line/axisym_tokamak_bxcv_hermes_norm_parcurv_g22.npz"`

That refreshed benchmark path was then run against the longer Hermes dataset
`runs/hermes_open_field_05/data` to `t = 0.36`. The comparison artifacts are:

- `runs/tokamak_benchmark_literal_t036/bundle_hermes_short.npz`
- `runs/tokamak_benchmark_literal_t036/bundle_jax_short.npz`
- `runs/tokamak_benchmark_literal_t036/figures/tokamak_sol_benchmark_panel.png`
- `runs/tokamak_benchmark_literal_t036/figures/tokamak_sol_benchmark_overview.png`
- `runs/tokamak_benchmark_literal_t036/figures/tokamak_sol_benchmark_panel.csv`

The new overview figure is generated by:

```bash
PYTHONPATH=src python tools/plot_benchmark_overview.py \
  --hermes runs/tokamak_benchmark_literal_t036/bundle_hermes_short.npz \
  --jax runs/tokamak_benchmark_literal_t036/bundle_jax_short.npz \
  --out runs/tokamak_benchmark_literal_t036/figures/tokamak_sol_benchmark_overview.png \
  --plane xz \
  --coeff-path examples/open_field_line/axisym_tokamak_bxcv_hermes_norm_parcurv_g22.npz \
  --snapshot-mode fluct
```

This long-window comparison is explicitly **diagnostic**, not a promotion
signal. The resulting bundles show that the literal engine is still far from
long-window Hermes parity:

- benchmark mean relative diagnostic L2: `1.1728155373109848`
- benchmark max relative diagnostic L2: `2.153985971639393`
- end-of-window RMS relative errors from
  `runs/tokamak_benchmark_literal_t036/figures/tokamak_sol_benchmark_panel.csv`:
  - `n`: `-0.9838326450588256`
  - `Te`: `-0.9599748970605642`
  - `omega`: `-0.9875844958024667`
  - `phi`: `-0.9087412180841332`

So the new panel is useful as a regression and visualization artifact, but it
confirms that the remaining live solver gap still blocks any claim of benchmark
parity or shipping readiness.
