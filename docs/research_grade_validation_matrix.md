...
The parallel-viscosity source remains the next active/target offender, so the
next native fix should reconcile neutral-viscosity preparation or boundary
sequencing before changing the viscosity stencil.

| Open lane | Current evidence | Command surface | Publication-ready output |
| --- | --- | --- | --- |
| `neutral_mixed_eta_h_boundary` | Clean patched reference checkout writes valid JSONL; native/reference traces share the 10 required state, RHS, and source fields; native replay on the reference accepted-step grid matches `148/148` points. Native and reference traces both emit `Vh` and `eta_h`; the current comparator ranks `eta_h` input drift as the leading target/guard offender (`3.23e-3`) and `SNVh_parallel_viscosity` as the next active/target source offender (`5.35e-5`). | `PYTHONPATH=src jax-drb trace-neutral-mixed-accepted-steps --reference-trace-jsonl ...`; `PYTHONPATH=src jax-drb trace-neutral-mixed-reference-accepted-steps ...`; `PYTHONPATH=src jax-drb compare-neutral-mixed-accepted-traces ...`. | Target-band accepted-step figure and JSON report comparing `Nh`, `Ph`, `NVh`, `Vh`, `eta_h`, `ddt(*)`, `SNVh`, and `SNVh_*` on the reference time grid with active/guard metrics separated. |
