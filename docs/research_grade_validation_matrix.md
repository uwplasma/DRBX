...
roundoff. The accepted-step comparator also ranks `Nh`, `Ph`, and `NVh`
state-input errors and reports that the current `eta_h` target-adjacent drift is
about `99` times larger than the largest state-input drift, so the
parallel-viscosity source remains an accepted-step closure or
