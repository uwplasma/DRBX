...
`eta_h` input traces present, the leading target-adjacent offender is
`eta_h` input drift rather than a missing pressure-gradient source formula.
...
roundoff, so the parallel-viscosity source remains an accepted-step
state/history or boundary-sequencing offender rather than a formula-level
closure mismatch.

| Open lane | Current evidence | Command surface | Publication-ready output |
| --- | --- | --- | --- |
| `neutral_mixed_eta_h_boundary` | Clean patched reference checkout writes valid JSONL; native/reference traces share the 10 required state, RHS, and source fields; native replay on the reference accepted-step grid matches `148/148` points. Native and reference traces both emit `Vh` and `eta_h`; the current comparator ranks `eta_h` input drift as the leading target/guard offender (`3.23e-3`) and `SNVh_parallel_viscosity` as the next active/target source offender (`5.35e-5`). The final-state input-closure report matches reference `Dnn`, `Vh`, and `eta_h` diagnostics to roundoff, narrowing the remaining blocker to accepted-step state/history or boundary sequencing. | `PYTHONPATH=src jax-drb trace-neutral-mixed-accepted-steps --reference-trace-jsonl ...`; `PYTHONPATH=src jax-drb trace-neutral-mixed-reference-accepted-steps ...`; `PYTHONPATH=src jax-drb compare-neutral-mixed-accepted-traces ...`; `build_neutral_mixed_reference_input_closure_report(...)`. | Target-band accepted-step figure and JSON report comparing `Nh`, `Ph`, `NVh`, `Vh`, `eta_h`, `ddt(*)`, `SNVh`, and `SNVh_*` on the reference time grid with active/guard metrics separated, plus final-state input-closure JSON for `Dnn`, `Vh`, and `eta_h`. |
