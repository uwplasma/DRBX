...
| --- | --- | --- | --- |
| `neutral_mixed_eta_h_boundary` | Clean patched reference checkout writes valid JSONL; native/reference traces share the required state, RHS, and source fields. The preferred max-order-2 reference lane now matches `309/309` accepted times with zero solver-order mismatches, while the older variable-order lane matches `148/148` points and remains useful for source-field coverage. Native and reference traces both emit `Dnnh`, `Vh`, `eta_h`, the `Dnnh` preparation ladder, flattened target-adjacent values, and reference solver order. The native `Grad(logPnlim)` norm now carries the structured metric terms (`g11`, `g22`, `g33`, and supported `g23`) used by the reference. The current max-order-2 comparator ranks `Dnnh_flux_max` as the leading pointwise target-band ladder offender (`5.13e-3`), and the ladder-transition register identifies the dominant jump as `Dnnh_raw -> Dnnh_flux_max` with the target error rising from about `2.83e-4` to about `5.13e-3`. The final-state input-closure report matches reference `Dnn`, `Vh`, and `eta_h` diagnostics to roundoff, so the remaining owner is accepted-step state/history sequencing or near-target flux-limit-cap preparation rather than pressure-gradient or viscosity source formulas. | `PYTHONPATH=src jax-drb trace-neutral-mixed-accepted-steps --reference-trace-jsonl ...`; `PYTHONPATH=src jax-drb trace-neutral-mixed-reference-accepted-steps --cvode-max-order 2 ...`; `PYTHONPATH=src jax-drb compare-neutral-mixed-accepted-traces ...`; `build_neutral_mixed_reference_input_closure_report(...)`. | Target-band accepted-step figure and JSON report comparing `Nh`, `Ph`, `NVh`, `Dnnh`, `Dnnh_flux_max`, `Vh`, `eta_h`, solver order, `ddt(*)`, `SNVh`, and `SNVh_*` on the reference time grid with active/guard metrics and pointwise target deltas separated, plus final-state input-closure JSON for `Dnn`, `Vh`, and `eta_h`. |
| `recycling_fixed_bdf2_jax_promotion` | Fixed-layout RHS diagnostics, JAX-linearized action counts, packed feedback-integral evolution, lightweight promotion gates, and a safety-preserving `linearize` initial-residual mode exist; full output-window promotion and live-reference profiling remain open. | `PYTHONPATH=src python scripts/run_recycling_jvp_promotion_gate.py`; `PYTHONPATH=src python scripts/profile_recycling_jax_linearized_gate.py --case dthe --initial-residual-mode linearize --require-initial-residual-mode linearize`. | Promotion summary JSON, mode-compare JSON, cProfile/RSS/JAX-trace bundle, and a reviewer-facing parity/runtime figure for the fixed-layout recycling lane. |
| `vmec_extender_fci_geometry_promotion` | Native imported-field, selected-field, reduced FCI, metric-MMS, sheath/recycling, vorticity, and PyTree RHS gates exist, and the VMEC-extender synthetic import example is clean-clone runnable. ESSOS imported-FCI routing now also has a self-contained dry-run artifact contract plus live report diagnostics for connection-length health, grid/refinement metadata, exact sheath consumption of forward/backward map masks, directional target labels, direction-specific wall-hit coverage, endpoint-length availability, and a clean-clone nested-grid connection-length refinement campaign using manufactured non-axisymmetric levels. The importer now separates raw, adjacent-step, and target-exit length arrays; VMEC and hybrid adjacent-step-per-radian probes pass the three-level observed-order gate, while raw coil/hybrid length and pure coil adjacent-step tracing remain negative controls. Full non-axisymmetric imported-field turbulence claims still need pure-coil tracing refinement, imported-grid, and timestep sensitivity. | `PYTHONPATH=src python examples/geometry-3D/stellarator-fci/validation_campaign_demo.py`; `.venv/bin/pytest -q tests/test_geometry_fci_maps.py tests/test_validation_stellarator_fci_campaigns.py`; `PYTHONPATH=src python examples/geometry-3D/vmec-extender/imported_field_demo.py`; edit top-level parameters in `examples/geometry-3D/essos-field-lines/imported_fci_campaign.py`, then run `PYTHONPATH=src python examples/geometry-3D/essos-field-lines/imported_fci_campaign.py`; run `PYTHONPATH=src python examples/geometry-3D/essos-field-lines/imported_connection_length_refinement_demo.py`; pass three-or-more live nested imported connection-length grids with `require_observed_order=True` before publication claims. | Poincare/connection-length/refinement reports, conservative-operator MMS figure, reduced-turbulence diagnostics, clean-clone VMEC-extender smoke artifacts, source-aware imported FCI dry-run schema artifacts, target-label, consumed-map, direction-specific wall-hit, and endpoint-length diagnostics, and polished non-axisymmetric movie artifacts. |
