...
linear stability analysis, differentiable reduced studies, curated tokamak
workflows, and reusable 3D geometry diagnostics.
...
| Linear stability | drift-wave, shear-Alfven, and interchange dispersion solver |
...
| Differentiability | `jax.grad`, covariance pushforward, inverse design, and differentiable FCI drift-reduced RHS |
| Validation | manufactured-solution convergence, geometry and operator campaigns, and publication-ready plots |
| Performance evidence | local CPU scaling, profiling bundles, and JAX-native kernel profiling |
...
with [Physics Models](physics_models.md) for citations and derivations, then use
...
simulation payloads, GIFs, and release figures are restored from the release
bundle with `scripts/fetch_example_artifacts.py`.
