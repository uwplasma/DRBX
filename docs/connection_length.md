...
DRBX keeps the connection-length calculation separate from the drift-reduced
...
[`src/drbx/geometry/essos_import.py`](../src/drbx/geometry/essos_import.py)
has three map-source semantics:

- `coil`: field lines are traced with the external coil-field runtime. DRBX
...
- `vmec`: DRBX integrates a VMEC-coordinate map using
...
- `hybrid`: DRBX uses the smooth VMEC map coordinates but keeps the
...
that aggregates `promotion_ready`, `evidence_role`, finest errors, observed
order, and finite-overlap thresholds across all resolved map sources.
