...
- `impurity_radiation_and_detachment_control` is no longer only a missing family label; the dedicated `impurity_radiation_campaign` package now covers neon OpenADAS loading, finite radiation-loss evaluation, and exact `D/T/He/Ne` RHS closure, `controller_feedback_campaign` adds the first dense-history reference-backed controller gate on the native upstream-density feedback path, and `detachment_controller_campaign` now promotes a bounded reduced Hermes-backed detachment-controller lane on the local non-PETSc reference build. A reduced `temperature_feedback_campaign` package is also now in-tree, but the bounded local Hermes Tt-control example still exceeds the current ten-minute validation policy here, so broader production temperature/detachment workflows still remain open beyond the reduced detachment lane.
- `neutral_mixed` is now beyond a prefix-only transient probe: the matrix-free native runner path clears a bounded full short-window centerline gate inside the ten-minute policy, while the broader global mass/pressure short-window surface remains the real remaining blocker.
- `direct_tokamak_recycling` is now wider than the promoted first output: both the compact D/T `nout=2` lane and the richer drift-enabled D/T `nout=2` lane clear bounded mixed operational gates, so the remaining blocker is the broader longer-window/neon surface rather than the existence of any richer live transient gate.
