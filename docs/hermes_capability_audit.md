...
- `direct_tokamak_recycling` is now wider than the promoted first output in two distinct ways: both the compact D/T `nout=2` lane and the richer drift-enabled D/T `nout=2` lane clear bounded mixed operational gates, and the neon-enabled `tokamak_recycling_dthene_one_step` lane now also clears a live Hermes-backed mixed operational gate. The remaining direct-tokamak task is therefore longer-window closure and eventual fully native distributed guard evolution, not the existence of any richer or neon-enabled live transient surface.
