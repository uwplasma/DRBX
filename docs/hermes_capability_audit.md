...
- `impurity_radiation_and_detachment_control` is no longer only a missing family label; the dedicated `impurity_radiation_campaign` package now covers neon OpenADAS loading, finite radiation-loss evaluation, and exact `D/T/He/Ne` RHS closure, while `controller_feedback_campaign` adds the first dense-history reference-backed controller gate on the native upstream-density feedback path. A reduced `temperature_feedback_campaign` package is also now in-tree, but the bounded local Hermes Tt-control example still exceeds the current ten-minute validation policy here, so temperature-feedback and detachment-controller workflows still remain open beyond that.
