...
- shared helper module: [src/drbx/validation/autodiff_diffusion.py](../src/drbx/validation/autodiff_diffusion.py)
- uncertainty helper module: [src/drbx/validation/autodiff_diffusion_uncertainty.py](../src/drbx/validation/autodiff_diffusion_uncertainty.py)
...
- `host_pmap`: one process with `DRBX_HOST_DEVICE_COUNT=N` and device-parallel `pmap`

To inspect the runtime mode directly in a fresh process:

```bash
DRBX_HOST_DEVICE_COUNT=4 PYTHONPATH=src python - <<'PY'
...
- the explicit host-device CPU benchmark uses `DRBX_HOST_DEVICE_COUNT=N` to expose multiple CPU devices before import, then maps the objective with `pmap`
