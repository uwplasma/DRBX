...
| `integrated_2d_production_one_step` | `native-scaffolded` | The broader integrated 2D production workflow now also has a committed one-step rung on the same dump-backed geometry lane. The transient path now uses the stiffer `bdf` backend, preserves only the dump-backed ion target state while letting the electron target state evolve through the sheath closure, and stages per-time dumped ion velocity diagnostics (`Vd+`, `Vd`) when rebuilding the recycling diagnostics. That removes the old dominant `Pe` miss and roughly halves the remaining `Sd_target_recycle` one-step residual, leaving `Sd_target_recycle` as the leading remaining one-step gap with `Pe`, `Ed_target_recycle`, `NVd+`, and `Nd` in the same smaller residual band. |
| `integrated_2d_production_short_window` | `native-scaffolded` | The broader integrated 2D production workflow now also has a committed short-window rung (`nout=5`) on the same dump-backed geometry lane. It is not parity-clean yet, but it widens Step 3 from single-output production checks into a real production transient ladder; after the ion-only target-preservation change and the staged per-time `Vd+`/`Vd` diagnostic reconstruction, the current residuals are still led by `Pe`, `NVd+`, and `Sd_target_recycle`, but the recycling-diagnostic gap is about half of the earlier staged value while `Pd` remains comparatively tight. |
