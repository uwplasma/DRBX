...
| `integrated_2d_recycling_rhs` | `native-scaffolded` | Staged local-dump RHS path now runs natively with slab-local target routing, dump-state preservation, dump-backed source staging, and native reconstruction of the staged recycling diagnostics. |
| `integrated_2d_recycling_one_step` | `scaffolded_reference_backed` | Dump-backed one-step transient baseline is committed and the staged transient path stays inside a bounded operational band, but the lane still depends on dump-seeded state, dump-backed source terms, and target preservation. |
| `integrated_2d_recycling_short_window` | `scaffolded_reference_backed` | The same dump-backed transient scaffold now clears a bounded operational band over the committed short-window baseline. |
| `integrated_2d_recycling_medium_window` | `scaffolded_reference_backed` | The same dump-backed transient scaffold now clears a bounded operational band over the committed medium-window baseline, but it is still staged evidence rather than a claim-bearing native lane. |
