...
| `alfven_wave_rhs` | `native-scaffolded` | First electromagnetic Stage 4 rung now runs through a dump-backed native scaffold on the finite-electron-mass Alfvén-wave benchmark, matching the committed `Apar`, `Ajpar`, `phi`, `Vort`, `NVe`, `ddt(NVe)`, and `ddt(Vort)` baselines exactly while the real EM operators are still being ported. |
| `alfven_wave_one_step` | `native-scaffolded` | First-output electromagnetic state rung now runs through the same dump-backed native scaffold on the Alfvén-wave benchmark, matching the committed `Apar`, `Ajpar`, `phi`, `Vort`, and `NVe` baselines exactly before wider dispersion-history comparisons are ported. |
