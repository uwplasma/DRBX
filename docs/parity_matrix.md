...
| `alfven_wave_rhs` | `reference-only target` | First electromagnetic Stage 4 rung is now staged off the finite-electron-mass Alfvén-wave benchmark, locking `Apar`, `Ajpar`, `phi`, `Vort`, `NVe`, `ddt(NVe)`, and `ddt(Vort)` from a diagnostic `nout=0` reference run. |
| `alfven_wave_one_step` | `reference-only target` | First-output electromagnetic state rung is now staged on the same Alfvén-wave benchmark, locking `Apar`, `Ajpar`, `phi`, `Vort`, and `NVe` before widening into longer dispersion-history comparisons. |
