...
| `recycling_1d_one_step` | `native_operational` | First-output open-field recycling now clears the agreed `rtol <= 2e-1` operational gate against the committed one-step baseline. |
| `recycling_dthe_one_step` | `native_operational` | First-output multi-species open-field recycling now also clears the agreed `rtol <= 2e-1` operational gate against the committed one-step baseline. |
| `integrated_2d_recycling_rhs` | `native-scaffolded` | Staged local-dump RHS path now runs natively with slab-local target routing, dump-state preservation, dump-backed RHS source staging, and native reconstruction of the staged recycling diagnostics. |
| `integrated_2d_recycling_one_step` | `scaffolded_reference_backed` | Dump-backed one-step transient baseline is committed and the staged transient path stays inside a bounded operational band while no longer replaying dump-backed density, pressure, or momentum source fields during the transient. |
| `integrated_2d_recycling_short_window` | `scaffolded_reference_backed` | The same staged transient scaffold now clears a bounded operational band over the committed short-window baseline without replaying dump-backed source fields. |
| `integrated_2d_recycling_medium_window` | `scaffolded_reference_backed` | The same staged transient scaffold now clears a bounded operational band over the committed medium-window baseline, but it is still staged evidence rather than a claim-bearing native lane because it still depends on dump-seeded initial state and target preservation. |
