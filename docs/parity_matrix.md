...
| `integrated_2d_recycling_one_step` | `native_operational` | Dump-backed one-step transient baseline is committed and the native transient path stays inside a bounded operational band while no longer replaying dump-backed density, pressure, or momentum source fields or preserving dump target state during the transient. |
| `integrated_2d_recycling_short_window` | `native_operational` | The same native transient path now clears a bounded operational band over the committed short-window baseline without replaying dump-backed source fields or preserving dump target state. |
| `integrated_2d_recycling_medium_window` | `native_operational` | The same native transient path now clears a bounded operational band over the committed medium-window baseline; it is still operational rather than exact because it starts from a dump-seeded initial state. |
