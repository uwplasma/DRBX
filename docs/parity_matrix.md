...
| `integrated_2d_recycling_medium_window` | `native-scaffolded` | The same integrated 2D recycling workflow now extends to the staged `nout=20` medium window, with the native runner honoring the manifest override rather than the input default `nout=5`. Live differences remain concentrated in `Ed_target_recycle`, `Sd_target_recycle`, `Pe`, and small neutral-side residuals, so this is an extended Step 3 transient scaffold rather than a locked parity case. |
