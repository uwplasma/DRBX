...
| `integrated_2d_production_rhs` | `native-scaffolded` | The broader integrated 2D production workflow now has a committed one-RHS reference rung on the same dump-backed geometry lane used for integrated recycling. The first live native comparison is already limited to small `ddt(Pe)` / `ddt(Pd)` pressure-summary residuals, so this widens Step 3 without adding a new geometry blocker. |
| `integrated_2d_production_one_step` | `native-scaffolded` | The broader integrated 2D production workflow now also has a committed one-step rung on the same dump-backed geometry lane. The transient path now uses the stiffer `bdf` backend, which tightens `Pd` and improves `Pe`; the remaining live residual is led by `Pe`, with `Sd_target_recycle`, `Ed_target_recycle`, `NVd+`, and `Nd` clustered in the same small residual band rather than a broad transient failure. |
| `integrated_2d_production_short_window` | `native-scaffolded` | The broader integrated 2D production workflow now also has a committed short-window rung (`nout=5`) on the same dump-backed geometry lane. It is not parity-clean yet, but it widens Step 3 from single-output production checks into a real production transient ladder; the current residuals are led by `Pe`, then `Sd_target_recycle`, then `NVd+` / `Nd`, while `Pd` is already comparatively tight. |
...
| `alfven_wave_rhs` | `native-scaffolded` | First electromagnetic Stage 4 rung now runs through a dump-backed native scaffold on the finite-electron-mass Alfvén-wave benchmark, matching the committed `Apar`, `Ajpar`, `phi`, `Vort`, `NVe`, `ddt(NVe)`, and `ddt(Vort)` baselines exactly while already computing `Ajpar` natively from the charged momentum sum. |
| `alfven_wave_one_step` | `native-scaffolded` | First-output electromagnetic state rung now runs through the same dump-backed native scaffold on the Alfvén-wave benchmark, matching the committed `Apar`, `Ajpar`, `phi`, `Vort`, and `NVe` baselines exactly, with `Ajpar` now reconstructed natively using the same outer-radial zeroing convention seen in the saved reference field. |
