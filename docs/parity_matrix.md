...
| `integrated_2d_production_one_step` | `native-scaffolded` | The broader integrated 2D production workflow now also has a committed one-step rung on the same dump-backed geometry lane. The transient path now uses the stiffer `bdf` backend and preserves only the dump-backed ion target state while letting the electron target state evolve through the sheath closure; that removes the old dominant `Pe` miss, leaving `Sd_target_recycle` as the leading remaining one-step residual and putting `Pe`, `Ed_target_recycle`, `NVd+`, and `Nd` into the same small residual band. |
| `integrated_2d_production_short_window` | `native-scaffolded` | The broader integrated 2D production workflow now also has a committed short-window rung (`nout=5`) on the same dump-backed geometry lane. It is not parity-clean yet, but it widens Step 3 from single-output production checks into a real production transient ladder; after the ion-only target-preservation change, the current residuals are led by `NVd+` and `Sd_target_recycle`, while `Pe` is much tighter and `Pd` remains comparatively tight. |
