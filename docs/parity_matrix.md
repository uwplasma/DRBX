...
- the first direct tokamak-2D geometry rung is still `tokamak_diffusion_flow_one_step`, and the next recycling lane is now active instead of blocked: `tokamak_recycling_rhs` launches live through the same shared `process_count = 6` harness path, has committed baselines plus a committed snapshot cache, and already matches its direct-tokamak RHS baselines natively; `tokamak_recycling_one_step` is now a committed curated small-step transient rung (`timestep=1`) with optional-history cache support and a native `bdf` plus ion-only target-preservation path that stays within a tight operational band rather than exact bitwise parity;
