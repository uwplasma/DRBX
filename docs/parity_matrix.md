...
| `integrated_2d_recycling_medium_window` | `native-scaffolded` | The same integrated 2D recycling workflow now extends to the staged `nout=20` medium window, with the native runner honoring the manifest override rather than the input default `nout=5`. After fixing the recycling-energy diagnostic path to use the configured sheath `gamma_i`, the remaining meaningful differences are led by `Sd_target_recycle`, then `Pe`, with `Ed_target_recycle` reduced into the small-residual band. |
