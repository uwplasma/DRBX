...
- native `one_rhs` execution for `neutral_mixed_rhs`, including the reference neutral diffusivity formula, wall-consistent neutral guard reconstruction, X/Z face-flux transport stencil, and active-domain parity against trimmed neutral baselines with documented field-level tolerances;
