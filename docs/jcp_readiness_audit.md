...
1. Controller-oriented temperature/detachment physics is still reduced-promoted, not full-production.
   `controller_feedback_campaign`, `temperature_feedback_campaign`, and `detachment_controller_campaign` now all provide bounded controller evidence. The temperature-feedback lane is now a real reduced Hermes-backed gate (`ny=16`, `nout=4`, `timestep=100`) and automatically prepares a clean patched reference worktree when the local Hermes source still carries the known `temperature_feedback.hxx` permission bug. `detachment_controller_campaign` remains the broader reduced detachment lane (`ny=32`, `nout=24`, `timestep=100`). What is still missing is not the existence of bounded controller validation, but a broader production temperature/detachment-control workflow.
...
- `neutral_mixed_short_window` now also clears a bounded full-array `Nh`/`Ph`/`NVh` gate on the trimmed active-domain short-window surface;
- direct tokamak recycling is now widened beyond the earlier two-output D/T windows: the neon-enabled `tokamak_recycling_dthene_one_step` lane clears a live Hermes-backed mixed operational gate, and the same neon-enabled family now also clears bounded `nout=3` and `nout=5` short-window gates;
...
- the reduced temperature-feedback lane is now also reproducible on a bounded patched-reference workflow rather than remaining a timeout-only negative result.
...
1. Promote a broader production temperature/detachment-control lane beyond the reduced controller gates.
2. Promote still longer-window direct tokamak recycling surfaces beyond the current first-output, `nout=2`, and neon-enabled `nout=3`/`nout=5` bounded gates.
3. Decide whether a broader production 3D workflow, rather than only reduced/native-selected 3D lanes, is required for the final broad standalone claim.
