...
| `integrated_2d_recycling_rhs` | `native-scaffolded` | Staged local-dump RHS path now runs natively with slab-local target routing, dump-state preservation, dumped `SPd+` injection into the ion pressure RHS, and a source-faithful `sheath_boundary_simple` electron boundary path; parity is not locked, but the earlier target-row `ddt(Pe)` miss is no longer the dominant defect. |
...
- that Step 3 scaffold is intentionally not counted as a parity win yet: the recent slab-local target, dump-state preservation, dumped `SPd+` injection, and source-faithful `sheath_boundary_simple` electron boundary fixes moved `Sd_target_recycle`, `Ed_target_recycle`, `ddt(Pd+)`, and the dominant `ddt(Pe)` statistics close to the reference, so the remaining live mismatch is now a smaller edge-statistics cleanup rather than a large target-row electron-pressure defect;
