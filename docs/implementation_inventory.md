...
- the newest production-specific diagnostic now compares the native one-step state and the native RHS evaluated on the reference-evolved state for `integrated_2d_production_one_step`. That narrows the remaining blocker further than the older traces: `Sd_target_recycle` and `Ed_target_recycle` are exact on the reference-evolved state, `ddt(NVd+)` is already small, but `ddt(Pe)` is still badly wrong at the target-band cell and `ddt(Nd+)` / `ddt(Pd+)` / `ddt(Nd)` remain visibly off there. The next Step 3 fix should therefore target the local target-band pressure/density RHS assembly rather than more recycling-diagnostic reconstruction.
