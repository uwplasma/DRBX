...
- staged manifest entries for `integrated_2d_recycling_rhs`, `integrated_2d_recycling_one_step`, and `integrated_2d_recycling_short_window`, including external artifact staging and `process_count = 10`, so the 2D recycling branch now has stable integrated geometry targets from one-RHS through the first multi-output transient window rather than the broken tokamak example;
