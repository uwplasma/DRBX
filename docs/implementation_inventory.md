...
- the production harness is now ready for that operator slice: [reference_dump.py](/Users/rogerio/local/jax_drb/src/jax_drb/native/reference_dump.py) now accepts the `(t, x, y)` shape used by the dumped anomalous-coefficient outputs, and the committed [integrated_2d_production_rhs_snapshot.npz](/Users/rogerio/local/jax_drb/references/baselines/reference_snapshots/integrated_2d_production_rhs_snapshot.npz) now carries `anomalous_D`, `anomalous_Chi`, and `anomalous_nu` for both `d+` and `e`. That removes the remaining snapshot/plumbing gap before the actual anomalous-diffusion implementation.
