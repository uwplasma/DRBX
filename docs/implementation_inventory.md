...
- Step 4 is now formally staged with the smallest electromagnetic benchmark in the source test suite:
  ...
  - the first real EM operator slices are now in-tree: `Ajpar` is computed natively from the charged-species momentum sum, and `Apar` is solved natively on the single-cell slab/Neumann Alfvén benchmark from the source-faithful Helmholtz coefficients (`alpha_em` from charged densities, `beta_em` from normalization), including the periodic-Y guard wrap seen in the saved reference field. The live `one_step` / `one_rhs` summary and array compares remain exact after those replacements;
