...
- Step 4 is now formally staged with the smallest electromagnetic benchmark in the source test suite:
  ...
  - the first real EM operator is now in-tree as well: `Ajpar` is computed natively from the charged-species momentum sum on the Alfvén scaffold, with the same outer-radial-plane zeroing convention seen in the saved reference field, and the live `one_step` / `one_rhs` compares remain exact after that replacement;
