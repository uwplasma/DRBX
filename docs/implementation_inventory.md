...
- staged manifest entries for `tokamak_recycling_one_step` and `tokamak_recycling_dthe_one_step`, so the 2D recycling branch now has named geometric targets even though the committed baselines still need curated multi-rank reference launch support;
