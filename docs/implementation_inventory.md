...
- the neutral module now also contains active-domain implicit stepping substrate based on backward-Euler residuals plus matrix-free Newton-Krylov solves, so the remaining neutral transient work can focus on matching the reference multistep/adaptive behavior rather than rebuilding pack/unpack, guard reconstruction, and residual machinery;
- the same module now also exposes an active-domain Jacobian sparsity pattern matching the local neutral `x/y/z` stencil and cross-field coupling, so sparse implicit prototypes can reuse the exact active-state neighborhood structure;
- the neutral implicit branch now also contains grouped difference-quotient sparse Jacobian assembly, a direct sparse Newton/GMRES solve substrate, and a BDF2 residual/step path, which is materially closer to the traced `cvode`/BDF role than the earlier generic BDF wrapper probes;
...
- the sparse direct path is now in-tree, but it is not the default validated stepper yet; the public implicit helpers still default to the matrix-free nonlinear solve while sparse globalization and parity are tightened;
