...
| `integrated_2d_recycling_rhs` | `native-scaffolded` | Staged local-dump RHS path now runs natively with slab-local target routing, dump-state preservation, and electron sheath sink restoration on the preserved dump state, but parity is not locked. |
...
- that Step 3 scaffold is intentionally not counted as a parity win yet: the recent slab-local target, dump-state preservation, and electron-sheath-sink restoration fixes moved `Sd_target_recycle` / `Ed_target_recycle` close to the reference and reduced the `ddt(Pe)` gap substantially, but live comparison is still concentrated in target-row `ddt(Pd+)` with a smaller remaining `ddt(Pe)` residual, so a more faithful 2D pressure/energy boundary treatment is still required before `integrated_2d_recycling_rhs` can be promoted to `native-validated`;
