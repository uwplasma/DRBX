...
- that Step 3 scaffold is intentionally not counted as a parity win yet: the recent slab-local target and dump-state preservation fixes moved `Sd_target_recycle` / `Ed_target_recycle` close to the reference, but live comparison is still concentrated in target-row `ddt(Pd+)` and `ddt(Pe)`, so a more faithful 2D pressure/energy boundary treatment is still required before `integrated_2d_recycling_rhs` can be promoted to `native-validated`;
