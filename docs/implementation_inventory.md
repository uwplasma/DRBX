...
- direct probes against those compact neutral metrics now confirm that the current explicit RK history path still overflows to `NaN` on the short-window case even with heavy substepping, so the next neutral iteration needs a semi-implicit or IMEX transient update rather than more explicit tuning;
