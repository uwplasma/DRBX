...
| `integrated_2d_recycling_rhs` | `native-scaffolded` | Staged local-dump RHS path now runs natively with slab-local target routing, dump-state preservation, electron sheath sink restoration, and dumped `SPd+` injection into the ion pressure RHS, but parity is not locked. |
...
- that Step 3 scaffold is intentionally not counted as a parity win yet: the recent slab-local target, dump-state preservation, electron-sheath-sink restoration, and dumped `SPd+` injection fixes moved `Sd_target_recycle`, `Ed_target_recycle`, and `ddt(Pd+)` close to the reference, so the remaining meaningful live mismatch is now mainly target-row `ddt(Pe)` and the associated electron pressure boundary treatment;
