...
- recent production-specific probes narrowed that one-step blocker further: naively staging dump-backed `SPe` or disabling dump-target preservation both make the overall production one-step surface worse, so the remaining work is in the target-band transient update itself rather than another broad source-override pass.
