...
def build_hermes_capability_audit() -> dict[str, object]:
    families = [
        ...
        },
        {
            ...
            "jax_status": "partially_closed",
            "jax_capability": "native_rhs_exact_bounded_full_short_window_field_window",
            "evidence": [
                "RHS parity is committed",
                "native transient runner path now exists for one-step and short-window",
                "full short-window centerline plus total-density/pressure metric gate now clears on the matrix-free path inside the local ten-minute policy",
                "full short-window field histories for Nh/Ph/NVh now also clear a bounded full-array gate on the same trimmed active domain",
            ],
            "next_gate": "widen neutral beyond the committed short-window field surface only if a broader standalone full-field claim is desired",
        },
        {
        ...
        },
        {
            ...
            "jax_status": "partially_closed",
            "jax_capability": "native_exact_first_output_plus_bounded_two_output_and_neon_short_windows",
            "evidence": [
                ...
                "the same neon-enabled family now also clears a bounded nout=5 mixed operational gate",
            ],
        ...
        },
        {
            ...
            "jax_status": "partially_closed",
            "jax_capability": "dedicated_campaigns_plus_reduced_temperature_and_detachment_controller_lanes",
            "evidence": [
                ...
                "dedicated detachment-controller campaign committed on a broader bounded reduced Hermes-backed CVODE lane (ny=32, nout=24, timestep=100) with nontrivial control response and exact saved-diagnostic balance checks",
                "reduced temperature-feedback campaign now also clears a bounded Hermes-backed CVODE lane (ny=16, nout=4, timestep=100) with exact multiplier/proportional/source balance checks and bounded integral reconstruction",
                "the temperature-feedback lane auto-prepares a clean patched reference worktree when the local Hermes source still carries the known temperature_feedback.hxx permission bug, so the user tree does not need to be modified",
                "full production temperature/detachment-control workflows are still not promoted beyond the reduced controller lanes",
            ],
            "next_gate": "promote a broader production temperature/detachment-control lane beyond the reduced controller gates",
        },
        {
