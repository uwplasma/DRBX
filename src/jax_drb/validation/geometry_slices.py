...
def save_slice_summary_plot(
    ...
    *,
    ...
    axis.plot(coords, means, color="#005f73", linewidth=2.0, label="mean")
    axis.fill_between(coords, mins, maxs, color="#94d2bd", alpha=0.35, label="min/max band")
    axis.set_xlabel(str(report.get("coordinate_name", "coord")))
    axis.set_ylabel(str(report.get("field_name", "field")))
...
def save_slice_gif(
    *,
    ...
    axis.set_xlabel("axis-0")
    axis.set_ylabel("axis-1")
    figure.colorbar(image, ax=axis, shrink=0.88, pad=0.02, label=field_name)
    ...
    def _update(frame_index: int):
        ...
        title.set_text(f"{field_name} · {spec.name} · {spec.coordinate_name}={coord:.3f}")
        return (image, title)
