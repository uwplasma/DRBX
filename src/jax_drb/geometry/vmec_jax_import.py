...
(``JAX_DRB_VMEC_JAX_ROOT``, defaulting to ``~/local/vmec_jax``) the same way
:mod:`jax_drb.geometry.essos_import` imports ESSOS, and adds the small pieces
`jax_drb` examples need on top of a loaded wout:
...
The adapter keeps `jax_drb` importable without vmec_jax: everything raises
...
def _resolve_vmec_jax_root(vmec_jax_root: str | Path | None = None) -> Path:
    ...
    return Path(os.environ.get("JAX_DRB_VMEC_JAX_ROOT", _PRIVATE_DEFAULT_VMEC_JAX_ROOT)).expanduser()
