...
def div_par_mod_and_fvv_open_active(
    field: np.ndarray,
    inertia_density: np.ndarray,
    velocity: np.ndarray,
    wave_speed: np.ndarray,
    *,
    ...
    """Return active-domain paired ``Div_par_mod`` and ``Div_par_fvv`` operators."""
    ...
    if use_jax_backend(field, inertia_density, velocity, wave_speed):
        ...
        div_mod_result = mod_right_flux * flux_factor_rc - mod_left_flux * flux_factor_lc
        div_mod_result = div_mod_result.at[:, 1:, :].add(
            -mod_right_flux[:, :-1, :] * flux_factor_rp[:, :-1, :]
        )
        div_mod_result = div_mod_result.at[:, :-1, :].add(
            mod_left_flux[:, 1:, :] * flux_factor_lm[:, 1:, :]
        )

        div_fvv_result = fvv_right_flux * flux_factor_rc - fvv_left_flux * flux_factor_lc
        div_fvv_result = div_fvv_result.at[:, 1:, :].add(
            -fvv_right_flux[:, :-1, :] * flux_factor_rp[:, :-1, :]
        )
        div_fvv_result = div_fvv_result.at[:, :-1, :].add(
            fvv_left_flux[:, 1:, :] * flux_factor_lm[:, 1:, :]
        )
        return div_mod_result, div_fvv_result
