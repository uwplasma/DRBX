    ...
    *,
    ...
    for reaction in (reactions if isinstance(reactions, tuple) else (reactions,)):
        ...
        if len(lhs) == 2 and lhs[1] == "e" and len(rhs) == 2 and rhs[1] == "2e":
            atom = lhs[0]
            ion = rhs[0]
            result = amjuel_ionisation(atom, ion, species=species, electron_density=electron_density, dataset_scalars=dataset_scalars)
            accumulate_terms(result, density_source, energy_source, momentum_source, diagnostics)
        elif len(lhs) == 2 and lhs[1] == "e" and len(rhs) == 1:
            ion = lhs[0]
            atom = rhs[0]
            result = amjuel_recombination(atom, ion, species=species, electron_density=electron_density, dataset_scalars=dataset_scalars)
            accumulate_terms(result, density_source, energy_source, momentum_source, diagnostics)
...
def neutral_ionisation_collision_rates(
    ...
    *,
    ...
    for sp in species.values():
        if sp.charge > 0.0:
            electron_density = electron_density + sp.charge * np.asarray(prepared[sp.name].density, dtype=np.float64)
...
def neutral_charge_exchange_collision_rates(
    ...
    *,
    ...
    for reaction in (reactions if isinstance(reactions, tuple) else (reactions,)):
        ...
        totals[atom_name] = np.asarray(totals.get(atom_name, 0.0) + prepared[ion_name].density * sigma_v, dtype=np.float64)
    return totals
