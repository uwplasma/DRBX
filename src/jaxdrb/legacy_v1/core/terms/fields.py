...
def _metric_div_coeff(
    ...
    def _plane(field_plane, coeff_plane, gxx_plane, gxy_plane, gyy_plane, j_plane):
        # Hermes/BOUT INVERT_SET boundaries enforce Dirichlet values from the
        # field itself at the radial guard cells. Reuse the same policy in the
        # forward operator to keep omega_from_phi and phi_from_omega consistent.
        ref_field = field_plane if bool(getattr(params, "poisson_invert_set", False)) else None
        bc_plane = _poisson_bc_eval(params, bc_phi, ref=ref_field)
...
def phi_from_omega(
    ...
    if params.poisson_b_weighted:
        ...
        if mode == "hermes":
            ...
            u_guess = None if phi_guess is None else (phi_guess + p_hat)
            if grid is None:
                u = geom.inv_laplacian(rhs, x0=u_guess)
                phi = u - p_hat
            ...
            if (
                ...
                if split_n0 and rhs.ndim == 3:
                    ...
                    if u_guess is not None:
                        u_guess0 = u_guess.mean(axis=2)
                        u_guess1 = u_guess - u_guess0[..., None]
                    u1 = _metric_solve_coeff(
                        params,
                        geom,
                        rhs1,
                        coeff,
                    ...
                    if return_iters:
                        u1, iters1 = u1
                    u1 = u1 - u1.mean(axis=2, keepdims=True)
                    bc_xy = BC2D(
                    ...
                    u0_xy = _metric_solve_xy(
                        params,
                        geom,
                        jnp.swapaxes(rhs0, 0, 1),
                        jnp.swapaxes(coeff0, 0, 1),
                        bc_xy,
                        x0=None if u_guess0 is None else jnp.swapaxes(u_guess0, 0, 1),
                        return_iters=return_iters,
                    )
                    if return_iters:
                        u0_xy, iters0 = u0_xy
                    u0 = jnp.swapaxes(u0_xy, 0, 1)
                    phi = (u0[..., None] + u1) - p_hat
                    if return_iters:
                        iters = jnp.maximum(jnp.asarray(iters0), jnp.asarray(iters1))
                        return phi, iters
                    return phi

                u = _metric_solve_coeff(
                    params,
                    geom,
                    rhs,
                    coeff,
