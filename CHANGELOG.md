...
All notable changes to `drbx` are recorded here. The current development
...
- New `drbx.native.stellarator_turbulence` (whole-step turbulence driver,
  moved out of `tests/`) and `drbx.native.fci_differentiable_case`
  (reusable differentiable-FCI case API, moved out of the example).
- `drbx.config.boutinp.rewrite_input_precision()` (moved from the
  precision-benchmark example).
- vmec_jax geometry adapter (`drbx.geometry.vmec_jax_import`) with
...
- Exposed opt-in `sparse_jvp` and `jax_linearized` recycling solver modes with
  `DRBX_RECYCLING_JACOBIAN_MODE` and `DRBX_RECYCLING_JVP_BATCH_SIZE`
  controls.
