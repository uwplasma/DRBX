...
precision entry with the public ``jax_drb.config.rewrite_input_precision``
helper), runs ``python -m jax_drb`` on each deck in a fresh subprocess
...
def run_precision_case(precision: str, repeat_index: int, input_path: Path, cache_dir: Path) -> dict[str, Any]:
    """Run one jax_drb subprocess and collect timing plus warning counts."""
