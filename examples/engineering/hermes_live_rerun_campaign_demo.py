...
from jax_drb.validation import create_hermes_live_rerun_campaign_package
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Rerun a representative native-versus-live-Hermes validation matrix "
            "and write the JSON, NPZ, and publication-grade PNG artifacts."
        )
    )
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=repo_root / "docs" / "data" / "hermes_live_rerun_campaign_artifacts",
    )
