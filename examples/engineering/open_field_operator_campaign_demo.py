...
from jax_drb.validation import create_open_field_operator_campaign_package


def main() -> None:
    repo_root = Path(__file__).resolve().parents[2]
    artifacts = create_open_field_operator_campaign_package(
        output_root=Path("/pscratch/sd/y/yiqunx/tmp") / "open_field_operator_campaign_artifacts",
    )
