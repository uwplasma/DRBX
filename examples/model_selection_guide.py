...
@dataclass(frozen=True)
class DeckSummary:
    """Metadata extracted by JAXDRB from a generated starter deck."""
...
def validate_deck(path: Path) -> DeckSummary:
    """Load a deck with JAXDRB and extract runtime metadata."""
...
def print_api_discovery(output_root: Path, deck_summaries: tuple[DeckSummary, ...]) -> None:
    examples = {
        "inspect a deck": f"jax_drb inspect {output_root / 'diffusion_start.toml'}",
        "run a deck": f"jax_drb run {output_root / 'diffusion_start.toml'} --verbose",
        ...
        "discover public native API": "python -c \"import jax_drb.native as n; print(n.__all__)\"",
    }
