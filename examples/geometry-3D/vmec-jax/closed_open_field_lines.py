...
# the same checkout (JAX_DRB_ESSOS_ROOT).
...
if not vmec_jax_runtime_available():
    raise SystemExit(
        ...
        "    JAX_DRB_VMEC_JAX_ROOT=~/local/vmec_jax JAX_DRB_ESSOS_ROOT=~/local/ESSOS_test "
        "PYTHONPATH=src python examples/geometry-3D/vmec-jax/closed_open_field_lines.py"
    )
if not essos_runtime_available():
    raise SystemExit(
        "ESSOS is not importable. Point JAX_DRB_ESSOS_ROOT at a checkout, e.g.\n"
        "    JAX_DRB_ESSOS_ROOT=~/local/ESSOS_test PYTHONPATH=src python "
