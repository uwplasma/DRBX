"""Direct ESSOS coil-field open-SOL promotion workflow (contract by default).

The script drives the pure direct-coil open-field-line lane of the ESSOS
imported-geometry campaign. By default every live gate is disabled and the run
is fully self-contained: it writes the FCI dry-run contract plus a promotion
ledger (``data/essos_direct_coil_open_sol_workflow_summary.json``) that records
which gates would have to pass -- FCI endpoint/source, source-profile,
endpoint-label refinement (plain, collocated, boundary-resolved),
connection-length refinement, reduced-transient stationarity, and diagnostic
media -- before any direct-coil movie may be promoted.

Live gates need an ESSOS checkout plus the Landreman-Paul QA coil JSON: set
``COIL_JSON_PATH``/``VMEC_WOUT_PATH``/``ESSOS_ROOT`` and flip the individual
``RUN_LIVE_*`` flags in the PARAMETERS block. Artifacts land under
``artifacts/essos_direct_coil_open_sol`` (relative to the current working
directory) and every written path is printed.

Edit the PARAMETERS constants below and run from the repository root:

    PYTHONPATH=src python examples/geometry-3D/essos-field-lines/direct_coil_open_sol.py
