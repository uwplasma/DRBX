"""Hybrid VMEC/coil open-SOL promotion workflow (contract + release audit).

The script drives the hybrid open-field-line lane: smooth VMEC coordinates with
coil-derived endpoint masks and |B| modulation, the planned open-SOL bridge
while pure direct-coil maps remain too rough for promotion. By default it is
self-contained: it writes the FCI dry-run contract, audits the committed
release-backed high-resolution hybrid evidence (FCI report, stationarity
report, refinement summary, media manifest), and writes the promotion ledger
``data/essos_hybrid_open_sol_workflow_summary.json``.

Live gates need an ESSOS checkout plus the Landreman-Paul QA coil JSON and
VMEC wout: set ``COIL_JSON_PATH``/``VMEC_WOUT_PATH``/``ESSOS_ROOT`` and flip
the ``RUN_LIVE_*`` flags in the PARAMETERS block (FCI, source-profile,
parallel-step refinement, stationarity, grid/time movie refinement, media).
Artifacts land under ``artifacts/essos_hybrid_open_sol`` (relative to the
...
Edit the PARAMETERS constants below and run from the repository root:
    ...
    PYTHONPATH=src python examples/geometry-3D/essos-field-lines/hybrid_open_sol.py

(The trailing ``RUN_EXAMPLE`` block is what test loaders toggle off to import
the helper functions without running the workflow.)
"""
