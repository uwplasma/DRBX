    ...
    def update(self, **kwargs):
        ...
        for key, val in kwargs.items():
            if hasattr(params, key):
                params = eqx.tree_at(lambda p: getattr(p, key), params, val)
                continue
            updated = False
            for group_name in ("physics", "transport", "closure", "numerics", "bcs"):
                group = getattr(params, group_name)
                if hasattr(group, key):
                    new_group = eqx.tree_at(lambda g: getattr(g, key), group, val)
...
def update_params_from_dict(params: DRBSystemParams, data: dict) -> DRBSystemParams:
        ...
        for k, v in values.items():
            if hasattr(out, k):
                ...
                if isinstance(attr, eqx.Module) and isinstance(v, dict):
                    ...
                    out = eqx.tree_at(lambda g: getattr(g, k), out, new_attr)
                else:
                    out = eqx.tree_at(lambda g: getattr(g, k), out, v)
            else:
    ...
    for k, v in data.items():
        if hasattr(out, k):
            attr = getattr(out, k)
            if isinstance(attr, eqx.Module) and isinstance(v, dict):
                out = eqx.tree_at(lambda p: getattr(p, k), out, _update_group(attr, v))
            else:
                out = eqx.tree_at(lambda p: getattr(p, k), out, v)
            continue
        # Try nested groups.
        updated = False
        for group_name in ("physics", "transport", "closure", "numerics", "bcs"):
            ...
            if hasattr(group, k):
                ...
                out = eqx.tree_at(lambda p: getattr(p, group_name), out, new_group)
                updated = True
                break
        if not updated:
            raise AttributeError(f"Unknown parameter: {k}")
