...
class DRBSystem(eqx.Module):
    ...
    def rhs_split(self, t: float, y: DRBSystemState) -> DRBSystemSplit:
        if self.params.model_kind == "hw":
            return self._rhs_hw_split(t, y)
        return self._rhs_drb_split(t, y)

    def _rhs_hw_split(self, t: float, y: DRBSystemState) -> DRBSystemSplit:
        _ = t
        if not self._is_2d():
            raise ValueError("HW model is only supported for 2D geometries.")
        ...
        phi = self._phi_from_omega(omega, n)
        ...
        dn_couple = couple
        dw_couple = couple
        dn_diff = float(self.params.Dn) * lap_n
        dw_diff = float(self.params.DOmega) * lap_w
        dn_hyper = -float(self.params.nu4_n) * bih_n
        ...
        dissipative = DRBSystemState(
            n=dn_couple + dn_diff + dn_hyper,
            omega=dw_couple + dw_diff + dw_hyper,
            vpar_e=jnp.zeros_like(y.vpar_e),
            vpar_i=jnp.zeros_like(y.vpar_i),
            Te=jnp.zeros_like(y.Te),
            Ti=None if y.Ti is None else jnp.zeros_like(y.Ti),
    ...
    def _energy_hw(self, y: DRBSystemState) -> jnp.ndarray:
    ...
    def _energy_budget_hw(self, y: DRBSystemState) -> dict[str, jnp.ndarray]:
        n = y.n
        omega = y.omega
        phi = self._phi_from_omega(omega, y.n)
        bc_n = self._bc_or(self.params.bc_n)
        bc_omega = self._bc_or(self.params.bc_omega)
        bc_phi = self._bc_phi()
