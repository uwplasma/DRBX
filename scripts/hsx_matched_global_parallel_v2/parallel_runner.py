...
The numerical kernels live in ``frozen_numerics_v2.py``.  This file only
...
CONFIG_SCHEMA = "drbx.hsx-matched-cubic-portable-config-v2"
PLAN_SCHEMA = "drbx.hsx-matched-cubic-work-plan-v2"
RECEIPT_SCHEMA = "drbx.hsx-matched-cubic-parallel-receipt-v2"
THREAD_ENV = {
    "OMP_NUM_THREADS": "1",
...
def _bootstrap(deployment_root: Path, source_root: Path, output_root: Path):
    ...
    name = "hsx_matched_frozen_numerics_v2"
    if name in sys.modules:
        return sys.modules[name]
    target = HERE / "frozen_numerics_v2.py"
...
def _materialize(
    ...
    runtime = {
        "schema": "drbx.hsx-matched-cubic-global-config-v1",
        "time": portable["time"],
        ...
        "paths": {
            ...
            "derivative_root": str(inputs["derivative_root"]),
            "reference_root": str(inputs["reference_root"]),
            "bounded_root": str(inputs["bounded_root"]),
            "factor_root": str(inputs["factor_root"]),
            "cross_root": str(inputs["cross_root"]),
        },
...
def _manifest_identity(
    ...
    source = _load_json(source_manifest) if source_manifest.is_file() else {
        "frozen_numerics_v2_sha256": _sha256(HERE / "frozen_numerics_v2.py")
    }
    payload = {
...
def command_execute(args: argparse.Namespace) -> int:
    with _exclusive_output(args.output_root):
        ...
        if workers == 1:
            ...
            results = [_run_unit(unit) for unit in plan["units"]]
        else:
            context = mp.get_context("spawn")
            with context.Pool(workers, initializer=_init_worker, initargs=(settings,)) as pool:
                results = list(pool.imap_unordered(_run_unit, plan["units"], chunksize=1))
        receipt = {
            "schema": RECEIPT_SCHEMA, "status": "complete", "resolution": plan["resolution"],
            "coverage": plan["coverage"], "workers": workers, "seconds": time.perf_counter() - started,
...
def _validate_chunks(args: argparse.Namespace, require_receipt: bool = True) -> tuple[dict[str, Any], Any, Path]:
    ...
    if require_receipt and not receipt.is_file():
...
def command_compare(args: argparse.Namespace) -> int:
    ...
    report = {
        "schema": "drbx.hsx-matched-cubic-parallel-comparison-v2", "status": "pass" if exact else "fail",
...
def parser() -> argparse.ArgumentParser:
    ...
    execute.add_argument("--workers", type=int, required=True)
