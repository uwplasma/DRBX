...
The first non-diverted second adapter is now in-tree too: a traced-field-line geometry scaffold that writes a metric report, compact metric arrays, reusable line diagnostics, toroidal-slice summaries, a GIF, and a validation contract on the same public artifact model. That keeps the 3D program honest by pressure-testing the geometry and diagnostics layer beyond a single tokamak benchmark family.
