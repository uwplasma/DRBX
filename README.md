...
In practice, that means both open-field one-step recycling lanes are now treated as bounded `native_operational` rungs for project planning: `recycling_1d_one_step` and `recycling_dthe_one_step` both clear a relative error band of `rtol <= 2e-1` against their committed one-step baselines, and the one-step runner no longer relies on a synthesized final-state template for non-active cells. That is sufficient to unblock the remaining 2D closure work without overstating them as exact.

One consequence of that policy is that some direct tokamak recycling rungs remain intentionally labeled `native_operational` or `scaffolded_reference_backed`: when a local slab does not own a physical target on one side, the missing guard row is a communicated neighbor state rather than a local sheath boundary. The current `tokamak_recycling_dthe_one_step` rung no longer depends on committed guard-row replay inside the active solve, but it is still not counted as native closure until the fully native distributed recycling backbone is in place.

The same cleanup is now applied on the staged integrated 2D recycling lane too: its transient path no longer depends on dump-backed density, pressure, or momentum source replay, but it still relies on dump-seeded initial state and target preservation, so it remains `scaffolded_reference_backed` rather than claim-bearing native evidence.
