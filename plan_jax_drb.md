...
| B7 | V1/V2 | FCI verification in non-axisymmetric field: rotating-ellipse parallel-operator convergence; filament propagation | BSTING: Shanahan et al., PPCF 61, 025007 (2019), arXiv:1808.08899 | parallel-operator order ~2 with bounded perpendicular pollution; toroidally nonuniform filament dynamics — **DONE (parallel-operator convergence)**: `build_rotating_ellipse_geometry` (autodiff metric) + `tests/test_rotating_ellipse_fci.py` gate direct & traced-field-line parallel gradient at order ≈2 on a genuinely ζ-dependent metric, plus shape-differentiability; filament propagation still open |
