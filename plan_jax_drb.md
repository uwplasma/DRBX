...
| B7 | V1/V2 | FCI verification in non-axisymmetric field: rotating-ellipse parallel-operator convergence; filament propagation | BSTING: Shanahan et al., PPCF 61, 025007 (2019), arXiv:1808.08899 | parallel-operator order ~2 with bounded perpendicular pollution; toroidally nonuniform filament dynamics — **DONE**: `build_rotating_ellipse_geometry` (autodiff metric) + `tests/test_rotating_ellipse_fci.py` gate direct & traced-field-line parallel gradient at order ≈2 on a genuinely ζ-dependent metric, plus shape-differentiability; `tests/test_rotating_ellipse_filament.py` gates a seeded four-field filament generating interchange vorticity on the rotating surfaces (fixed a latent conservative-diffusion-stencil bug in the four-field lane to unblock it) |
| B8 | V2 | Stellarator island-divertor isothermal turbulence (analytic field) | Shanahan et al., JPP 90 (2024), arXiv:2403.18220; GBS: Coelho et al., NF 64, 076057 (2024) | dominant island-mode structure and outboard positive skewness; open-SOL source accounting closes — **CORE DONE**: `geometry/island_divertor.py` (sheared iota + 3 resonant chains; connection-length tracer; emergent open masks) gated in `tests/test_island_divertor.py` (topology + emergent masks + turbulence draining through the divertor); saturated-statistics skewness/spectrum comparison still open |
